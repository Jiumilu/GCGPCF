...
| XWAIL | `ready_for_review / local_dev_boundary` | `docs/harness/XWAIL/evidence/xwail-min-validator-runtime-20260625.md` | 最小 Validator/XAP 命令已在 local dev 通过；不得声明完整工具链闭环、WAES 发布或 AaaS 绑定完成 |
| AaaS | `ready_for_review / local_dev_boundary` | `docs/harness/AaaS/evidence/aaas-service-runtime-20260625.md` | 最小 ServicePackage/Metering/SLA/EvidenceRequirement 命令已在 local dev 通过；不得声明真实计费、客户订阅、商业交付或 WAES 发布完成 |
...
| Brain | `ready_for_review / authorization_boundary / ready_for_human_review` | `docs/harness/Brain/evidence/brain-review-handoff-20260625.md` | A1/A2/A3、KDS RAG export 依赖输入和 Brain review handoff 门禁已通过；不得升级 accepted、integrated、production_ready |
...
| 依赖链 | 当前状态 | 风险 | 下一步 |
|---|---|---|---|
| `WAES -> XWAIL -> AaaS` | `partial_verified / authorization_boundary` | XWAIL 最小 Validator/XAP 与 AaaS 最小 ServicePackage/Metering/SLA/EvidenceRequirement 均已在 local dev 验证；WAES 综合检查仍受 lint 与授权边界阻断，且 WAES 未授权/发布 | 下一步在用户授权后修复 WAES lint/runtime，并将 XWAIL/AaaS 候选输入接入 WAES 注册、授权、发布门禁 |
