#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 025."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-025-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-025-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-025.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-024-20260621.json"
...
LAND_ECOLOGY_KEYS = [
    ...
    "environmental_impact_assessment_gaps",
    "biodiversity_baseline_gaps",
    "protected_area_screening_gaps",
    "soil_groundwater_assessment_gaps",
    "restoration_mitigation_plan_gaps",
    "accepted_for_land_ecology_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("land_ecology_state") != "blocked_no_real_source_record":
        failures.append("land_ecology_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-025-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_025_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-025", "invalid round id")
    ...
    require(evidence.get("land_ecology_requirements") == [
        "land_use_permit",
    ...
    ], "land ecology requirements mismatch")
    ...
    require(checks.get("land_ecology_state") == "blocked_no_real_source_record", "land ecology state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-025-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-025-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 025 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "land_use_permit_gap",
        ...
        require(phrase in loop_round, f"loop round missing land ecology case: {phrase}")

    print(
        "was_real_source_record_monitor_025=pass "
        "submitted_real_candidate_files=0 land_use_permit_gaps=0 environmental_impact_assessment_gaps=0 "
        "biodiversity_baseline_gaps=0 protected_area_screening_gaps=0 soil_groundwater_assessment_gaps=0 "
        "restoration_mitigation_plan_gaps=0 accepted_for_land_ecology_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-026"
    )
    return 0
