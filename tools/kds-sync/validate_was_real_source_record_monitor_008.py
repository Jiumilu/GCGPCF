#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 008."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-008-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-008-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-008.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-007-20260621.json"
...
CONSISTENCY_KEYS = ["owner_confirmation_missing", "issuer_confirmation_missing", "owner_runtime_context_mismatch", "kds_backlink_runtime_context_mismatch"]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CONSISTENCY_KEYS + ["accepted_for_next_gate"]:
    ...
    if value.get("candidate_consistency_state") != "not_applicable_no_submission":
        failures.append("candidate_consistency_state_must_be_not_applicable")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-008-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_008_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-008", "invalid round id")
    ...
    require(checks.get("candidate_consistency_state") == "not_applicable_no_submission", "candidate consistency state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-008-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-008-negative-*.json"))
    require(len(negatives) == 4, "negative fixture count must be 4")
    ...
    require("真实 P4 输入 monitor 008 已建立" in loop_round, "loop round feedback missing")
    for phrase in CONSISTENCY_KEYS:
        require(phrase in loop_round, f"loop round missing consistency case: {phrase}")

    print(
        "was_real_source_record_monitor_008=pass "
        "submitted_real_candidate_files=0 owner_confirmation_missing=0 issuer_confirmation_missing=0 "
        "owner_runtime_context_mismatch=0 kds_backlink_runtime_context_mismatch=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-009"
    )
    return 0
