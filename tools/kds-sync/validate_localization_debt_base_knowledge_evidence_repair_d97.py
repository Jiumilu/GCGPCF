#!/usr/bin/env python3
"""Validate D97 scoped base-knowledge evidence localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-base-knowledge-evidence-repair-d97-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-base-knowledge-evidence-repair-d97-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D97-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_base_knowledge_evidence_repair_d97=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d97_evidence_or_loop")
    ...
    if len(changed_files) != 8:
        fail("changed_file_count_not_8")
    if evidence.get("changed_line_items") != 58:
        fail("changed_line_items_not_58")
    ...
    if delta.get("target_file_findings") != -24:
        fail("unexpected_target_delta")
    after = evidence.get("after", {})
    if after.get("target_file_findings") != 0:
        fail("after_target_not_zero")

    print("localization_debt_base_knowledge_evidence_repair_d97=pass")
