#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 032."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-032-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-032-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-032.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-031-20260622.json"
...
FINANCIAL_SETTLEMENT_KEYS = [
    "sales_invoice_gaps",
    "payment_receipt_gaps",
    "credit_note_or_adjustment_gaps",
    "tax_filing_record_gaps",
    "green_subsidy_or_carbon_credit_revenue_record_gaps",
    "insurance_claim_record_gaps",
    "accepted_for_financial_settlement_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in FINANCIAL_SETTLEMENT_KEYS:
    ...
    if value.get("financial_settlement_state") != "blocked_no_real_source_record":
        failures.append("financial_settlement_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-032-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_032_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-032", "invalid round id")
    ...
    require(evidence.get("financial_settlement_requirements") == [
        "sales_invoice",
        "payment_receipt",
    ...
    ], "financial settlement requirements mismatch")
    ...
    require(checks.get("financial_settlement_state") == "blocked_no_real_source_record", "financial settlement state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-032-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-032-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 032 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "sales_invoice_gap",
        "payment_receipt_gap",
        ...
        require(phrase in loop_round, f"loop round missing financial settlement case: {phrase}")

    print(
        "was_real_source_record_monitor_032=pass "
        "submitted_real_candidate_files=0 sales_invoice_gaps=0 "
        "payment_receipt_gaps=0 credit_note_or_adjustment_gaps=0 "
        "tax_filing_record_gaps=0 green_subsidy_or_carbon_credit_revenue_record_gaps=0 "
        "insurance_claim_record_gaps=0 accepted_for_financial_settlement_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-033"
    )
    return 0
