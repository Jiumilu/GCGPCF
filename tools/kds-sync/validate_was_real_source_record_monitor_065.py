#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 065."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-065-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-065-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-065.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-064-20260622.json"
...
CIRCULARITY_KEYS = [
    "epr_registration_gaps",
    "producer_responsibility_scheme_membership_gaps",
    "epr_fee_payment_record_gaps",
    "takeback_target_report_gaps",
    "regulator_acknowledgement_gaps",
    "epr_audit_evidence_gaps",
    "epr_corrective_action_closure_gaps",
    "accepted_for_epr_compliance_profile",
]
REQUIREMENTS = [
    "epr_registration",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("epr_compliance_state") != "blocked_no_real_source_record":
        failures.append("epr_compliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-065-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_065_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-065", "invalid round id")
    ...
    require(evidence.get("epr_compliance_requirements") == REQUIREMENTS, "EPR compliance requirements mismatch")
    ...
    require(checks.get("epr_compliance_state") == "blocked_no_real_source_record", "EPR compliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-065-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-065-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 065 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing EPR compliance case: {phrase}")

    print(
        "was_real_source_record_monitor_065=pass "
        "submitted_real_candidate_files=0 epr_registration_gaps=0 producer_responsibility_scheme_membership_gaps=0 "
        "epr_fee_payment_record_gaps=0 takeback_target_report_gaps=0 "
        "regulator_acknowledgement_gaps=0 epr_audit_evidence_gaps=0 "
        "epr_corrective_action_closure_gaps=0 "
        "accepted_for_epr_compliance_profile=0 accepted_for_next_gate=0 "
