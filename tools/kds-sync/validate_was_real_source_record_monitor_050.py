#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 050."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-050-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-050-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-050.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-049-20260622.json"
...
SUPPLIER_GOVERNANCE_KEYS = [
    "supplier_qualification_file_gaps",
    "supplier_audit_report_gaps",
    "supplier_code_of_conduct_acknowledgement_gaps",
    "supplier_due_diligence_record_gaps",
    "corrective_action_followup_gaps",
    "subcontractor_disclosure_gaps",
    "supplier_performance_review_gaps",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SUPPLIER_GOVERNANCE_KEYS:
    ...
    if value.get("supplier_governance_state") != "blocked_no_real_source_record":
        failures.append("supplier_governance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-050-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_050_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-050", "invalid round id")
    ...
    require(evidence.get("supplier_governance_requirements") == REQUIREMENTS, "supplier governance requirements mismatch")
    ...
    require(checks.get("supplier_governance_state") == "blocked_no_real_source_record", "supplier governance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-050-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-050-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 050 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing supplier governance case: {phrase}")

    print(
        "was_real_source_record_monitor_050=pass "
        "submitted_real_candidate_files=0 supplier_qualification_file_gaps=0 "
        "supplier_audit_report_gaps=0 supplier_code_of_conduct_acknowledgement_gaps=0 "
        "supplier_due_diligence_record_gaps=0 corrective_action_followup_gaps=0 "
        "subcontractor_disclosure_gaps=0 supplier_performance_review_gaps=0 "
        "accepted_for_supplier_governance_profile=0 accepted_for_next_gate=0 hold_required=1 "
