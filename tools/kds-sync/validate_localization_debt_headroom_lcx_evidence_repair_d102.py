#!/usr/bin/env python3
"""Validate D102 scoped Headroom LCX evidence localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-headroom-lcx-evidence-repair-d102-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-headroom-lcx-evidence-repair-d102-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D102-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_headroom_lcx_evidence_repair_d102=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d102_evidence_or_loop")
    ...
    for key in ("real_kds_api_write", "business_system_writeback", "status_upgrade", "accepted", "integrated", "production_ready", "production_token_measurement"):
    ...
    if len(changed_files) != 4:
        fail("changed_file_count_not_4")
    if evidence.get("changed_line_items") != 66:
        fail("changed_line_items_not_66")
    ...
    if delta.get("target_file_findings") != -6:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -6:
    ...
    print("localization_debt_headroom_lcx_evidence_repair_d102=pass")
