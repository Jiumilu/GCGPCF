#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 024."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-024-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-024-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-024.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-023-20260621.json"
...
PRODUCT_CLAIM_KEYS = [
    "digital_product_passport_gaps",
    "product_labeling_evidence_gaps",
    "qr_serial_identity_gaps",
    "green_claim_substantiation_gaps",
    "customer_visible_claim_gaps",
    "claim_version_control_gaps",
    "accepted_for_product_claim_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("product_claim_state") != "blocked_no_real_source_record":
        failures.append("product_claim_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-024-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_024_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-024", "invalid round id")
    ...
    require(evidence.get("product_claim_requirements") == [
    ...
    ], "product claim requirements mismatch")
    ...
    require(checks.get("product_claim_state") == "blocked_no_real_source_record", "product claim state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-024-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-024-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 024 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing product claim case: {phrase}")

    print(
        "was_real_source_record_monitor_024=pass "
        "submitted_real_candidate_files=0 digital_product_passport_gaps=0 product_labeling_evidence_gaps=0 "
        "qr_serial_identity_gaps=0 green_claim_substantiation_gaps=0 customer_visible_claim_gaps=0 "
        "claim_version_control_gaps=0 accepted_for_product_claim_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-025"
    )
    return 0
