#!/usr/bin/env python3
"""Validate D89 scoped GCKF heading localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d89-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d89-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D89-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_gckf_heading_repair_d89=fail reason={message}")
    sys.exit(1)
...
def load_current_localization() -> dict[str, object]:
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d89_evidence_or_loop")
    ...
    current = load_current_localization()
    current_findings = current.get("sample_findings", [])
    current_keys = {
    ...
    if delta.get("findings") != -5 or delta.get("docs_gc_knowledge_fabric_findings") != -5:
        fail("unexpected_delta")

    print("localization_debt_gckf_heading_repair_d89=pass")
