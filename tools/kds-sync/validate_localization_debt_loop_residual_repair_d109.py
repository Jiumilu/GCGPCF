#!/usr/bin/env python3
"""Validate D109 scoped residual loop localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-loop-residual-repair-d109-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-loop-residual-repair-d109-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D109-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_loop_residual_repair_d109=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d109_evidence_or_loop")
    ...
    if evidence.get("changed_line_items") != 26:
        fail("changed_line_items_not_26")
    ...
    if delta.get("target_file_findings") != -12:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -12:
    ...
    print("localization_debt_loop_residual_repair_d109=pass")
