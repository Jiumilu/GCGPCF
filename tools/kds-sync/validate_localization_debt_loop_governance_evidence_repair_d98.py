#!/usr/bin/env python3
"""Validate D98 scoped Loop governance evidence localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-loop-governance-evidence-repair-d98-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-loop-governance-evidence-repair-d98-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D98-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_loop_governance_evidence_repair_d98=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d98_evidence_or_loop")
    ...
    if len(changed_files) != 7:
        fail("changed_file_count_not_7")
    ...
    if delta.get("target_file_findings") != -22:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -27:
    ...
    print("localization_debt_loop_governance_evidence_repair_d98=pass")
