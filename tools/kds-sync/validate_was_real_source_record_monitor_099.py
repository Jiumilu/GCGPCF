#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 099."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-099-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-099-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-099.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-098-20260622.json"
...
MONITOR_KEYS = [
    ...
    "producer_responsibility_organization_membership_gaps",
    "product_stewardship_plan_approval_gaps",
    "annual_recycling_report_gaps",
    "epr_fee_payment_receipt_gaps",
    "regulator_submission_acknowledgement_gaps",
    "customer_takeback_instruction_publication_gaps",
    "accepted_for_epr_product_stewardship_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "epr_registration_certificate_gap",
    "producer_responsibility_organization_membership_gap",
    "product_stewardship_plan_approval_gap",
    "annual_recycling_report_gap",
    "epr_fee_payment_receipt_gap",
    "regulator_submission_acknowledgement_gap",
    "customer_takeback_instruction_publication_gap",
]
NEGATIVE_FIXTURES = [f"real-source-record-monitor-099-negative-{case}.json" for case in LOOP_CASES]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("epr_product_stewardship_state") != "blocked_no_real_source_record":
        failures.append("epr_product_stewardship_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-099-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_099_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-099", "invalid round id")
    ...
    require(evidence.get("epr_product_stewardship_requirements") == REQUIREMENTS, "epr product stewardship requirements mismatch")
    ...
    require(checks.get("epr_product_stewardship_state") == "blocked_no_real_source_record", "epr product stewardship state mismatch")
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-099-positive.json")
    ...
    require("真实 P4 输入 monitor 099 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing epr product stewardship case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_099=pass submitted_real_candidate_files=0 "
        "epr_registration_certificate_gaps=0 "
        "producer_responsibility_organization_membership_gaps=0 "
        "product_stewardship_plan_approval_gaps=0 "
        "annual_recycling_report_gaps=0 "
        ...
        "regulator_submission_acknowledgement_gaps=0 "
        "customer_takeback_instruction_publication_gaps=0 "
        "accepted_for_epr_product_stewardship_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-100"
    )
    return 0
