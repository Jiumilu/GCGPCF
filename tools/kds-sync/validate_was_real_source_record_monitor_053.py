#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 053."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-053-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-053-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-053.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-052-20260622.json"
...
SHIPMENT_READINESS_KEYS = [
    "supplier_production_plan_gaps",
    "supplier_material_allocation_gaps",
    "in_process_inspection_record_gaps",
    "pre_shipment_inspection_record_gaps",
    "packing_list_gaps",
    "advance_shipping_notice_gaps",
    "supplier_shipment_release_gaps",
    "accepted_for_supplier_shipment_readiness_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SHIPMENT_READINESS_KEYS:
    ...
    if value.get("supplier_shipment_readiness_state") != "blocked_no_real_source_record":
        failures.append("supplier_shipment_readiness_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-053-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_053_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-053", "invalid round id")
    ...
    require(evidence.get("supplier_shipment_readiness_requirements") == REQUIREMENTS, "shipment readiness requirements mismatch")
    ...
    require(checks.get("supplier_shipment_readiness_state") == "blocked_no_real_source_record", "shipment readiness state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-053-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-053-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 053 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing shipment readiness case: {phrase}")

    print(
        "was_real_source_record_monitor_053=pass "
        "submitted_real_candidate_files=0 supplier_production_plan_gaps=0 "
        "supplier_material_allocation_gaps=0 in_process_inspection_record_gaps=0 "
        "pre_shipment_inspection_record_gaps=0 packing_list_gaps=0 "
        "advance_shipping_notice_gaps=0 supplier_shipment_release_gaps=0 "
        "accepted_for_supplier_shipment_readiness_profile=0 accepted_for_next_gate=0 "
