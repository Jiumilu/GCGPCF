#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 051."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-051-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-051-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-051.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-050-20260622.json"
...
SUPPLIER_ONBOARDING_KEYS = [
    "supplier_master_data_approval_gaps",
    "supplier_contract_agreement_gaps",
    "bank_account_verification_gaps",
    "tax_registration_record_gaps",
    "sanctions_screening_result_gaps",
    "export_control_screening_gaps",
    "supplier_onboarding_approval_gaps",
    "accepted_for_supplier_onboarding_profile",
]
REQUIREMENTS = [
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-051-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_051_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-051", "invalid round id")
    ...
    require(evidence.get("supplier_onboarding_requirements") == REQUIREMENTS, "supplier onboarding requirements mismatch")
    ...
    require(checks.get("supplier_onboarding_state") == "blocked_no_real_source_record", "supplier onboarding state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-051-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-051-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 051 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing supplier onboarding case: {phrase}")

    print(
        "was_real_source_record_monitor_051=pass "
        "submitted_real_candidate_files=0 supplier_master_data_approval_gaps=0 "
        "supplier_contract_agreement_gaps=0 bank_account_verification_gaps=0 "
        "tax_registration_record_gaps=0 sanctions_screening_result_gaps=0 "
        "export_control_screening_gaps=0 supplier_onboarding_approval_gaps=0 "
        "accepted_for_supplier_onboarding_profile=0 accepted_for_next_gate=0 hold_required=1 "
