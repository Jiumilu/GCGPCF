#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 067."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-067-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-067-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-067.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-066-20260622.json"
...
WASTE_DISPOSAL_KEYS = [
    "waste_classification_record_gaps",
    "hazardous_waste_manifest_gaps",
    "licensed_disposal_vendor_permit_gaps",
    "waste_transfer_receipt_gaps",
    "treatment_or_recycling_certificate_gaps",
    "residue_disposal_confirmation_gaps",
    "regulator_disposal_report_gaps",
    "accepted_for_waste_disposal_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "waste_classification_record_gap",
    "hazardous_waste_manifest_gap",
    "licensed_disposal_vendor_permit_gap",
    "waste_transfer_receipt_gap",
    "treatment_or_recycling_certificate_gap",
    "residue_disposal_confirmation_gap",
    "regulator_disposal_report_gap",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in WASTE_DISPOSAL_KEYS:
    ...
    if value.get("waste_disposal_state") != "blocked_no_real_source_record":
        failures.append("waste_disposal_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-067-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_067_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-067", "invalid round id")
    ...
    require(evidence.get("waste_disposal_requirements") == REQUIREMENTS, "waste disposal requirements mismatch")
    ...
    require(checks.get("waste_disposal_state") == "blocked_no_real_source_record", "waste disposal state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-067-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-067-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 067 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing waste disposal case: {phrase}")

    print(
        "was_real_source_record_monitor_067=pass "
        "submitted_real_candidate_files=0 waste_classification_record_gaps=0 hazardous_waste_manifest_gaps=0 "
        "licensed_disposal_vendor_permit_gaps=0 waste_transfer_receipt_gaps=0 "
        "treatment_or_recycling_certificate_gaps=0 residue_disposal_confirmation_gaps=0 "
        "regulator_disposal_report_gaps=0 "
        "accepted_for_waste_disposal_profile=0 accepted_for_next_gate=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-068"
    )
    return 0
