#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 006."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-006-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-006-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-006.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-005-20260621.json"
...
def disallowed_intake_files() -> dict[str, list[str]]:
    hidden: list[str] = []
    temporary: list[str] = []
    llm_only: list[str] = []
    for path in sorted(INTAKE_DIR.iterdir()):
        if path.name == "was-real-source-record-candidate-template.yaml":
            continue
        lower = path.name.lower()
        if path.name.startswith("."):
            hidden.append(path.name)
        if lower.endswith((".tmp", ".temp", ".draft", ".bak", ".swp")) or lower.startswith("draft-"):
            temporary.append(path.name)
        if "llm" in lower and "candidate" in lower:
            llm_only.append(path.name)
    return {"hidden": hidden, "temporary": temporary, "llm_only": llm_only}


def monitor_failures(value: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    for key in ["submitted_real_candidate_files", "disallowed_hidden_files", "disallowed_temporary_files", "disallowed_llm_only_files", "accepted_for_next_gate"]:
        if value.get(key) != 0:
            failures.append(f"{key}_must_be_zero")
    if value.get("hold_required") != 1:
        failures.append("hold_required_must_be_one")
    if value.get("intake_state") != "waiting":
        failures.append("intake_state_must_be_waiting")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-006-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_006_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-006", "invalid round id")
    ...
    disallowed = disallowed_intake_files()
    ...
    require(checks.get("disallowed_hidden_files") == len(disallowed["hidden"]) == 0, "hidden file count mismatch")
    require(checks.get("disallowed_temporary_files") == len(disallowed["temporary"]) == 0, "temporary file count mismatch")
    require(checks.get("disallowed_llm_only_files") == len(disallowed["llm_only"]) == 0, "LLM-only file count mismatch")
    ...
    require(checks.get("candidate_directory_hygiene") == "pass", "candidate directory hygiene must pass")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-006-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-006-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 006 已建立" in loop_round, "loop round feedback missing")
    require("hidden_file" in loop_round and "temporary_file" in loop_round and "llm_only_file" in loop_round, "loop round missing bypass cases")

    print(
        "was_real_source_record_monitor_006=pass "
        "submitted_real_candidate_files=0 disallowed_hidden_files=0 disallowed_temporary_files=0 "
        "disallowed_llm_only_files=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-007"
    )
    return 0
