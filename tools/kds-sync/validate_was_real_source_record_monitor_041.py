#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 041."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-041-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-041-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-041.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-040-20260622.json"
...
CUSTOMER_AFTER_SALES_KEYS = [
    "customer_delivery_receipt_gaps",
    "pod_confirmation_gaps",
    "after_sales_exception_report_gaps",
    "complaint_handling_record_gaps",
    "return_replacement_authorization_gaps",
    "customer_satisfaction_feedback_gaps",
    "improvement_closure_gaps",
    "accepted_for_customer_after_sales_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CUSTOMER_AFTER_SALES_KEYS:
    ...
    if value.get("customer_after_sales_state") != "blocked_no_real_source_record":
        failures.append("customer_after_sales_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-041-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_041_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-041", "invalid round id")
    ...
    require(evidence.get("customer_after_sales_requirements") == [
        ...
        "improvement_closure",
    ], "customer after-sales requirements mismatch")
    ...
    require(checks.get("customer_after_sales_state") == "blocked_no_real_source_record", "customer after-sales state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-041-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-041-negative-*.json"))
    ...
    for phrase in [
        ...
        "customer_delivery_receipt_gaps | `0`",
        "pod_confirmation_gaps | `0`",
        "after_sales_exception_report_gaps | `0`",
        "complaint_handling_record_gaps | `0`",
        "return_replacement_authorization_gaps | `0`",
        "customer_satisfaction_feedback_gaps | `0`",
        "improvement_closure_gaps | `0`",
    ...
    require("真实 P4 输入 monitor 041 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing customer after-sales case: {phrase}")

    print(
        "was_real_source_record_monitor_041=pass "
        "submitted_real_candidate_files=0 customer_delivery_receipt_gaps=0 "
        "pod_confirmation_gaps=0 after_sales_exception_report_gaps=0 "
        "complaint_handling_record_gaps=0 return_replacement_authorization_gaps=0 "
        "customer_satisfaction_feedback_gaps=0 improvement_closure_gaps=0 "
        "accepted_for_customer_after_sales_profile=0 accepted_for_next_gate=0 hold_required=1 "
