#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 005."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-005-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-005-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-005.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-004-20260621.json"
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("post_precheck_promotion") != "blocked":
        failures.append("post_precheck_promotion_must_be_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-005-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_005_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-005", "invalid round id")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-005-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    fixture_gate = evidence.get("fixture_gate", {})
    require(fixture_gate.get("negative_fixtures") == 6, "evidence negative fixture count must be 6")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-005-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 005 已建立" in loop_round, "loop round feedback missing")

    print(
        "was_real_source_record_monitor_005=pass "
        "required_p4_inputs=6 submitted_real_inputs=0 submitted_files_found=0 candidate_files_checked=0 "
        "accepted_for_next_gate=0 hold_required=1 monitor_state=waiting post_precheck_promotion=blocked "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-006"
    )
    return 0
