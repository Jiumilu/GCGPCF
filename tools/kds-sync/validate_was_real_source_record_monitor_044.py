#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 044."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-044-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-044-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-044.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-043-20260622.json"
...
DATA_KEYS = [
    "master_data_version_gaps",
    "data_lineage_gaps",
    "api_integration_log_gaps",
    "access_authorization_gaps",
    "data_quality_validation_gaps",
    "audit_trail_gaps",
    "retention_deletion_policy_gaps",
    "accepted_for_data_traceability_profile",
]
REQUIREMENTS = [
    "master_data_version",
    "data_lineage",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("data_traceability_state") != "blocked_no_real_source_record":
        failures.append("data_traceability_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-044-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_044_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-044", "invalid round id")
    ...
    require(evidence.get("data_traceability_requirements") == REQUIREMENTS, "data traceability requirements mismatch")
    ...
    require(checks.get("data_traceability_state") == "blocked_no_real_source_record", "data traceability state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-044-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-044-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 044 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing data traceability case: {phrase}")

    print(
        "was_real_source_record_monitor_044=pass "
        "submitted_real_candidate_files=0 master_data_version_gaps=0 "
        "data_lineage_gaps=0 api_integration_log_gaps=0 "
        "access_authorization_gaps=0 data_quality_validation_gaps=0 "
        "audit_trail_gaps=0 retention_deletion_policy_gaps=0 "
        "accepted_for_data_traceability_profile=0 accepted_for_next_gate=0 hold_required=1 "
