#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 059."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-059-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-059-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-059.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-058-20260622.json"
...
WATER_STEWARDSHIP_KEYS = [
    ...
    "water_consumption_ledger_gaps",
    "wastewater_discharge_permit_gaps",
    "effluent_test_report_gaps",
    "treatment_facility_operation_log_gaps",
    "abnormal_discharge_incident_record_gaps",
    "water_reuse_evidence_gaps",
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-059-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_059_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-059", "invalid round id")
    ...
    require(evidence.get("water_stewardship_requirements") == REQUIREMENTS, "water stewardship requirements mismatch")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-059-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-059-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 059 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing water stewardship case: {phrase}")

    print(
        "was_real_source_record_monitor_059=pass "
        "submitted_real_candidate_files=0 water_withdrawal_permit_gaps=0 water_consumption_ledger_gaps=0 "
        "wastewater_discharge_permit_gaps=0 effluent_test_report_gaps=0 treatment_facility_operation_log_gaps=0 "
        "abnormal_discharge_incident_record_gaps=0 water_reuse_evidence_gaps=0 "
        "accepted_for_water_stewardship_profile=0 accepted_for_next_gate=0 "
