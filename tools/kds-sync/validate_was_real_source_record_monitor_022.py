#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 022."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-022-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-022-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-022.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-021-20260621.json"
...
CIRCULAR_KEYS = [
    "takeback_program_evidence_gaps",
    "reuse_remanufacture_record_gaps",
    "recycling_certificate_gaps",
    "waste_disposal_manifest_gaps",
    "hazardous_waste_handling_gaps",
    "resource_recovery_accounting_gaps",
    "accepted_for_circular_economy_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("circular_economy_state") != "blocked_no_real_source_record":
        failures.append("circular_economy_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-022-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_022_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-022", "invalid round id")
    ...
    require(evidence.get("circular_economy_requirements") == [
    ...
    ], "circular economy requirements mismatch")
    ...
    require(checks.get("circular_economy_state") == "blocked_no_real_source_record", "circular economy state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-022-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-022-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 022 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing circular economy case: {phrase}")

    print(
        "was_real_source_record_monitor_022=pass "
        "submitted_real_candidate_files=0 takeback_program_evidence_gaps=0 reuse_remanufacture_record_gaps=0 "
        "recycling_certificate_gaps=0 waste_disposal_manifest_gaps=0 hazardous_waste_handling_gaps=0 "
        "resource_recovery_accounting_gaps=0 accepted_for_circular_economy_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-023"
    )
    return 0
