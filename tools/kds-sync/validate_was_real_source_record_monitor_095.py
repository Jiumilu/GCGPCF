#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 095."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-095-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-095-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-095.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-010-20260622.json"
...
MONITOR_KEYS = [
    "verifier_accreditation_certificate_gaps",
    "verifier_competence_matrix_gaps",
    "assurance_sampling_plan_gaps",
    "site_visit_attendance_record_gaps",
    "evidence_sampling_trace_gaps",
    "assurance_exception_log_gaps",
    "assurance_final_opinion_statement_gaps",
    "accepted_for_external_assurance_execution_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "verifier_accreditation_certificate_gap",
    "verifier_competence_matrix_gap",
    "assurance_sampling_plan_gap",
    "site_visit_attendance_record_gap",
    "evidence_sampling_trace_gap",
    "assurance_exception_log_gap",
    "assurance_final_opinion_statement_gap",
]
NEGATIVE_FIXTURES = [f"real-source-record-monitor-095-negative-{case}.json" for case in LOOP_CASES]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("external_assurance_execution_state") != "blocked_no_real_source_record":
        failures.append("external_assurance_execution_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-095-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_095_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-095", "invalid round id")
    ...
    require(evidence.get("external_assurance_execution_requirements") == REQUIREMENTS, "external assurance execution requirements mismatch")
    ...
    require(checks.get("external_assurance_execution_state") == "blocked_no_real_source_record", "external assurance execution state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-095-positive.json")
    ...
    require("真实 P4 输入 monitor 095 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing external assurance execution case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_095=pass submitted_real_candidate_files=0 "
        "verifier_accreditation_certificate_gaps=0 verifier_competence_matrix_gaps=0 "
        "assurance_sampling_plan_gaps=0 site_visit_attendance_record_gaps=0 "
        "evidence_sampling_trace_gaps=0 assurance_exception_log_gaps=0 "
        "assurance_final_opinion_statement_gaps=0 accepted_for_external_assurance_execution_profile=0 "
