#!/usr/bin/env python3
"""Validate D96 scoped project Loop boundary localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-project-loop-boundary-repair-d96-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-project-loop-boundary-repair-d96-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D96-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_project_loop_boundary_repair_d96=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d96_evidence_or_loop")
    ...
    if evidence.get("changed_line_items") != 10:
        fail("changed_line_items_not_10")
    ...
    if delta.get("target_file_findings") != -10:
        fail("unexpected_target_delta")
    after = evidence.get("after", {})
    if after.get("target_file_findings") != 0:
        fail("after_target_not_zero")

    print("localization_debt_project_loop_boundary_repair_d96=pass")
