#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 062."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-062-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-062-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-062.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-061-20260622.json"
...
USE_PHASE_KEYS = [
    "use_phase_energy_performance_record_gaps",
    "customer_operation_environmental_instruction_gaps",
    "maintenance_consumable_record_gaps",
    "use_phase_emission_or_resource_model_gaps",
    "field_performance_feedback_gaps",
    "software_or_firmware_efficiency_update_record_gaps",
    "end_user_environmental_claim_substantiation_gaps",
    "accepted_for_use_phase_environmental_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("use_phase_environmental_state") != "blocked_no_real_source_record":
        failures.append("use_phase_environmental_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-062-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_062_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-062", "invalid round id")
    ...
    require(evidence.get("use_phase_environmental_requirements") == REQUIREMENTS, "use-phase environmental requirements mismatch")
    ...
    require(checks.get("use_phase_environmental_state") == "blocked_no_real_source_record", "use-phase environmental state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-062-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-062-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 062 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing use-phase environmental case: {phrase}")

    print(
        "was_real_source_record_monitor_062=pass "
        "submitted_real_candidate_files=0 use_phase_energy_performance_record_gaps=0 "
        "customer_operation_environmental_instruction_gaps=0 maintenance_consumable_record_gaps=0 "
        "use_phase_emission_or_resource_model_gaps=0 field_performance_feedback_gaps=0 "
        "software_or_firmware_efficiency_update_record_gaps=0 end_user_environmental_claim_substantiation_gaps=0 "
        "accepted_for_use_phase_environmental_profile=0 accepted_for_next_gate=0 "
