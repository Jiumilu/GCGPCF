#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 047."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-047-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-047-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-047.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-046-20260622.json"
...
CERTIFICATION_KEYS = [
    "product_certification_gaps",
    "test_lab_accreditation_gaps",
    "standard_conformity_gaps",
    "regulatory_filing_gaps",
    "ip_license_gaps",
    "documentation_release_gaps",
    "change_approval_gaps",
    "accepted_for_certification_standards_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("certification_standards_state") != "blocked_no_real_source_record":
        failures.append("certification_standards_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-047-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_047_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-047", "invalid round id")
    ...
    require(evidence.get("certification_standards_requirements") == REQUIREMENTS, "certification standards requirements mismatch")
    ...
    require(checks.get("certification_standards_state") == "blocked_no_real_source_record", "certification standards state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-047-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-047-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 047 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing certification standards case: {phrase}")

    print(
        "was_real_source_record_monitor_047=pass "
        "submitted_real_candidate_files=0 product_certification_gaps=0 "
        "test_lab_accreditation_gaps=0 standard_conformity_gaps=0 "
        "regulatory_filing_gaps=0 ip_license_gaps=0 "
        "documentation_release_gaps=0 change_approval_gaps=0 "
        "accepted_for_certification_standards_profile=0 accepted_for_next_gate=0 hold_required=1 "
