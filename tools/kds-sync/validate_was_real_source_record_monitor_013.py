#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 013."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-013-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-013-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-013.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-012-20260621.json"
...
GREEN_CHAIN_KEYS = [
    "carbon_evidence_gaps",
    "material_origin_gaps",
    "compliance_certificate_gaps",
    "green_finance_evidence_gaps",
    "supplier_route_delivery_trace_gaps",
    "cross_chain_consistency_gaps",
    "accepted_for_green_supply_chain_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("green_supply_chain_state") != "blocked_no_real_source_record":
        failures.append("green_supply_chain_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-013-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_013_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-013", "invalid round id")
    ...
    require(evidence.get("green_supply_chain_requirements") == [
        "carbon_evidence",
        "material_origin",
    ...
    ], "green supply-chain requirements mismatch")
    ...
    require(checks.get("green_supply_chain_state") == "blocked_no_real_source_record", "green supply-chain state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-013-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-013-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 013 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "carbon_evidence_gap",
        "material_origin_gap",
        ...
        require(phrase in loop_round, f"loop round missing green-chain case: {phrase}")

    print(
        "was_real_source_record_monitor_013=pass "
        "submitted_real_candidate_files=0 carbon_evidence_gaps=0 material_origin_gaps=0 "
        "compliance_certificate_gaps=0 green_finance_evidence_gaps=0 "
        "supplier_route_delivery_trace_gaps=0 cross_chain_consistency_gaps=0 "
        "accepted_for_green_supply_chain_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-014"
    )
    return 0
