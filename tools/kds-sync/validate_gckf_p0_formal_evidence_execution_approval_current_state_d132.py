#!/usr/bin/env python3
"""Validate D132 GCKF P0 formal evidence execution approval current-state evidence."""
...
FIXTURE = ROOT / "fixtures" / "api" / "gckf-p0-formal-evidence-execution-approval-current-state-d132-20260622.json"
EVIDENCE_JSON = ROOT / "docs/harness/evidence/gckf-p0-formal-evidence-execution-approval-current-state-d132-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/gckf-p0-formal-evidence-execution-approval-current-state-d132-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D132-001.md"


def fail(message: str) -> None:
    print(f"gckf_p0_formal_evidence_execution_approval_current_state_d132=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    ...
    require(EVIDENCE_MD.exists() and LOOP_MD.exists(), "missing_d132_markdown_or_loop")

    expected = fixture["expectedSummary"]
    historical_approval = load_json(ROOT / fixture["sourceHistoricalExecutionApproval"])
    current_request = load_json(ROOT / fixture["sourceCurrentExecutionRequest"])
    approval = fixture["approvalCandidate"]
    source_approval = historical_approval["approvalCandidate"]
    request = current_request["executionRequest"]

    require(fixture.get("executionApprovalStatus") == expected["executionApprovalStatus"], "execution_approval_status_mismatch")
    ...
    require(approval.get("approvalType") == expected["approvalType"], "approval_type_mismatch")
    require(approval.get("approvalStatus") == expected["approvalStatus"], "approval_status_mismatch")
    require(approval.get("approvalOutcome") == expected["approvalOutcome"], "approval_outcome_mismatch")
    ...
    require(approval.get("sourceExecutionRequestId") == request.get("id"), "source_execution_request_id_mismatch")

    require(historical_approval.get("executionApprovalStatus") == "candidate_approval", "historical_execution_approval_status_mismatch")
    require(source_approval.get("approvalStatus") == "candidate_approval", "historical_approval_status_mismatch")
    require(source_approval.get("executionStatus") == "not_executed", "historical_execution_status_mismatch")
    require(current_request.get("executionRequestStatus") == expected["sourceExecutionRequestStatus"], "source_execution_request_status_mismatch")
    require(request.get("executionStatus") == expected["sourceExecutionRequestExecutionStatus"], "source_execution_request_execution_status_mismatch")
    require(fixture.get("coveredRequestType") == expected["coveredRequestType"], "covered_request_type_mismatch")
    require(fixture["coveredRequestType"] == request.get("requestType"), "covered_request_not_matched")
    ...
    for check in {
        "source_execution_request_status_is_candidate_with_hold",
        ...
        require(check in checks, f"missing_approval_check:{check}")
    ...
    require(evidence.get("current_execution_approval_status") == "candidate_approval_with_hold", "evidence_execution_approval_status_mismatch")
    require(evidence.get("maximum_state") == "review_ready_with_hold", "evidence_maximum_state_mismatch")
    require(evidence.get("approval_scope", {}).get("required_inputs") == 15, "evidence_required_input_count_mismatch")
    require(evidence.get("hold_context", {}).get("source_execution_request_status") == "candidate_with_hold", "evidence_source_execution_request_status_mismatch")

    d33_output = run_command("python3", "scripts/api/validate_gckf_p0_formal_evidence_execution_approval_dry_run.py")
    require(d33_output.startswith("gckf_p0_formal_evidence_execution_approval_dry_run=pass"), "d33_validator_not_pass")

    d131_output = run_command("python3", "tools/kds-sync/validate_gckf_p0_formal_evidence_execution_request_current_state_d131.py")
    require(d131_output.startswith("gckf_p0_formal_evidence_execution_request_current_state_d131=pass"), "d131_validator_not_pass")
    ...
    print("gckf_p0_formal_evidence_execution_approval_current_state_d132=pass")
    print(f"execution_approval_status={fixture.get('executionApprovalStatus')}")
    print(f"maximum_state={fixture.get('maximumState')}")
    print(f"approval_status={approval.get('approvalStatus')}")
