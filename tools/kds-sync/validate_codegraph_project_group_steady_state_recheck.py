#!/usr/bin/env python3
"""Validate project-group CodeGraph steady-state recheck evidence."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/codegraph-project-group-steady-state-recheck-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/codegraph-project-group-steady-state-recheck-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-CODEGRAPH-PROJECT-GROUP-STEADY-STATE-RECHECK-011.md"
...
def main() -> int:
    ...
    require(evidence["evidence_id"] == "CODEGRAPH-PROJECT-GROUP-STEADY-STATE-RECHECK-20260621", "invalid evidence id")
    require(evidence["status"] == "steady_state_recheck_review_required_due_brain_studio_drift", "invalid status")
    require(evidence["scope"] == "GPCF-CODEGRAPH-PROJECT-GROUP-STEADY-STATE-RECHECK-011", "invalid scope")
    require(evidence["next_round"] == "GPCF-CODEGRAPH-WATCHLIST-SYNC-PLAN-001", "invalid next round")
    ...
    require(summary["brain_current_pending"] >= 1, "Brain current drift must be recorded")
    require(summary["studio_current_pending"] >= 1, "Studio current drift must be recorded")
    ...
    for repo_id, name, path in REPOS:
        ...
        if repo_id in {"brain", "studio"}:
            require(total > 0, f"{name} active drift must remain visible")
        elif repo_id == "gfis":
    ...
    for phrase in [
        "steady_state_recheck_review_required_due_brain_studio_drift",
        "Brain `modified=6`",
        "Studio `modified=5`",
    ...
    print(
        "codegraph_project_group_steady_state_recheck=pass_with_watch "
        f"repo_count=14 brain_pending={live_pending['brain']} "
        f"studio_pending={live_pending['studio']} gfis_pending={live_pending['gfis']} "
        "codegraph_git_entries=0 next=GPCF-CODEGRAPH-WATCHLIST-SYNC-PLAN-001"
    )
    return 0
