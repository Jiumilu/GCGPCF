#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 030."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-030-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-030-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-030.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-029-20260622.json"
...
INVENTORY_STORAGE_KEYS = [
    "warehouse_receiving_record_gaps",
    "inventory_batch_ledger_gaps",
    "storage_condition_record_gaps",
    "packaging_material_declaration_gaps",
    "quarantine_release_record_gaps",
    "stock_movement_traceability_gaps",
    "accepted_for_inventory_storage_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in INVENTORY_STORAGE_KEYS:
    ...
    if value.get("inventory_storage_state") != "blocked_no_real_source_record":
        failures.append("inventory_storage_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-030-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_030_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-030", "invalid round id")
    ...
    require(evidence.get("inventory_storage_requirements") == [
    ...
    ], "inventory storage requirements mismatch")
    ...
    require(checks.get("inventory_storage_state") == "blocked_no_real_source_record", "inventory storage state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-030-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-030-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 030 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing inventory storage case: {phrase}")

    print(
        "was_real_source_record_monitor_030=pass "
        "submitted_real_candidate_files=0 warehouse_receiving_record_gaps=0 "
        "inventory_batch_ledger_gaps=0 storage_condition_record_gaps=0 "
        "packaging_material_declaration_gaps=0 quarantine_release_record_gaps=0 "
        "stock_movement_traceability_gaps=0 accepted_for_inventory_storage_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-031"
    )
    return 0
