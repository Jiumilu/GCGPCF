#!/usr/bin/env python3
"""Validate D103 scoped KDS-DKS Loop title localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-kds-dks-loop-title-repair-d103-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-kds-dks-loop-title-repair-d103-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D103-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_kds_dks_loop_title_repair_d103=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    ...
    if len(changed_files) != 32:
        fail("changed_file_count_not_32")
    if evidence.get("changed_line_items") != 128:
        fail("changed_line_items_not_128")
    ...
    if delta.get("all_repo_findings") != -47:
    ...
    print("localization_debt_kds_dks_loop_title_repair_d103=pass")
