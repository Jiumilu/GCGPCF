#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 071."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-071-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-071-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-071.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-070-20260622.json"
...
SOCIAL_RESPONSIBILITY_KEYS = [
    "labor_standard_audit_report_gaps",
    "working_hours_wage_record_gaps",
    "occupational_health_safety_record_gaps",
    "worker_grievance_mechanism_record_gaps",
    "human_rights_due_diligence_record_gaps",
    "forced_labor_screening_record_gaps",
    "community_impact_consultation_record_gaps",
    "accepted_for_social_responsibility_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SOCIAL_RESPONSIBILITY_KEYS:
    ...
    if value.get("social_responsibility_state") != "blocked_no_real_source_record":
        failures.append("social_responsibility_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-071-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_071_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-071", "invalid round id")
    ...
    require(evidence.get("social_responsibility_requirements") == REQUIREMENTS, "social responsibility requirements mismatch")
    ...
    require(checks.get("social_responsibility_state") == "blocked_no_real_source_record", "social responsibility state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-071-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-071-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 071 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing social responsibility case: {phrase}")

    print(
        "was_real_source_record_monitor_071=pass "
        "submitted_real_candidate_files=0 labor_standard_audit_report_gaps=0 working_hours_wage_record_gaps=0 "
        "occupational_health_safety_record_gaps=0 worker_grievance_mechanism_record_gaps=0 "
        "human_rights_due_diligence_record_gaps=0 forced_labor_screening_record_gaps=0 "
        "community_impact_consultation_record_gaps=0 "
        "accepted_for_social_responsibility_profile=0 accepted_for_next_gate=0 "
