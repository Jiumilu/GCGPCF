#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 029."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-029-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-029-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-029.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-028-20260622.json"
...
LOGISTICS_COMPLIANCE_KEYS = [
    "transport_mode_record_gaps",
    "bill_of_lading_or_waybill_gaps",
    "customs_declaration_gaps",
    "chain_of_custody_handoff_gaps",
    "proof_of_delivery_gaps",
    "logistics_emission_record_gaps",
    "accepted_for_logistics_compliance_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in LOGISTICS_COMPLIANCE_KEYS:
    ...
    if value.get("logistics_compliance_state") != "blocked_no_real_source_record":
        failures.append("logistics_compliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-029-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_029_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-029", "invalid round id")
    ...
    require(evidence.get("logistics_compliance_requirements") == [
    ...
    ], "logistics compliance requirements mismatch")
    ...
    require(checks.get("logistics_compliance_state") == "blocked_no_real_source_record", "logistics compliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-029-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-029-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 029 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing logistics compliance case: {phrase}")

    print(
        "was_real_source_record_monitor_029=pass "
        "submitted_real_candidate_files=0 transport_mode_record_gaps=0 "
        "bill_of_lading_or_waybill_gaps=0 customs_declaration_gaps=0 "
        "chain_of_custody_handoff_gaps=0 proof_of_delivery_gaps=0 "
        "logistics_emission_record_gaps=0 accepted_for_logistics_compliance_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-030"
    )
    return 0
