#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 015."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-015-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-015-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-015.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-014-20260621.json"
...
POLICY_KEYS = [
    "applicable_policy_gaps",
    "jurisdiction_compliance_gaps",
    "sanctions_screening_gaps",
    "restricted_goods_list_gaps",
    "license_customs_trade_control_gaps",
    "end_use_statement_gaps",
    "accepted_for_policy_regulatory_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("policy_regulatory_state") != "blocked_no_real_source_record":
        failures.append("policy_regulatory_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-015-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_015_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-015", "invalid round id")
    ...
    require(evidence.get("policy_regulatory_requirements") == [
        "applicable_policy",
        "jurisdiction_compliance",
        "sanctions_screening",
        "restricted_goods_list",
        "license_customs_trade_control",
        "end_use_statement",
    ], "policy regulatory requirements mismatch")
    ...
    require(checks.get("policy_regulatory_state") == "blocked_no_real_source_record", "policy regulatory state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-015-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-015-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 015 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "applicable_policy_gap",
        "jurisdiction_compliance_gap",
        "sanctions_screening_gap",
        "restricted_goods_list_gap",
        "license_customs_trade_control_gap",
        "end_use_statement_gap",
    ]:
        require(phrase in loop_round, f"loop round missing policy/regulatory case: {phrase}")

    print(
        "was_real_source_record_monitor_015=pass "
        "submitted_real_candidate_files=0 applicable_policy_gaps=0 jurisdiction_compliance_gaps=0 "
        "sanctions_screening_gaps=0 restricted_goods_list_gaps=0 "
        "license_customs_trade_control_gaps=0 end_use_statement_gaps=0 "
        "accepted_for_policy_regulatory_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-016"
    )
    return 0
