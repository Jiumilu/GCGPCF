#!/usr/bin/env python3
"""Validate D92 scoped GCKF heading localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d92-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d92-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D92-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_gckf_heading_repair_d92=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d92_evidence_or_loop")
    ...
    if len(changed_files) != 15:
        fail("changed_file_count_not_15")
    ...
    if delta.get("docs_gc_knowledge_fabric_h1_findings") != -15:
        fail("unexpected_h1_delta")
    if delta.get("docs_gc_knowledge_fabric_findings") != -14:
        fail("unexpected_gckf_delta")

    print("localization_debt_gckf_heading_repair_d92=pass")
