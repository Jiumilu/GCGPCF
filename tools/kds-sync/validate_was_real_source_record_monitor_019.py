#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 019."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-019-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-019-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-019.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-018-20260621.json"
...
RISK_RESILIENCE_KEYS = [
    "risk_transfer_document_gaps",
    "insurance_policy_evidence_gaps",
    "incident_report_gaps",
    "recall_record_gaps",
    "emergency_response_plan_gaps",
    "business_continuity_proof_gaps",
    "accepted_for_risk_resilience_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in RISK_RESILIENCE_KEYS:
    ...
    if value.get("risk_resilience_state") != "blocked_no_real_source_record":
        failures.append("risk_resilience_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-019-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_019_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-019", "invalid round id")
    ...
    require(evidence.get("risk_resilience_requirements") == [
        "risk_transfer_document",
        "insurance_policy_evidence",
        "incident_report",
        "recall_record",
        "emergency_response_plan",
        "business_continuity_proof",
    ], "risk resilience requirements mismatch")
    ...
    require(checks.get("risk_resilience_state") == "blocked_no_real_source_record", "risk resilience state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-019-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-019-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 019 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "risk_transfer_document_gap",
        "insurance_policy_evidence_gap",
        "incident_report_gap",
        "recall_record_gap",
        "emergency_response_plan_gap",
        "business_continuity_proof_gap",
    ]:
        require(phrase in loop_round, f"loop round missing risk resilience case: {phrase}")

    print(
        "was_real_source_record_monitor_019=pass "
        "submitted_real_candidate_files=0 risk_transfer_document_gaps=0 insurance_policy_evidence_gaps=0 "
        "incident_report_gaps=0 recall_record_gaps=0 emergency_response_plan_gaps=0 "
        "business_continuity_proof_gaps=0 accepted_for_risk_resilience_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-020"
    )
    return 0
