#!/usr/bin/env python3
"""Validate D101 scoped Loop governance residual localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-loop-governance-residual-repair-d101-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-loop-governance-residual-repair-d101-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D101-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_loop_governance_residual_repair_d101=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d101_evidence_or_loop")
    ...
    if len(changed_files) != 9:
        fail("changed_file_count_not_9")
    if evidence.get("changed_line_items") != 210:
        fail("changed_line_items_not_210")
    ...
    if delta.get("target_file_findings") != -23:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -13:
    ...
    print("localization_debt_loop_governance_residual_repair_d101=pass")
