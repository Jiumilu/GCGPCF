#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 038."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-038-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-038-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-038.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-037-20260622.json"
...
RESPONSIBLE_SOURCING_KEYS = ['conflict_minerals_declaration_gaps', 'forced_labor_due_diligence_gaps', 'deforestation_free_declaration_gaps', 'country_of_origin_certificate_gaps', 'chain_of_custody_due_diligence_gaps', 'grievance_remediation_record_gaps', 'accepted_for_responsible_sourcing_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in RESPONSIBLE_SOURCING_KEYS:
    ...
    if value.get("responsible_sourcing_state") != "blocked_no_real_source_record":
        failures.append("responsible_sourcing_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-038-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_038_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-038", "invalid round id")
    ...
    require(evidence.get("responsible_sourcing_requirements") == ['conflict_minerals_declaration', 'forced_labor_due_diligence', 'deforestation_free_declaration', 'country_of_origin_certificate', 'chain_of_custody_due_diligence', 'grievance_remediation_record'], "responsible sourcing requirements mismatch")
    ...
    require(checks.get("responsible_sourcing_state") == "blocked_no_real_source_record", "responsible sourcing state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-038-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-038-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 038 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['conflict_minerals_declaration_gap', 'forced_labor_due_diligence_gap', 'deforestation_free_declaration_gap', 'country_of_origin_certificate_gap', 'chain_of_custody_due_diligence_gap', 'grievance_remediation_record_gap']:
        require(phrase in loop_round, f"loop round missing responsible sourcing case: {phrase}")

    print(
        "was_real_source_record_monitor_038=pass "
        "submitted_real_candidate_files=0 conflict_minerals_declaration_gaps=0 "
        "forced_labor_due_diligence_gaps=0 deforestation_free_declaration_gaps=0 "
        "country_of_origin_certificate_gaps=0 chain_of_custody_due_diligence_gaps=0 "
        "grievance_remediation_record_gaps=0 accepted_for_responsible_sourcing_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-039"
    )
    return 0
