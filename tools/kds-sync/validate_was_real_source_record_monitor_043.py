#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 043."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-043-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-043-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-043.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-042-20260622.json"
...
FINANCE_KEYS = [
    "invoice_issuance_gaps",
    "payment_confirmation_gaps",
    "tax_compliance_receipt_gaps",
    "trade_credit_insurance_gaps",
    "financing_approval_gaps",
    "reconciliation_statement_gaps",
    "financial_dispute_resolution_gaps",
    "accepted_for_finance_settlement_profile",
]
REQUIREMENTS = [
    "invoice_issuance",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("finance_settlement_state") != "blocked_no_real_source_record":
        failures.append("finance_settlement_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-043-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_043_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-043", "invalid round id")
    ...
    require(evidence.get("finance_settlement_requirements") == REQUIREMENTS, "finance settlement requirements mismatch")
    ...
    require(checks.get("finance_settlement_state") == "blocked_no_real_source_record", "finance settlement state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-043-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-043-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 043 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing finance settlement case: {phrase}")

    print(
        "was_real_source_record_monitor_043=pass "
        "submitted_real_candidate_files=0 invoice_issuance_gaps=0 "
        "payment_confirmation_gaps=0 tax_compliance_receipt_gaps=0 "
        "trade_credit_insurance_gaps=0 financing_approval_gaps=0 "
        "reconciliation_statement_gaps=0 financial_dispute_resolution_gaps=0 "
        "accepted_for_finance_settlement_profile=0 accepted_for_next_gate=0 hold_required=1 "
