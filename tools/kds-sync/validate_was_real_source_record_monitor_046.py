#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 046."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-046-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-046-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-046.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-045-20260622.json"
...
WORKFORCE_KEYS = [
    "role_authorization_gaps",
    "training_qualification_gaps",
    "safety_record_gaps",
    "contractor_personnel_gaps",
    "responsibility_matrix_gaps",
    "handover_record_gaps",
    "labor_compliance_gaps",
    "accepted_for_workforce_authority_profile",
]
REQUIREMENTS = [
    "role_authorization",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("workforce_authority_state") != "blocked_no_real_source_record":
        failures.append("workforce_authority_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-046-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_046_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-046", "invalid round id")
    ...
    require(evidence.get("workforce_authority_requirements") == REQUIREMENTS, "workforce authority requirements mismatch")
    ...
    require(checks.get("workforce_authority_state") == "blocked_no_real_source_record", "workforce authority state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-046-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-046-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 046 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing workforce authority case: {phrase}")

    print(
        "was_real_source_record_monitor_046=pass "
        "submitted_real_candidate_files=0 role_authorization_gaps=0 "
        "training_qualification_gaps=0 safety_record_gaps=0 "
        "contractor_personnel_gaps=0 responsibility_matrix_gaps=0 "
        "handover_record_gaps=0 labor_compliance_gaps=0 "
        "accepted_for_workforce_authority_profile=0 accepted_for_next_gate=0 hold_required=1 "
