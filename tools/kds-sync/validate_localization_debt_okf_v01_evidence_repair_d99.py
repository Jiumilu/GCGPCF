#!/usr/bin/env python3
"""Validate D99 scoped OKF v0.1 evidence localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-okf-v01-evidence-repair-d99-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-okf-v01-evidence-repair-d99-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D99-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_okf_v01_evidence_repair_d99=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d99_evidence_or_loop")
    ...
    if len(changed_files) != 11:
        fail("changed_file_count_not_11")
    if evidence.get("changed_line_items") != 248:
        fail("changed_line_items_not_248")
    ...
    if delta.get("target_file_findings") != -25:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -25:
    ...
    print("localization_debt_okf_v01_evidence_repair_d99=pass")
