#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 096."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-096-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-096-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-096.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-012-20260622.json"
...
MONITOR_KEYS = ['assurance_report_distribution_acknowledgement_gaps', 'management_response_approval_record_gaps', 'corrective_action_owner_assignment_gaps', 'remediation_evidence_package_gaps', 'verifier_follow_up_review_record_gaps', 'waes_reliance_decision_memo_gaps', 'kds_assurance_publication_receipt_gaps', 'accepted_for_assurance_reliance_profile']
REQUIREMENTS = ['assurance_report_distribution_acknowledgement', 'management_response_approval_record', 'corrective_action_owner_assignment', 'remediation_evidence_package', 'verifier_follow_up_review_record', 'waes_reliance_decision_memo', 'kds_assurance_publication_receipt']
REQUIRED_COMMANDS = [
...
LOOP_CASES = ['assurance_report_distribution_acknowledgement_gap', 'management_response_approval_record_gap', 'corrective_action_owner_assignment_gap', 'remediation_evidence_package_gap', 'verifier_follow_up_review_record_gap', 'waes_reliance_decision_memo_gap', 'kds_assurance_publication_receipt_gap']
NEGATIVE_FIXTURES = [f"real-source-record-monitor-096-negative-{case}.json" for case in LOOP_CASES]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("assurance_reliance_state") != "blocked_no_real_source_record":
        failures.append("assurance_reliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-096-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_096_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-096", "invalid round id")
    ...
    require(evidence.get("assurance_reliance_requirements") == REQUIREMENTS, "assurance reliance requirements mismatch")
    ...
    require(checks.get("assurance_reliance_state") == "blocked_no_real_source_record", "assurance reliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-096-positive.json")
    ...
    require("真实 P4 输入 monitor 096 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing assurance reliance case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_096=pass submitted_real_candidate_files=0 "
        "assurance_report_distribution_acknowledgement_gaps=0 "
        "management_response_approval_record_gaps=0 "
        "corrective_action_owner_assignment_gaps=0 "
        "remediation_evidence_package_gaps=0 "
        ...
        "waes_reliance_decision_memo_gaps=0 "
        "kds_assurance_publication_receipt_gaps=0 "
        "accepted_for_assurance_reliance_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-097"
    )
    return 0
