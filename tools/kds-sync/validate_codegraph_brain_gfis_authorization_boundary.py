#!/usr/bin/env python3
"""Validate CodeGraph Brain/GFIS authorization boundary evidence."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/codegraph-brain-gfis-authorization-boundary-20260623.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/codegraph-brain-gfis-authorization-boundary-20260623.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-CODEGRAPH-BRAIN-GFIS-AUTHORIZATION-BOUNDARY-018.md"
REPOS = {
...
def main() -> int:
    ...
    require(evidence["evidence_id"] == "CODEGRAPH-BRAIN-GFIS-AUTHORIZATION-BOUNDARY-20260623", "invalid evidence id")
    require(evidence["scope"] == "GPCF-CODEGRAPH-BRAIN-GFIS-AUTHORIZATION-BOUNDARY-018", "invalid scope")
    require(evidence["status"] == "gfis_authorization_boundary_with_kds_studio_watch", "invalid status")
    require(evidence["next_round"] == "GPCF-CODEGRAPH-GFIS-SCOPE-REVIEW-019", "invalid next round")

    for name, repo in REPOS.items():
        expected = evidence["watchlist"][name]
        ...
        require(status["pendingChanges"] == expected["codegraph_pending"], f"pending mismatch: {name}")
        ...
        require(counts == expected["git_dirty"], f"git dirty mismatch: {name}: {counts}")
        ...
        require(git_status.stdout.strip() == "", f".codegraph must remain isolated: {name}")

    questions = evidence["authorization_questions"]
    require(questions[0]["id"] == "authorize_gfis_read_only_scope_review", "first question must recommend GFIS scope review")
    require(questions[0]["recommended"] is True, "GFIS read-only scope review must be recommended")
    for item in questions:
        require(item["default_if_no_answer"] == "not_authorized", f"default must be not_authorized: {item['id']}")
    require("clean_reindex" in evidence["forbidden"], "clean reindex must be forbidden")
    require("business_development" in evidence["forbidden"], "business development must be forbidden")

    governance = evidence["governance"]
    require(governance["mode"] == "gfis_authorization_boundary_only", "invalid governance mode")
    ...
    for phrase in [
        "gfis_authorization_boundary_with_kds_studio_watch",
        "Brain 仍有 1 项 Git dirty",
        "KDS | added=0, modified=0, removed=0 | 59",
        "Studio | added=0, modified=18, removed=0 | 20",
        "GFIS | added=0, modified=0, removed=0 | 1",
        "是否授权 GFIS 执行只读 CodeGraph/Git scope review",
        "GFIS reindexRecommended=false，且用户此前明确暂不授权 clean reindex",
        "GPCF-CODEGRAPH-GFIS-SCOPE-REVIEW-019"
    ]:
        require(phrase in evidence_md, f"evidence markdown missing phrase: {phrase}")

    print(
        "codegraph_brain_gfis_authorization_boundary=pass "
        "brain_clean=false kds_codegraph_clean=true kds_git_mirror_watch=true studio_post_clean_drift=true "
        "gfis_pending=0 gfis_dirty_total=1 "
        "gfis_scope_review_recommended=true "
        "gfis_clean_reindex_authorized=false "
        "business_development=false commit=false push=false deploy=false "
        "accepted=false integrated=false production_ready=false "
        "next=GPCF-CODEGRAPH-GFIS-SCOPE-REVIEW-019"
    )
    return 0
