#!/usr/bin/env python3
"""Validate D104 scoped KDS-DKS routing text localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-kds-dks-routing-text-repair-d104-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-kds-dks-routing-text-repair-d104-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D104-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_kds_dks_routing_text_repair_d104=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d104_evidence_or_loop")
    ...
    if len(changed_files) != 22:
        fail("changed_file_count_not_22")
    if evidence.get("changed_line_items") != 27:
        fail("changed_line_items_not_27")
    ...
    if delta.get("target_file_findings") != -27:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -36:
    ...
    print("localization_debt_kds_dks_routing_text_repair_d104=pass")
