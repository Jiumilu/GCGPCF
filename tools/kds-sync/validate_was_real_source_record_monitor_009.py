#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 009."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-009-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-009-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-009.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-008-20260621.json"
...
BATCH_KEYS = ["parallel_candidate_files", "same_order_multiple_versions", "cross_customer_mixed_candidates", "conflicting_candidate_fields"]
ATTACHMENT_KEYS = ["missing_attachment_evidence_refs", "source_record_hash_mismatch", "source_refs_evidence_refs_mismatch", "kds_backlink_source_refs_mismatch"]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in BATCH_KEYS + ATTACHMENT_KEYS + ["accepted_for_next_gate"]:
    ...
    if value.get("candidate_batch_state") != "not_applicable_no_submission":
        failures.append("candidate_batch_state_must_be_not_applicable")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-009-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_009_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-009", "invalid round id")
    ...
    require(checks.get("candidate_batch_state") == "not_applicable_no_submission", "candidate batch state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-009-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-009-negative-*.json"))
    require(len(negatives) == 8, "negative fixture count must be 8")
    ...
    require("真实 P4 输入 monitor 009 已建立" in loop_round, "loop round feedback missing")
    for phrase in BATCH_KEYS:
        require(phrase in loop_round, f"loop round missing batch case: {phrase}")
    for phrase in ATTACHMENT_KEYS:
        require(phrase in loop_round, f"loop round missing attachment case: {phrase}")

    print(
        "was_real_source_record_monitor_009=pass "
        "submitted_real_candidate_files=0 parallel_candidate_files=0 same_order_multiple_versions=0 "
        "cross_customer_mixed_candidates=0 conflicting_candidate_fields=0 "
        "missing_attachment_evidence_refs=0 source_record_hash_mismatch=0 "
        "source_refs_evidence_refs_mismatch=0 kds_backlink_source_refs_mismatch=0 "
        "accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-010"
    )
    return 0
