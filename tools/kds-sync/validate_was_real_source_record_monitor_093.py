#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 093."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-093-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-093-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-093.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-092-20260622.json"
...
MONITOR_KEYS = [
    ...
    "segregation_of_duties_review_gaps",
    "approval_chain_record_gaps",
    "override_exception_log_gaps",
    "authority_revocation_record_gaps",
    "emergency_access_approval_gaps",
    "accountability_attestation_gaps",
    "accepted_for_authority_control_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "authority_delegation_matrix_gap",
    "segregation_of_duties_review_gap",
    "approval_chain_record_gap",
    "override_exception_log_gap",
    "authority_revocation_record_gap",
    "emergency_access_approval_gap",
    "accountability_attestation_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-093-negative-authority_delegation_matrix_gap.json",
    "real-source-record-monitor-093-negative-segregation_of_duties_review_gap.json",
    "real-source-record-monitor-093-negative-approval_chain_record_gap.json",
    "real-source-record-monitor-093-negative-override_exception_log_gap.json",
    "real-source-record-monitor-093-negative-authority_revocation_record_gap.json",
    "real-source-record-monitor-093-negative-emergency_access_approval_gap.json",
    "real-source-record-monitor-093-negative-accountability_attestation_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("authority_control_state") != "blocked_no_real_source_record":
        failures.append("authority_control_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-093-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_093_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-093", "invalid round id")
    ...
    require(evidence.get("authority_control_requirements") == REQUIREMENTS, "authority control requirements mismatch")
    ...
    require(checks.get("authority_control_state") == "blocked_no_real_source_record", "authority control state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-093-positive.json")
    ...
    require("真实 P4 输入 monitor 093 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing authority control case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_093=pass submitted_real_candidate_files=0 "
        "authority_delegation_matrix_gaps=0 segregation_of_duties_review_gaps=0 "
        "approval_chain_record_gaps=0 override_exception_log_gaps=0 "
        "authority_revocation_record_gaps=0 emergency_access_approval_gaps=0 "
        "accountability_attestation_gaps=0 accepted_for_authority_control_profile=0 "
