#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 100."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-100-20260623.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-100-20260623.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-100.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-099-20260622.json"
...
MONITOR_KEYS = [
    "digital_product_passport_dataset_gaps",
    "product_data_carrier_or_qr_label_gaps",
    "public_disclosure_page_snapshot_gaps",
    "traceability_dataset_version_gaps",
    "data_access_governance_record_gaps",
    "passport_update_change_log_gaps",
    "regulator_or_customer_retrieval_receipt_gaps",
    "accepted_for_digital_product_passport_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "digital_product_passport_dataset_gap",
    "product_data_carrier_or_qr_label_gap",
    "public_disclosure_page_snapshot_gap",
    "traceability_dataset_version_gap",
    "data_access_governance_record_gap",
    "passport_update_change_log_gap",
    "regulator_or_customer_retrieval_receipt_gap",
]
NEGATIVE_FIXTURES = [f"real-source-record-monitor-100-negative-{case}.json" for case in LOOP_CASES]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("digital_product_passport_state") != "blocked_no_real_source_record":
        failures.append("digital_product_passport_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-100-20260623", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_100_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-100", "invalid round id")
    ...
    require(evidence.get("digital_product_passport_requirements") == REQUIREMENTS, "digital product passport requirements mismatch")
    ...
    require(checks.get("digital_product_passport_state") == "blocked_no_real_source_record", "digital product passport state mismatch")
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-100-positive.json")
    ...
    require("真实 P4 输入 monitor 100 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing digital product passport case: {phrase}")
    ...
    print(
        ...
        "passport_update_change_log_gaps=0 "
        "regulator_or_customer_retrieval_receipt_gaps=0 "
        "accepted_for_digital_product_passport_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-101"
    )
    return 0
