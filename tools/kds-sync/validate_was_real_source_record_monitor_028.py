#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 028."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-028-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-028-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-028.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-002-20260622.json"
...
SUPPLIER_ESG_KEYS = [
    "supplier_esg_audit_report_gaps",
    "supplier_code_of_conduct_acknowledgement_gaps",
    "labor_standard_attestation_gaps",
    "anti_bribery_conflict_declaration_gaps",
    "corrective_action_plan_closure_gaps",
    "supplier_tier_traceability_gaps",
    "accepted_for_supplier_esg_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("supplier_esg_state") != "blocked_no_real_source_record":
        failures.append("supplier_esg_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-028-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_028_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-028", "invalid round id")
    ...
    require(evidence.get("supplier_esg_requirements") == [
    ...
    ], "supplier ESG requirements mismatch")
    ...
    require(checks.get("supplier_esg_state") == "blocked_no_real_source_record", "supplier ESG state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-028-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-028-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 028 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing supplier ESG case: {phrase}")

    print(
        "was_real_source_record_monitor_028=pass "
        "submitted_real_candidate_files=0 supplier_esg_audit_report_gaps=0 "
        "supplier_code_of_conduct_acknowledgement_gaps=0 labor_standard_attestation_gaps=0 "
        "anti_bribery_conflict_declaration_gaps=0 corrective_action_plan_closure_gaps=0 "
        "supplier_tier_traceability_gaps=0 accepted_for_supplier_esg_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-029"
    )
    return 0
