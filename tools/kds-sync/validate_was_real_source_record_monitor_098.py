#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 098."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-098-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-098-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-098.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-014-20260622.json"
...
MONITOR_KEYS = [
    "take_back_program_approval_gaps",
    "reverse_logistics_receipt_gaps",
    "recycling_certificate_gaps",
    "waste_transfer_manifest_gaps",
    "authorized_disposal_permit_gaps",
    "reuse_or_recovery_rate_calculation_gaps",
    "kds_circularity_publication_receipt_gaps",
    "accepted_for_circularity_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "take_back_program_approval_gap",
    "reverse_logistics_receipt_gap",
    "recycling_certificate_gap",
    "waste_transfer_manifest_gap",
    "authorized_disposal_permit_gap",
    "reuse_or_recovery_rate_calculation_gap",
    "kds_circularity_publication_receipt_gap",
]
NEGATIVE_FIXTURES = [f"real-source-record-monitor-098-negative-{case}.json" for case in LOOP_CASES]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("circularity_state") != "blocked_no_real_source_record":
        failures.append("circularity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-098-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_098_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-098", "invalid round id")
    ...
    require(evidence.get("circularity_requirements") == REQUIREMENTS, "circularity requirements mismatch")
    ...
    require(checks.get("circularity_state") == "blocked_no_real_source_record", "circularity state mismatch")
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-098-positive.json")
    ...
    require("真实 P4 输入 monitor 098 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing circularity case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_098=pass submitted_real_candidate_files=0 "
        "take_back_program_approval_gaps=0 "
        "reverse_logistics_receipt_gaps=0 "
        "recycling_certificate_gaps=0 "
        "waste_transfer_manifest_gaps=0 "
        ...
        "reuse_or_recovery_rate_calculation_gaps=0 "
        "kds_circularity_publication_receipt_gaps=0 "
        "accepted_for_circularity_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-099"
    )
    return 0
