#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 061."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-061-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-061-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-061.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-060-20260622.json"
...
WATER_STEWARDSHIP_KEYS = [
    "design_material_reduction_record_gaps",
    "design_for_disassembly_record_gaps",
    "repairability_assessment_gaps",
    "product_lifetime_extension_plan_gaps",
    "material_efficiency_calculation_gaps",
    "substitute_material_assessment_gaps",
    "design_change_environmental_review_gaps",
    "accepted_for_eco_design_profile"
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("eco_design_state") != "blocked_no_real_source_record":
        failures.append("eco_design_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-061-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_061_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-061", "invalid round id")
    ...
    require(evidence.get("eco_design_requirements") == REQUIREMENTS, "eco-design requirements mismatch")
    ...
    require(checks.get("eco_design_state") == "blocked_no_real_source_record", "eco-design state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-061-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-061-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 061 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing eco-design case: {phrase}")

    print(
        "was_real_source_record_monitor_061=pass "
        "submitted_real_candidate_files=0 design_material_reduction_record_gaps=0 design_for_disassembly_record_gaps=0 "
        "repairability_assessment_gaps=0 product_lifetime_extension_plan_gaps=0 material_efficiency_calculation_gaps=0 "
        "substitute_material_assessment_gaps=0 design_change_environmental_review_gaps=0 "
        "accepted_for_eco_design_profile=0 accepted_for_next_gate=0 "
