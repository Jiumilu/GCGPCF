#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 018."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-018-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-018-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-018.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-017-20260621.json"
...
DATA_GOVERNANCE_KEYS = [
    "data_custody_chain_gaps",
    "access_authorization_gaps",
    "audit_trail_gaps",
    "privacy_security_gaps",
    "retention_policy_gaps",
    "tamper_evidence_gaps",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in DATA_GOVERNANCE_KEYS:
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-018-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_018_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-018", "invalid round id")
    ...
    require(evidence.get("data_governance_requirements") == [
        "data_custody_chain",
        ...
        "retention_policy",
        "tamper_evidence",
    ], "data governance requirements mismatch")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-018-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-018-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 018 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "data_custody_chain_gap",
        "access_authorization_gap",
        "audit_trail_gap",
        "privacy_security_gap",
    ...
    print(
        "was_real_source_record_monitor_018=pass "
        "submitted_real_candidate_files=0 data_custody_chain_gaps=0 access_authorization_gaps=0 "
        "audit_trail_gaps=0 privacy_security_gaps=0 retention_policy_gaps=0 tamper_evidence_gaps=0 "
        "accepted_for_data_governance_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-019"
    )
    return 0
