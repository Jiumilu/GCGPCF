#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 088."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-088-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-088-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-088.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-087-20260622.json"
...
MONITOR_KEYS = [
    ...
    "administrative_rectification_order_gaps",
    "environmental_litigation_or_arbitration_record_gaps",
    "regulator_reinspection_report_gaps",
    "noncompliance_root_cause_and_corrective_action_gaps",
    "penalty_or_settlement_payment_record_gaps",
    "regulatory_case_closure_notice_gaps",
    "accepted_for_regulatory_dispute_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "environmental_regulatory_penalty_notice_gap",
    "administrative_rectification_order_gap",
    "environmental_litigation_or_arbitration_record_gap",
    "regulator_reinspection_report_gap",
    "noncompliance_root_cause_and_corrective_action_gap",
    "penalty_or_settlement_payment_record_gap",
    "regulatory_case_closure_notice_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-088-negative-environmental_regulatory_penalty_notice_gap.json",
    "real-source-record-monitor-088-negative-administrative_rectification_order_gap.json",
    "real-source-record-monitor-088-negative-environmental_litigation_or_arbitration_record_gap.json",
    "real-source-record-monitor-088-negative-regulator_reinspection_report_gap.json",
    "real-source-record-monitor-088-negative-noncompliance_root_cause_and_corrective_action_gap.json",
    "real-source-record-monitor-088-negative-penalty_or_settlement_payment_record_gap.json",
    "real-source-record-monitor-088-negative-regulatory_case_closure_notice_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("regulatory_dispute_state") != "blocked_no_real_source_record":
        failures.append("regulatory_dispute_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-088-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_088_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-088", "invalid round id")
    ...
    require(evidence.get("regulatory_dispute_requirements") == REQUIREMENTS, "regulatory dispute requirements mismatch")
    ...
    require(checks.get("regulatory_dispute_state") == "blocked_no_real_source_record", "regulatory dispute state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-088-positive.json")
    ...
    require("真实 P4 输入 monitor 088 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing regulatory dispute case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_088=pass submitted_real_candidate_files=0 "
        "environmental_regulatory_penalty_notice_gaps=0 administrative_rectification_order_gaps=0 "
        "environmental_litigation_or_arbitration_record_gaps=0 regulator_reinspection_report_gaps=0 "
        "noncompliance_root_cause_and_corrective_action_gaps=0 penalty_or_settlement_payment_record_gaps=0 "
        "regulatory_case_closure_notice_gaps=0 accepted_for_regulatory_dispute_profile=0 "
