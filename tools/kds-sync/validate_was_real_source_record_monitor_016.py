#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 016."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-016-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-016-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-016.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-015-20260621.json"
...
COMMERCIAL_KEYS = [
    "contract_terms_gaps",
    "payment_settlement_gaps",
    "invoice_tax_document_gaps",
    "logistics_document_gaps",
    "commercial_document_consistency_gaps",
    "delivery_finance_reconciliation_gaps",
    "accepted_for_commercial_logistics_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("commercial_logistics_state") != "blocked_no_real_source_record":
        failures.append("commercial_logistics_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-016-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_016_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-016", "invalid round id")
    ...
    require(evidence.get("commercial_logistics_requirements") == [
        "contract_terms",
        "payment_settlement",
    ...
    ], "commercial logistics requirements mismatch")
    ...
    require(checks.get("commercial_logistics_state") == "blocked_no_real_source_record", "commercial logistics state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-016-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-016-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 016 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "contract_terms_gap",
        ...
        require(phrase in loop_round, f"loop round missing commercial/logistics case: {phrase}")

    print(
        "was_real_source_record_monitor_016=pass "
        "submitted_real_candidate_files=0 contract_terms_gaps=0 payment_settlement_gaps=0 "
        "invoice_tax_document_gaps=0 logistics_document_gaps=0 commercial_document_consistency_gaps=0 "
        "delivery_finance_reconciliation_gaps=0 accepted_for_commercial_logistics_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-017"
    )
    return 0
