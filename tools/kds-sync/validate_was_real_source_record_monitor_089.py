#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 089."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-089-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-089-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-089.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-088-20260622.json"
...
MONITOR_KEYS = [
    "green_finance_facility_agreement_gaps",
    "esg_assurance_statement_gaps",
    "carbon_credit_registry_statement_gaps",
    "green_subsidy_or_tax_incentive_record_gaps",
    "sustainability_linked_kpi_verification_gaps",
    "green_invoice_settlement_record_gaps",
    "green_finance_audit_workpaper_gaps",
    "accepted_for_green_finance_esg_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "green_finance_facility_agreement_gap",
    "esg_assurance_statement_gap",
    "carbon_credit_registry_statement_gap",
    "green_subsidy_or_tax_incentive_record_gap",
    "sustainability_linked_kpi_verification_gap",
    "green_invoice_settlement_record_gap",
    "green_finance_audit_workpaper_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-089-negative-green_finance_facility_agreement_gap.json",
    "real-source-record-monitor-089-negative-esg_assurance_statement_gap.json",
    "real-source-record-monitor-089-negative-carbon_credit_registry_statement_gap.json",
    "real-source-record-monitor-089-negative-green_subsidy_or_tax_incentive_record_gap.json",
    "real-source-record-monitor-089-negative-sustainability_linked_kpi_verification_gap.json",
    "real-source-record-monitor-089-negative-green_invoice_settlement_record_gap.json",
    "real-source-record-monitor-089-negative-green_finance_audit_workpaper_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("green_finance_esg_state") != "blocked_no_real_source_record":
        failures.append("green_finance_esg_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-089-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_089_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-089", "invalid round id")
    ...
    require(evidence.get("green_finance_esg_requirements") == REQUIREMENTS, "green finance ESG requirements mismatch")
    ...
    require(checks.get("green_finance_esg_state") == "blocked_no_real_source_record", "green finance ESG state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-089-positive.json")
    ...
    require("真实 P4 输入 monitor 089 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing green finance ESG case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_089=pass submitted_real_candidate_files=0 "
        "green_finance_facility_agreement_gaps=0 esg_assurance_statement_gaps=0 "
        "carbon_credit_registry_statement_gaps=0 green_subsidy_or_tax_incentive_record_gaps=0 "
        "sustainability_linked_kpi_verification_gaps=0 green_invoice_settlement_record_gaps=0 "
        "green_finance_audit_workpaper_gaps=0 accepted_for_green_finance_esg_profile=0 "
