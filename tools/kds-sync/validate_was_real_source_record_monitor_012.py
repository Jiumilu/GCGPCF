#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 012."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-012-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-012-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-012.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-011-20260621.json"
...
SCOPE_KEYS = [
    "legacy_13_project_scope_candidates",
    "missing_was_scope_candidates",
    "unknown_unadmitted_project_candidates",
    "single_project_only_candidates",
    "scope_mismatch_between_evidence_loop_template",
    "future_project_without_admission",
    "business_chain_lineage_breaks",
    "data_lineage_breaks",
    "owner_accountability_gaps",
    "cross_project_handoff_evidence_gaps",
    "downstream_impact_unmapped",
    "source_to_runtime_trace_gaps",
    "accepted_for_project_group_lineage",
    "accepted_for_project_group_scope",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("project_group_scope_state") != "current_14_project_scope_only":
        failures.append("project_group_scope_state_must_remain_current")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-012-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_012_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-012", "invalid round id")
    ...
    future_policy = evidence.get("future_project_policy", {})
    require(future_policy.get("future_project_additions_allowed") is True, "future project additions policy mismatch")
    require(future_policy.get("admission_required_before_scope_use") is True, "future project admission policy mismatch")
    require(future_policy.get("required_admission_evidence") == [
        "project_group_admission_record",
        "controlled_document_registration",
        "kds_development_space_binding",
        "loop_was_context_update",
        "scenario_profile_matrix_update",
    ], "future project admission evidence mismatch")
    ...
    require(checks.get("project_group_scope_state") == "current_14_project_scope_only", "project group scope state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-012-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-012-negative-*.json"))
    require(len(negatives) == 12, "negative fixture count must be 12")
    ...
    require("真实 P4 输入 monitor 012 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing scope case: {phrase}")

    print(
        "was_real_source_record_monitor_012=pass "
        "submitted_real_candidate_files=0 legacy_13_project_scope_candidates=0 missing_was_scope_candidates=0 "
        "unknown_unadmitted_project_candidates=0 single_project_only_candidates=0 "
        "scope_mismatch_between_evidence_loop_template=0 future_project_without_admission=0 "
        "business_chain_lineage_breaks=0 data_lineage_breaks=0 owner_accountability_gaps=0 "
        "cross_project_handoff_evidence_gaps=0 downstream_impact_unmapped=0 "
        "source_to_runtime_trace_gaps=0 accepted_for_project_group_lineage=0 "
        "accepted_for_project_group_scope=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-013"
    )
    return 0
