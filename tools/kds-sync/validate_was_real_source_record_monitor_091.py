#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 091."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-091-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-091-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-091.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-090-20260622.json"
...
MONITOR_KEYS = [
    ...
    "personal_data_consent_or_authorization_gaps",
    "data_classification_register_gaps",
    "cross_border_data_transfer_assessment_gaps",
    "security_incident_response_record_gaps",
    "privileged_access_review_record_gaps",
    "data_retention_deletion_certificate_gaps",
    "accepted_for_data_governance_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "data_processing_agreement_gap",
    "personal_data_consent_or_authorization_gap",
    "data_classification_register_gap",
    "cross_border_data_transfer_assessment_gap",
    "security_incident_response_record_gap",
    "privileged_access_review_record_gap",
    "data_retention_deletion_certificate_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-091-negative-data_processing_agreement_gap.json",
    "real-source-record-monitor-091-negative-personal_data_consent_or_authorization_gap.json",
    "real-source-record-monitor-091-negative-data_classification_register_gap.json",
    "real-source-record-monitor-091-negative-cross_border_data_transfer_assessment_gap.json",
    "real-source-record-monitor-091-negative-security_incident_response_record_gap.json",
    "real-source-record-monitor-091-negative-privileged_access_review_record_gap.json",
    "real-source-record-monitor-091-negative-data_retention_deletion_certificate_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("data_governance_state") != "blocked_no_real_source_record":
        failures.append("data_governance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-091-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_091_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-091", "invalid round id")
    ...
    require(evidence.get("data_governance_requirements") == REQUIREMENTS, "data governance requirements mismatch")
    ...
    require(checks.get("data_governance_state") == "blocked_no_real_source_record", "data governance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-091-positive.json")
    ...
    require("真实 P4 输入 monitor 091 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing data governance case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_091=pass submitted_real_candidate_files=0 "
        "data_processing_agreement_gaps=0 personal_data_consent_or_authorization_gaps=0 "
        "data_classification_register_gaps=0 cross_border_data_transfer_assessment_gaps=0 "
        "security_incident_response_record_gaps=0 privileged_access_review_record_gaps=0 "
        "data_retention_deletion_certificate_gaps=0 accepted_for_data_governance_profile=0 "
