#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 069."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-069-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-069-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-069.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-068-20260622.json"
...
SUSTAINABLE_FINANCE_KEYS = [
    "green_financing_purpose_document_gaps",
    "fund_disbursement_trace_gaps",
    "supplier_esg_rating_record_gaps",
    "lender_due_diligence_record_gaps",
    "use_of_proceeds_tracking_gaps",
    "sustainability_performance_kpi_covenant_gaps",
    "financial_institution_verification_receipt_gaps",
    "accepted_for_sustainable_finance_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SUSTAINABLE_FINANCE_KEYS:
    ...
    if value.get("sustainable_finance_state") != "blocked_no_real_source_record":
        failures.append("sustainable_finance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-069-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_069_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-069", "invalid round id")
    ...
    require(evidence.get("sustainable_finance_requirements") == REQUIREMENTS, "sustainable finance requirements mismatch")
    ...
    require(checks.get("sustainable_finance_state") == "blocked_no_real_source_record", "sustainable finance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-069-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-069-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 069 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing sustainable finance case: {phrase}")

    print(
        "was_real_source_record_monitor_069=pass "
        "submitted_real_candidate_files=0 green_financing_purpose_document_gaps=0 fund_disbursement_trace_gaps=0 "
        "supplier_esg_rating_record_gaps=0 lender_due_diligence_record_gaps=0 "
        "use_of_proceeds_tracking_gaps=0 sustainability_performance_kpi_covenant_gaps=0 "
        "financial_institution_verification_receipt_gaps=0 "
        "accepted_for_sustainable_finance_profile=0 accepted_for_next_gate=0 "
