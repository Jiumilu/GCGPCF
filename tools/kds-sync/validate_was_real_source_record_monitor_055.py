#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 055."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-055-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-055-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-055.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-054-20260622.json"
...
CUSTOMS_TRADE_COMPLIANCE_KEYS = [
    "commercial_invoice_gaps",
    "packing_declaration_gaps",
    "export_declaration_gaps",
    "import_clearance_record_gaps",
    "customs_release_notice_gaps",
    "duty_tax_payment_record_gaps",
    "trade_compliance_license_gaps",
    "accepted_for_customs_trade_compliance_profile",
]
REQUIREMENTS = [
    "commercial_invoice",
    "packing_declaration",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CUSTOMS_TRADE_COMPLIANCE_KEYS:
    ...
    if value.get("customs_trade_compliance_state") != "blocked_no_real_source_record":
        failures.append("customs_trade_compliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-055-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_055_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-055", "invalid round id")
    ...
    require(evidence.get("customs_trade_compliance_requirements") == REQUIREMENTS, "customs trade compliance requirements mismatch")
    ...
    require(checks.get("customs_trade_compliance_state") == "blocked_no_real_source_record", "customs trade compliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-055-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-055-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 055 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing customs trade compliance case: {phrase}")

    print(
        "was_real_source_record_monitor_055=pass "
        "submitted_real_candidate_files=0 commercial_invoice_gaps=0 "
        "packing_declaration_gaps=0 export_declaration_gaps=0 "
        "import_clearance_record_gaps=0 customs_release_notice_gaps=0 "
        "duty_tax_payment_record_gaps=0 trade_compliance_license_gaps=0 "
        "accepted_for_customs_trade_compliance_profile=0 accepted_for_next_gate=0 "
