#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 052."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-052-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-052-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-052.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-051-20260622.json"
...
PURCHASE_COMMITMENT_KEYS = [
    "purchase_requisition_gaps",
    "purchase_order_gaps",
    "supplier_order_acknowledgement_gaps",
    "delivery_schedule_commitment_gaps",
    "purchase_order_change_record_gaps",
    "procurement_approval_record_gaps",
    "supplier_communication_log_gaps",
    "accepted_for_purchase_commitment_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in PURCHASE_COMMITMENT_KEYS:
    ...
    if value.get("purchase_commitment_state") != "blocked_no_real_source_record":
        failures.append("purchase_commitment_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-052-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_052_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-052", "invalid round id")
    ...
    require(evidence.get("purchase_commitment_requirements") == REQUIREMENTS, "purchase commitment requirements mismatch")
    ...
    require(checks.get("purchase_commitment_state") == "blocked_no_real_source_record", "purchase commitment state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-052-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-052-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 052 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing purchase commitment case: {phrase}")

    print(
        "was_real_source_record_monitor_052=pass "
        "submitted_real_candidate_files=0 purchase_requisition_gaps=0 purchase_order_gaps=0 "
        "supplier_order_acknowledgement_gaps=0 delivery_schedule_commitment_gaps=0 "
        "purchase_order_change_record_gaps=0 procurement_approval_record_gaps=0 "
        "supplier_communication_log_gaps=0 accepted_for_purchase_commitment_profile=0 "
