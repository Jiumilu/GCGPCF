#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 048."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-048-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-048-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-048.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-047-20260622.json"
...
MEASUREMENT_CALIBRATION_KEYS = [
    "measurement_equipment_registry_gaps",
    "calibration_certificate_gaps",
    "inspection_method_validation_gaps",
    "measurement_uncertainty_record_gaps",
    "equipment_maintenance_record_gaps",
    "out_of_tolerance_disposition_gaps",
    "measurement_traceability_chain_gaps",
    "accepted_for_measurement_calibration_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in MEASUREMENT_CALIBRATION_KEYS:
    ...
    if value.get("measurement_calibration_state") != "blocked_no_real_source_record":
        failures.append("measurement_calibration_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-048-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_048_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-048", "invalid round id")
    ...
    require(evidence.get("measurement_calibration_requirements") == REQUIREMENTS, "measurement calibration requirements mismatch")
    ...
    require(checks.get("measurement_calibration_state") == "blocked_no_real_source_record", "measurement calibration state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-048-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-048-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 048 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing measurement calibration case: {phrase}")

    print(
        "was_real_source_record_monitor_048=pass "
        "submitted_real_candidate_files=0 measurement_equipment_registry_gaps=0 "
        "calibration_certificate_gaps=0 inspection_method_validation_gaps=0 "
        "measurement_uncertainty_record_gaps=0 equipment_maintenance_record_gaps=0 "
        "out_of_tolerance_disposition_gaps=0 measurement_traceability_chain_gaps=0 "
        "accepted_for_measurement_calibration_profile=0 accepted_for_next_gate=0 hold_required=1 "
