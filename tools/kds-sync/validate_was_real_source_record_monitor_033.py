#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 033."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-033-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-033-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-033.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-032-20260622.json"
...
REGULATORY_PRODUCT_COMPLIANCE_KEYS = ['product_conformity_certificate_gaps', 'third_party_test_report_gaps', 'environmental_compliance_permit_gaps', 'chain_of_custody_certificate_gaps', 'import_export_license_gaps', 'digital_product_passport_record_gaps', 'accepted_for_regulatory_product_compliance_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in REGULATORY_PRODUCT_COMPLIANCE_KEYS:
    ...
    if value.get("regulatory_product_compliance_state") != "blocked_no_real_source_record":
        failures.append("regulatory_product_compliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-033-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_033_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-033", "invalid round id")
    ...
    require(evidence.get("regulatory_product_compliance_requirements") == ['product_conformity_certificate', 'third_party_test_report', 'environmental_compliance_permit', 'chain_of_custody_certificate', 'import_export_license', 'digital_product_passport_record'], "regulatory product compliance requirements mismatch")
    ...
    require(checks.get("regulatory_product_compliance_state") == "blocked_no_real_source_record", "regulatory product compliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-033-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-033-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 033 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['product_conformity_certificate_gap', 'third_party_test_report_gap', 'environmental_compliance_permit_gap', 'chain_of_custody_certificate_gap', 'import_export_license_gap', 'digital_product_passport_record_gap']:
        require(phrase in loop_round, f"loop round missing regulatory product compliance case: {phrase}")

    print(
        "was_real_source_record_monitor_033=pass "
        "submitted_real_candidate_files=0 product_conformity_certificate_gaps=0 "
        "third_party_test_report_gaps=0 environmental_compliance_permit_gaps=0 "
        "chain_of_custody_certificate_gaps=0 import_export_license_gaps=0 "
        "digital_product_passport_record_gaps=0 accepted_for_regulatory_product_compliance_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-034"
    )
    return 0
