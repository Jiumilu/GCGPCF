#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 023."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-023-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-023-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-023.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-022-20260621.json"
...
WATER_KEYS = [
    "water_withdrawal_permit_gaps",
    "wastewater_discharge_permit_gaps",
    "effluent_test_report_gaps",
    "water_reuse_record_gaps",
    "stormwater_control_evidence_gaps",
    "discharge_violation_record_gaps",
    "accepted_for_water_stewardship_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("water_stewardship_state") != "blocked_no_real_source_record":
        failures.append("water_stewardship_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-023-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_023_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-023", "invalid round id")
    ...
    require(evidence.get("water_stewardship_requirements") == [
        "water_withdrawal_permit",
        "wastewater_discharge_permit",
        "effluent_test_report",
        "water_reuse_record",
        "stormwater_control_evidence",
        "discharge_violation_record",
    ], "water stewardship requirements mismatch")
    ...
    require(checks.get("water_stewardship_state") == "blocked_no_real_source_record", "water stewardship state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-023-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negative_names = [
        "real-source-record-monitor-023-negative-water-withdrawal-permit-gap.json",
        "real-source-record-monitor-023-negative-wastewater-discharge-permit-gap.json",
        "real-source-record-monitor-023-negative-effluent-test-report-gap.json",
        "real-source-record-monitor-023-negative-water-reuse-record-gap.json",
        "real-source-record-monitor-023-negative-stormwater-control-evidence-gap.json",
        "real-source-record-monitor-023-negative-discharge-violation-record-gap.json",
    ]
    negatives = [FIXTURE_DIR / name for name in negative_names]
    ...
    require("真实 P4 输入 monitor 023 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "water_withdrawal_permit_gap",
        "wastewater_discharge_permit_gap",
        "effluent_test_report_gap",
        "water_reuse_record_gap",
        "stormwater_control_evidence_gap",
        "discharge_violation_record_gap",
    ]:
        require(phrase in loop_round, f"loop round missing water stewardship case: {phrase}")

    print(
        "was_real_source_record_monitor_023=pass "
        "submitted_real_candidate_files=0 water_withdrawal_permit_gaps=0 wastewater_discharge_permit_gaps=0 "
        "effluent_test_report_gaps=0 water_reuse_record_gaps=0 stormwater_control_evidence_gaps=0 "
        "discharge_violation_record_gaps=0 accepted_for_water_stewardship_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-024"
    )
    return 0
