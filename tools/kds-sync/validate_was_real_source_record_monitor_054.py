#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 054."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-054-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-054-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-054.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-053-20260622.json"
...
TRANSPORT_START_KEYS = [
    "carrier_booking_record_gaps",
    "pickup_appointment_gaps",
    "shipping_instruction_gaps",
    "export_transport_document_gaps",
    "chain_of_custody_handoff_gaps",
    "in_transit_tracking_event_gaps",
    "transport_insurance_or_liability_confirmation_gaps",
    "accepted_for_transport_start_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in TRANSPORT_START_KEYS:
    ...
    if value.get("transport_start_state") != "blocked_no_real_source_record":
        failures.append("transport_start_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-054-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_054_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-054", "invalid round id")
    ...
    require(evidence.get("transport_start_requirements") == REQUIREMENTS, "transport start requirements mismatch")
    ...
    require(checks.get("transport_start_state") == "blocked_no_real_source_record", "transport start state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-054-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-054-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 054 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing transport start case: {phrase}")

    print(
        "was_real_source_record_monitor_054=pass "
        "submitted_real_candidate_files=0 carrier_booking_record_gaps=0 "
        "pickup_appointment_gaps=0 shipping_instruction_gaps=0 "
        "export_transport_document_gaps=0 chain_of_custody_handoff_gaps=0 "
        "in_transit_tracking_event_gaps=0 transport_insurance_or_liability_confirmation_gaps=0 "
        "accepted_for_transport_start_profile=0 accepted_for_next_gate=0 "
