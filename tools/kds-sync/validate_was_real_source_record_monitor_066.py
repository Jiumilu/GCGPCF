#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 066."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-066-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-066-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-066.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-065-20260622.json"
...
GREEN_PROCUREMENT_KEYS = [
    "green_procurement_eligibility_gaps",
    "ecolabel_certificate_gaps",
    "product_environmental_declaration_gaps",
    "public_tender_green_criteria_mapping_gaps",
    "green_procurement_contract_clause_gaps",
    "lifecycle_cost_environmental_score_gaps",
    "supplier_green_commitment_gaps",
    "accepted_for_green_procurement_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in GREEN_PROCUREMENT_KEYS:
    ...
    if value.get("green_procurement_state") != "blocked_no_real_source_record":
        failures.append("green_procurement_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-066-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_066_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-066", "invalid round id")
    ...
    require(evidence.get("green_procurement_requirements") == REQUIREMENTS, "green procurement requirements mismatch")
    ...
    require(checks.get("green_procurement_state") == "blocked_no_real_source_record", "green procurement state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-066-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-066-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 066 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing green procurement case: {phrase}")

    print(
        "was_real_source_record_monitor_066=pass "
        "submitted_real_candidate_files=0 green_procurement_eligibility_gaps=0 ecolabel_certificate_gaps=0 "
        "product_environmental_declaration_gaps=0 public_tender_green_criteria_mapping_gaps=0 "
        "green_procurement_contract_clause_gaps=0 lifecycle_cost_environmental_score_gaps=0 "
        "supplier_green_commitment_gaps=0 "
        "accepted_for_green_procurement_profile=0 accepted_for_next_gate=0 "
