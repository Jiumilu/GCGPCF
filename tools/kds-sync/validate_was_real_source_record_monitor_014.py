#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 014."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-014-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-014-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-014.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-013-20260621.json"
...
ESG_KEYS = [
    "esg_rating_substitution_attempts",
    "carbon_accounting_method_gaps",
    "third_party_audit_report_gaps",
    "verifier_qualification_gaps",
    "audit_period_mismatch",
    "audit_statement_without_kds_backlink",
    "accepted_for_esg_carbon_audit_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("esg_carbon_audit_state") != "blocked_no_real_source_record":
        failures.append("esg_carbon_audit_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-014-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_014_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-014", "invalid round id")
    ...
    require(evidence.get("esg_carbon_audit_requirements") == [
        "no_esg_rating_substitution",
        "carbon_accounting_method",
        "third_party_audit_report",
        "verifier_qualification",
        "audit_period_alignment",
        "kds_backlink",
    ], "ESG carbon audit requirements mismatch")
    ...
    require(checks.get("esg_carbon_audit_state") == "blocked_no_real_source_record", "ESG carbon audit state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-014-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-014-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 014 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        "esg_rating_substitution_attempt",
        "carbon_accounting_method_gap",
        "third_party_audit_report_gap",
        "verifier_qualification_gap",
        "audit_period_mismatch",
        "audit_statement_without_kds_backlink",
    ]:
        require(phrase in loop_round, f"loop round missing ESG/audit case: {phrase}")

    print(
        "was_real_source_record_monitor_014=pass "
        "submitted_real_candidate_files=0 esg_rating_substitution_attempts=0 "
        "carbon_accounting_method_gaps=0 third_party_audit_report_gaps=0 "
        "verifier_qualification_gaps=0 audit_period_mismatch=0 "
        "audit_statement_without_kds_backlink=0 accepted_for_esg_carbon_audit_profile=0 "
        "accepted_for_next_gate=0 hold_required=1 real_source_records=0 valid_source_records=0 "
        "runtime_primary_key_ready=0 review_queue=0 runtime_intake=0 waes_review=0 "
        "verified=0 accepted=false integrated=false production_ready=false "
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-015"
    )
    return 0
