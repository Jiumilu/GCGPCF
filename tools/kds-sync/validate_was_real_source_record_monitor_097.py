#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 097."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-097-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-097-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-097.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-096-20260622.json"
...
MONITOR_KEYS = ['green_claim_approval_record_gaps', 'eco_label_use_authorization_gaps', 'product_carbon_claim_substantiation_gaps', 'customer_facing_disclosure_approval_gaps', 'marketing_material_compliance_review_gaps', 'green_claim_complaint_handling_record_gaps', 'claim_retraction_or_correction_log_gaps', 'accepted_for_green_claim_disclosure_profile']
REQUIREMENTS = ['green_claim_approval_record', 'eco_label_use_authorization', 'product_carbon_claim_substantiation', 'customer_facing_disclosure_approval', 'marketing_material_compliance_review', 'green_claim_complaint_handling_record', 'claim_retraction_or_correction_log']
...
LOOP_CASES = ['green_claim_approval_record_gap', 'eco_label_use_authorization_gap', 'product_carbon_claim_substantiation_gap', 'customer_facing_disclosure_approval_gap', 'marketing_material_compliance_review_gap', 'green_claim_complaint_handling_record_gap', 'claim_retraction_or_correction_log_gap']
NEGATIVE_FIXTURES = [f"real-source-record-monitor-097-negative-{case}.json" for case in LOOP_CASES]
...
def require_frontmatter(path: Path, text: str) -> None:
    ...
    metadata = text[:end]
    for phrase in ["status: controlled", "domain: ontology-governance", "kds_space: 开发", f"source_path: {path.relative_to(ROOT).as_posix()}", "sync_direction: bidirectional", "WAS"]:
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("green_claim_disclosure_state") != "blocked_no_real_source_record":
        failures.append("green_claim_disclosure_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-097-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_097_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-097", "invalid round id")
    ...
    require(evidence.get("green_claim_disclosure_requirements") == REQUIREMENTS, "green claim disclosure requirements mismatch")
    ...
    require(checks.get("green_claim_disclosure_state") == "blocked_no_real_source_record", "green claim disclosure state mismatch")
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-097-positive.json")
    ...
    require("真实 P4 输入 monitor 097 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing green claim disclosure case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_097=pass submitted_real_candidate_files=0 "
        "green_claim_approval_record_gaps=0 "
        "eco_label_use_authorization_gaps=0 "
        "product_carbon_claim_substantiation_gaps=0 "
        "customer_facing_disclosure_approval_gaps=0 "
        ...
        "green_claim_complaint_handling_record_gaps=0 "
        "claim_retraction_or_correction_log_gaps=0 "
        "accepted_for_green_claim_disclosure_profile=0 accepted_for_next_gate=0 hold_required=1 "
        "real_source_records=0 valid_source_records=0 runtime_primary_key_ready=0 "
        "review_queue=0 runtime_intake=0 waes_review=0 verified=0 accepted=false "
        "integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-098"
    )
    return 0
