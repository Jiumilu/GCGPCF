#!/usr/bin/env python3
"""Validate D173 GCKF P0 aggregation completeness precheck current-state evidence."""
...
FIXTURE = ROOT / "fixtures" / "api" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-completeness-precheck-current-state-d173-20260622.json"
EVIDENCE_JSON = ROOT / "docs" / "harness" / "evidence" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-completeness-precheck-current-state-d173-20260622.json"
EVIDENCE_MD = ROOT / "docs" / "harness" / "evidence" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-completeness-precheck-current-state-d173-20260622.md"
LOOP_MD = ROOT / "docs" / "harness" / "loops" / "loop-round-GPCF-GCKF-P0-D173-001.md"


def fail(message: str) -> None:
    print(
        "gckf_p0_formal_evidence_execution_committee_acceptance_acknowledgement_notification_receipt_aggregation_completeness_precheck_current_state_d173="
...
def main() -> None:
    ...
    historical = load_json(ROOT / fixture["sourceHistoricalAggregationCompletenessPrecheck"])
    current_source = load_json(ROOT / fixture["sourceCurrentAggregationPreview"])
    precheck = fixture["aggregationCompletenessPrecheck"]
    historical_precheck = historical["aggregationCompletenessPrecheck"]
    source_aggregation = current_source["committeeAcceptanceAcknowledgementNotificationReceiptAggregationPreview"]

    require(
      fixture.get("aggregationCompletenessPrecheckStatus") == expected["aggregationCompletenessPrecheckStatus"],
    ...
    require(
        fixture.get("notFinalAggregationCompletenessPrecheck") is expected["notFinalAggregationCompletenessPrecheck"],
    ...
    require(
        historical.get("aggregationCompletenessPrecheckStatus") == "candidate_preview",
    ...
    require(
        current_source.get("committeeAcceptanceAcknowledgementNotificationReceiptAggregationPreviewStatus")
        == expected["sourceAggregationPreviewStatus"],
        "source_aggregation_status_mismatch",
    )
    for source_key, expected_key in (
        ...
        require(source_aggregation.get(source_key) == expected[expected_key], f"{source_key}_source_mismatch")
    require(source_aggregation.get("dryRunOnly") is True, "source_aggregation_not_dry_run_only")
    require(
        precheck.get("sourceAggregationPreviewId") == source_aggregation.get("id"),
        "source_aggregation_id_mismatch",
    )
    require(
        fixture.get("coveredAggregationPreviewStatus") == expected["coveredAggregationPreviewStatus"],
        "covered_status_mismatch",
    )
    require(
        fixture["coveredAggregationPreviewStatus"] == source_aggregation.get("previewStatus"),
    ...
    require(
        evidence.get("current_aggregation_completeness_precheck_status") == expected["aggregationCompletenessPrecheckStatus"],
    ...
    require("hold_context_refs" in evidence_md or "Hold 上下文" in evidence_md, "evidence_md_missing_hold_section")
    ...
    print(
        "gckf_p0_formal_evidence_execution_committee_acceptance_acknowledgement_notification_receipt_aggregation_completeness_precheck_current_state_d173=pass"
    )
    print(f"aggregation_completeness_precheck_status={fixture['aggregationCompletenessPrecheckStatus']}")
