#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 056."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-056-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-056-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-056.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-055-20260622.json"
...
DESTINATION_RECEIVING_KEYS = [
    "delivery_appointment_gaps",
    "consignee_receiving_record_gaps",
    "proof_of_delivery_gaps",
    "unloading_inspection_record_gaps",
    "damage_or_discrepancy_report_gaps",
    "temperature_or_condition_log_gaps",
    "final_delivery_confirmation_gaps",
    "accepted_for_destination_receiving_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in DESTINATION_RECEIVING_KEYS:
    ...
    if value.get("destination_receiving_state") != "blocked_no_real_source_record":
        failures.append("destination_receiving_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-056-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_056_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-056", "invalid round id")
    ...
    require(evidence.get("destination_receiving_requirements") == REQUIREMENTS, "destination receiving requirements mismatch")
    ...
    require(checks.get("destination_receiving_state") == "blocked_no_real_source_record", "destination receiving state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-056-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-056-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 056 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing destination receiving case: {phrase}")

    print(
        "was_real_source_record_monitor_056=pass "
        "submitted_real_candidate_files=0 delivery_appointment_gaps=0 "
        "consignee_receiving_record_gaps=0 proof_of_delivery_gaps=0 "
        "unloading_inspection_record_gaps=0 damage_or_discrepancy_report_gaps=0 "
        "temperature_or_condition_log_gaps=0 final_delivery_confirmation_gaps=0 "
        "accepted_for_destination_receiving_profile=0 accepted_for_next_gate=0 "
