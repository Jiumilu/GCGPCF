#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 039."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-039-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-039-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-039.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-038-20260622.json"
...
SUPPLIER_ONBOARDING_KEYS = ['supplier_qualification_certificate_gaps', 'business_registration_license_gaps', 'beneficial_ownership_declaration_gaps', 'bank_account_verification_gaps', 'procurement_authorization_record_gaps', 'supplier_contract_approval_gaps', 'accepted_for_supplier_onboarding_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SUPPLIER_ONBOARDING_KEYS:
    ...
    if value.get("supplier_onboarding_state") != "blocked_no_real_source_record":
        failures.append("supplier_onboarding_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-039-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_039_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-039", "invalid round id")
    ...
    require(evidence.get("supplier_onboarding_requirements") == ['supplier_qualification_certificate', 'business_registration_license', 'beneficial_ownership_declaration', 'bank_account_verification', 'procurement_authorization_record', 'supplier_contract_approval'], "responsible sourcing requirements mismatch")
    ...
    require(checks.get("supplier_onboarding_state") == "blocked_no_real_source_record", "responsible sourcing state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-039-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-039-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 039 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['supplier_qualification_certificate_gap', 'business_registration_license_gap', 'beneficial_ownership_declaration_gap', 'bank_account_verification_gap', 'procurement_authorization_record_gap', 'supplier_contract_approval_gap']:
        require(phrase in loop_round, f"loop round missing responsible sourcing case: {phrase}")

    print(
        "was_real_source_record_monitor_039=pass "
        "submitted_real_candidate_files=0 supplier_qualification_certificate_gaps=0 "
        "business_registration_license_gaps=0 beneficial_ownership_declaration_gaps=0 "
        "bank_account_verification_gaps=0 procurement_authorization_record_gaps=0 "
        "supplier_contract_approval_gaps=0 accepted_for_supplier_onboarding_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-040"
    )
    return 0
