#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 031."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-031-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-031-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-031.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-030-20260622.json"
...
CUSTOMER_AFTER_SALES_KEYS = [
    ...
    "customer_complaint_record_gaps",
    "return_or_replacement_authorization_gaps",
    "product_recall_notice_gaps",
    "warranty_claim_record_gaps",
    "reverse_logistics_traceability_gaps",
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-031-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_031_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-031", "invalid round id")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-031-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-031-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 031 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
    ...
    print(
        "was_real_source_record_monitor_031=pass "
        "submitted_real_candidate_files=0 customer_acceptance_record_gaps=0 "
        "customer_complaint_record_gaps=0 return_or_replacement_authorization_gaps=0 "
        "product_recall_notice_gaps=0 warranty_claim_record_gaps=0 "
        "reverse_logistics_traceability_gaps=0 accepted_for_customer_after_sales_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-032"
    )
    return 0
