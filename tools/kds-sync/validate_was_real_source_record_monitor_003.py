#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 003."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-003-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-003-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-003.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-002-20260621.json"
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("weak_evidence_promoted") is not False:
        failures.append("weak_evidence_must_not_be_promoted")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-003-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_003_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-003", "invalid round id")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-003-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-003-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 003 已建立" in loop_round, "loop round feedback missing")

    print(
        "was_real_source_record_monitor_003=pass "
        "required_p4_inputs=6 submitted_real_inputs=0 submitted_files_found=0 candidate_files_checked=0 "
        "accepted_for_next_gate=0 hold_required=1 monitor_state=waiting weak_evidence_promoted=false "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-004"
    )
    return 0
