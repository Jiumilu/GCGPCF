#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 020."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-020-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-020-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-020.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-019-20260621.json"
...
SOCIAL_EHS_KEYS = [
    "labor_rights_evidence_gaps",
    "health_safety_record_gaps",
    "training_qualification_gaps",
    "subcontractor_responsibility_gaps",
    "grievance_whistleblower_channel_gaps",
    "community_impact_evidence_gaps",
    "accepted_for_social_ehs_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("social_ehs_state") != "blocked_no_real_source_record":
        failures.append("social_ehs_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-020-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_020_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-020", "invalid round id")
    ...
    require(evidence.get("social_ehs_requirements") == [
    ...
    ], "social EHS requirements mismatch")
    ...
    require(checks.get("social_ehs_state") == "blocked_no_real_source_record", "social EHS state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-020-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-020-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 020 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing social EHS case: {phrase}")

    print(
        "was_real_source_record_monitor_020=pass "
        "submitted_real_candidate_files=0 labor_rights_evidence_gaps=0 health_safety_record_gaps=0 "
        "training_qualification_gaps=0 subcontractor_responsibility_gaps=0 "
        "grievance_whistleblower_channel_gaps=0 community_impact_evidence_gaps=0 "
        "accepted_for_social_ehs_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-021"
    )
    return 0
