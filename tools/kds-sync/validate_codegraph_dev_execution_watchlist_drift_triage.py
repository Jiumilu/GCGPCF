#!/usr/bin/env python3
"""Validate CodeGraph watchlist drift triage evidence."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/codegraph-dev-execution-watchlist-drift-triage-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/codegraph-dev-execution-watchlist-drift-triage-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-CODEGRAPH-DEV-EXECUTION-WATCHLIST-DRIFT-TRIAGE-013.md"
...
def main() -> int:
    ...
    require(evidence["evidence_id"] == "CODEGRAPH-DEV-EXECUTION-WATCHLIST-DRIFT-TRIAGE-20260622", "invalid evidence id")
    require(evidence["scope"] == "GPCF-CODEGRAPH-DEV-EXECUTION-WATCHLIST-DRIFT-TRIAGE-013", "invalid scope")
    require(evidence["status"] == "triage_complete_authorization_boundary", "invalid status")
    require(evidence["next_round"] == "GPCF-CODEGRAPH-DEV-EXECUTION-WATCHLIST-AUTHORIZATION-PACK-014", "invalid next round")
    ...
    for name, repo in REPOS.items():
        ...
        require(status["pendingChanges"]["removed"] == item["codegraph_pending"]["removed"], f"removed pending mismatch: {name}")
        ...
        require(counts["total"] >= item["git_dirty_total"], f"git dirty total should not shrink below evidence during same round: {name}")
        require(item["decision"] in {"authorization_required", "precheck_candidate"}, f"invalid decision: {name}")

    governance = evidence["governance"]
    require(governance["mode"] == "read_only_triage", "mode must be read_only_triage")
    ...
    previous = run(["python3", "tools/kds-sync/validate_codegraph_dev_execution_steady_state_monitor.py"])
    require(previous.returncode == 0, f"steady-state monitor validator failed: {previous.stdout}{previous.stderr}")
    ...
    for phrase in [
        "triage_complete_authorization_boundary",
        "Brain",
        "GFIS",
        "KDS",
        "Studio",
    ...
    print(
        "codegraph_dev_execution_watchlist_drift_triage=pass "
        "repo_count=4 "
        "stop_type=authorization_boundary "
        "brain_decision=authorization_required "
        "gfis_clean_reindex_authorized=false "
        "kds_decision=authorization_required "
        "studio_decision=precheck_candidate "
        "accepted=false integrated=false production_ready=false "
        "next=GPCF-CODEGRAPH-DEV-EXECUTION-WATCHLIST-AUTHORIZATION-PACK-014"
    )
    return 0
