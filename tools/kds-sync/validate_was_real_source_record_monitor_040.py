#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 040."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-040-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-040-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-040.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-039-20260622.json"
...
SUPPLIER_PERFORMANCE_KEYS = [
    "supplier_scorecard_gaps",
    "delivery_performance_review_gaps",
    "quality_performance_review_gaps",
    "esg_performance_review_gaps",
    "corrective_action_closure_gaps",
    "supplier_requalification_decision_gaps",
    "accepted_for_supplier_performance_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in SUPPLIER_PERFORMANCE_KEYS:
    ...
    if value.get("supplier_performance_state") != "blocked_no_real_source_record":
        failures.append("supplier_performance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-040-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_040_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-040", "invalid round id")
    ...
    require(evidence.get("supplier_performance_requirements") == [
        "supplier_scorecard",
    ...
    ], "supplier performance requirements mismatch")
    ...
    require(checks.get("supplier_performance_state") == "blocked_no_real_source_record", "supplier performance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-040-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-040-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 040 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing supplier performance case: {phrase}")

    print(
        "was_real_source_record_monitor_040=pass "
        "submitted_real_candidate_files=0 supplier_scorecard_gaps=0 "
        "delivery_performance_review_gaps=0 quality_performance_review_gaps=0 "
        "esg_performance_review_gaps=0 corrective_action_closure_gaps=0 "
        "supplier_requalification_decision_gaps=0 accepted_for_supplier_performance_profile=0 "
