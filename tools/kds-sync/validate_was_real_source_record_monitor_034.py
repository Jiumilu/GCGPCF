#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 034."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-034-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-034-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-034.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-033-20260622.json"
...
SOCIAL_SAFETY_KEYS = ['worker_health_safety_training_record_gaps', 'ppe_issue_record_gaps', 'incident_near_miss_report_gaps', 'emergency_drill_record_gaps', 'community_grievance_record_gaps', 'social_license_or_local_permit_record_gaps', 'accepted_for_social_safety_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("social_safety_state") != "blocked_no_real_source_record":
        failures.append("social_safety_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-034-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_034_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-034", "invalid round id")
    ...
    require(evidence.get("social_safety_requirements") == ['worker_health_safety_training_record', 'ppe_issue_record', 'incident_near_miss_report', 'emergency_drill_record', 'community_grievance_record', 'social_license_or_local_permit_record'], "social safety requirements mismatch")
    ...
    require(checks.get("social_safety_state") == "blocked_no_real_source_record", "social safety state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-034-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-034-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 034 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['worker_health_safety_training_record_gap', 'ppe_issue_record_gap', 'incident_near_miss_report_gap', 'emergency_drill_record_gap', 'community_grievance_record_gap', 'social_license_or_local_permit_record_gap']:
        require(phrase in loop_round, f"loop round missing social safety case: {phrase}")

    print(
        "was_real_source_record_monitor_034=pass "
        "submitted_real_candidate_files=0 worker_health_safety_training_record_gaps=0 "
        "ppe_issue_record_gaps=0 incident_near_miss_report_gaps=0 "
        "emergency_drill_record_gaps=0 community_grievance_record_gaps=0 "
        "social_license_or_local_permit_record_gaps=0 accepted_for_social_safety_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-035"
    )
    return 0
