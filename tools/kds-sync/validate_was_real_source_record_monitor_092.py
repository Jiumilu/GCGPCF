#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 092."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-092-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-092-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-092.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-091-20260622.json"
...
MONITOR_KEYS = [
    "ai_assisted_decision_policy_gaps",
    "model_version_registry_gaps",
    "prompt_template_approval_record_gaps",
    "human_review_decision_log_gaps",
    "bias_and_fairness_test_report_gaps",
    "ai_output_provenance_record_gaps",
    "automated_decision_rollback_record_gaps",
    "accepted_for_ai_governance_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "ai_assisted_decision_policy_gap",
    "model_version_registry_gap",
    "prompt_template_approval_record_gap",
    "human_review_decision_log_gap",
    "bias_and_fairness_test_report_gap",
    "ai_output_provenance_record_gap",
    "automated_decision_rollback_record_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-092-negative-ai_assisted_decision_policy_gap.json",
    "real-source-record-monitor-092-negative-model_version_registry_gap.json",
    "real-source-record-monitor-092-negative-prompt_template_approval_record_gap.json",
    "real-source-record-monitor-092-negative-human_review_decision_log_gap.json",
    "real-source-record-monitor-092-negative-bias_and_fairness_test_report_gap.json",
    "real-source-record-monitor-092-negative-ai_output_provenance_record_gap.json",
    "real-source-record-monitor-092-negative-automated_decision_rollback_record_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("ai_governance_state") != "blocked_no_real_source_record":
        failures.append("ai_governance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-092-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_092_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-092", "invalid round id")
    ...
    require(evidence.get("ai_governance_requirements") == REQUIREMENTS, "AI governance requirements mismatch")
    ...
    require(checks.get("ai_governance_state") == "blocked_no_real_source_record", "AI governance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-092-positive.json")
    ...
    require("真实 P4 输入 monitor 092 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing AI governance case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_092=pass submitted_real_candidate_files=0 "
        "ai_assisted_decision_policy_gaps=0 model_version_registry_gaps=0 "
        "prompt_template_approval_record_gaps=0 human_review_decision_log_gaps=0 "
        "bias_and_fairness_test_report_gaps=0 ai_output_provenance_record_gaps=0 "
        "automated_decision_rollback_record_gaps=0 accepted_for_ai_governance_profile=0 "
