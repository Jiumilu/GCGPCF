#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 060."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-060-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-060-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-060.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-059-20260622.json"
...
WATER_STEWARDSHIP_KEYS = [
    "renewable_energy_contract_gaps",
    "energy_consumption_meter_record_gaps",
    "green_power_certificate_gaps",
    "renewable_energy_certificate_retirement_gaps",
    "onsite_generation_meter_record_gaps",
    "energy_mix_allocation_record_gaps",
    "grid_emission_factor_source_gaps",
    "accepted_for_renewable_energy_profile"
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("renewable_energy_state") != "blocked_no_real_source_record":
        failures.append("renewable_energy_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-060-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_060_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-060", "invalid round id")
    ...
    require(evidence.get("renewable_energy_requirements") == REQUIREMENTS, "renewable energy requirements mismatch")
    ...
    require(checks.get("renewable_energy_state") == "blocked_no_real_source_record", "renewable energy state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-060-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-060-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 060 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing renewable energy case: {phrase}")

    print(
        "was_real_source_record_monitor_060=pass "
        "submitted_real_candidate_files=0 renewable_energy_contract_gaps=0 energy_consumption_meter_record_gaps=0 "
        "green_power_certificate_gaps=0 renewable_energy_certificate_retirement_gaps=0 onsite_generation_meter_record_gaps=0 "
        "energy_mix_allocation_record_gaps=0 grid_emission_factor_source_gaps=0 "
        "accepted_for_renewable_energy_profile=0 accepted_for_next_gate=0 "
