#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 037."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-037-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-037-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-037.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-036-20260622.json"
...
CYBERSECURITY_INTEGRATION_KEYS = ['cybersecurity_control_assessment_gaps', 'api_integration_audit_log_gaps', 'master_data_change_approval_gaps', 'access_control_review_record_gaps', 'backup_restore_test_record_gaps', 'vulnerability_remediation_record_gaps', 'accepted_for_cybersecurity_integration_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CYBERSECURITY_INTEGRATION_KEYS:
    ...
    if value.get("cybersecurity_integration_state") != "blocked_no_real_source_record":
        failures.append("cybersecurity_integration_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-037-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_037_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-037", "invalid round id")
    ...
    require(evidence.get("cybersecurity_integration_requirements") == ['cybersecurity_control_assessment', 'api_integration_audit_log', 'master_data_change_approval', 'access_control_review_record', 'backup_restore_test_record', 'vulnerability_remediation_record'], "cybersecurity integration requirements mismatch")
    ...
    require(checks.get("cybersecurity_integration_state") == "blocked_no_real_source_record", "cybersecurity integration state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-037-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-037-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 037 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['cybersecurity_control_assessment_gap', 'api_integration_audit_log_gap', 'master_data_change_approval_gap', 'access_control_review_record_gap', 'backup_restore_test_record_gap', 'vulnerability_remediation_record_gap']:
        require(phrase in loop_round, f"loop round missing cybersecurity integration case: {phrase}")

    print(
        "was_real_source_record_monitor_037=pass "
        "submitted_real_candidate_files=0 cybersecurity_control_assessment_gaps=0 "
        "api_integration_audit_log_gaps=0 master_data_change_approval_gaps=0 "
        "access_control_review_record_gaps=0 backup_restore_test_record_gaps=0 "
        "vulnerability_remediation_record_gaps=0 accepted_for_cybersecurity_integration_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-038"
    )
    return 0
