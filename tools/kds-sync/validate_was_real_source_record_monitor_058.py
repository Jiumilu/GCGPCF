#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 058."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-058-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-058-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-058.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-057-20260622.json"
...
PACKAGING_CIRCULARITY_KEYS = [
    "packaging_specification_record_gaps",
    "recycled_content_declaration_gaps",
    "packaging_supplier_certificate_gaps",
    "packaging_waste_reduction_plan_gaps",
    "returnable_packaging_tracking_gaps",
    "packaging_recycling_evidence_gaps",
    "packaging_compliance_labeling_gaps",
    "accepted_for_packaging_circularity_profile"
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in PACKAGING_CIRCULARITY_KEYS:
    ...
    if value.get("packaging_circularity_state") != "blocked_no_real_source_record":
        failures.append("packaging_circularity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-058-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_058_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-058", "invalid round id")
    ...
    require(evidence.get("packaging_circularity_requirements") == REQUIREMENTS, "packaging circularity requirements mismatch")
    ...
    require(checks.get("packaging_circularity_state") == "blocked_no_real_source_record", "packaging circularity state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-058-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-058-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 058 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing packaging circularity case: {phrase}")

    print(
        "was_real_source_record_monitor_058=pass "
        "submitted_real_candidate_files=0 packaging_specification_record_gaps=0 recycled_content_declaration_gaps=0 "
        "packaging_supplier_certificate_gaps=0 packaging_waste_reduction_plan_gaps=0 returnable_packaging_tracking_gaps=0 "
        "packaging_recycling_evidence_gaps=0 packaging_compliance_labeling_gaps=0 "
        "accepted_for_packaging_circularity_profile=0 accepted_for_next_gate=0 "
