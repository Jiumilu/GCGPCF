#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 072."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-072-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-072-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-072.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-071-20260622.json"
...
BUSINESS_ETHICS_KEYS = [
    "business_ethics_policy_acknowledgement_gaps",
    "anti_bribery_corruption_due_diligence_gaps",
    "sanctions_export_control_screening_gaps",
    "conflict_of_interest_disclosure_gaps",
    "whistleblower_case_resolution_record_gaps",
    "responsible_sourcing_supplier_commitment_gaps",
    "third_party_audit_integrity_report_gaps",
    "accepted_for_business_ethics_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in BUSINESS_ETHICS_KEYS:
    ...
    if value.get("business_ethics_state") != "blocked_no_real_source_record":
        failures.append("business_ethics_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-072-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_072_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-072", "invalid round id")
    ...
    require(evidence.get("business_ethics_requirements") == REQUIREMENTS, "business ethics requirements mismatch")
    ...
    require(checks.get("business_ethics_state") == "blocked_no_real_source_record", "business ethics state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-072-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-072-negative-*.json"))
