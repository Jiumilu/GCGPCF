#!/usr/bin/env python3
"""Validate CodeGraph drift alert thresholds evidence."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/codegraph-drift-alert-thresholds-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/codegraph-drift-alert-thresholds-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-CODEGRAPH-DRIFT-ALERT-THRESHOLDS-008.md"
REGRESSION = ROOT / "docs/harness/evidence/codegraph-impact-regression-watch-20260621.json"
TARGET = "tools/kds-sync/validate_codegraph_impact_regression_watch.py"
...
def main() -> int:
    ...
    regression = json.loads(read(REGRESSION))
    evidence_md = read(EVIDENCE_MD)
    loop_round = read(LOOP_ROUND)

    require(evidence["evidence_id"] == "CODEGRAPH-DRIFT-ALERT-THRESHOLDS-20260621", "invalid evidence id")
    require(evidence["status"] == "drift_alert_thresholds_ready", "invalid status")
    require(evidence["scope"] == "GPCF-CODEGRAPH-DRIFT-ALERT-THRESHOLDS-008", "invalid scope")
    require(regression["status"] == "impact_regression_watch_active", "regression source mismatch")

    thresholds = evidence["thresholds"]
    require(thresholds["normal_repo"]["watch"] == "1 <= pending_total <= 5", "normal watch threshold mismatch")
    require(thresholds["normal_repo"]["action_required"] == "pending_total >= 6", "normal action threshold mismatch")
    require("explicit sync-only authorization" in thresholds["brain_or_studio_sync_only"]["required_authorization"], "sync authorization rule missing")
    ...
    sample = evidence["project_group_sample"]
    require(initialized == sample["initialized_repo_count"] == 14, "initialized repo count mismatch")
    require(git_entries == sample["codegraph_git_status_entries_total"] == 0, ".codegraph git entries must be zero")
    require(sample["git_protected_repo_count"] == 14, "git protected repo count mismatch")

    alerts = {item["repo"]: item for item in sample["alerts"]}
    gfis = alerts["GlobalCloud GFIS"]
    ...
    rg = run([
        "rg",
        "-n",
        "validate_codegraph_impact_regression_watch|CODEGRAPH-IMPACT-REGRESSION-WATCH-20260621|GPCF-CODEGRAPH-IMPACT-REGRESSION-WATCH-007|GPCF-CODEGRAPH-DRIFT-ALERT-THRESHOLDS-008",
        "docs",
        "tools",
        "governance",
        "harness",
