#!/usr/bin/env python3
"""Validate D94 scoped GCKF remaining heading localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d94-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d94-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D94-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_gckf_heading_repair_d94=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d94_evidence_or_loop")
    ...
    if len(changed_files) != 20:
        fail("changed_file_count_not_20")
    ...
    h1_findings = [
        finding
        for finding in gckf_findings
        if finding.get("line") == 19 and str(finding.get("detail", "")).startswith("# ")
    ]
    if h1_findings:
        fail("gckf_h1_findings_remaining")
    ...
    if delta.get("docs_gc_knowledge_fabric_h1_findings") != -20:
        fail("unexpected_h1_delta")
    after = evidence.get("after", {})
    if after.get("docs_gc_knowledge_fabric_h1_findings") != 0:
        fail("after_h1_not_zero")

    print("localization_debt_gckf_heading_repair_d94=pass")
