#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 021."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-021-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-021-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-021.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-020-20260621.json"
...
QUALITY_KEYS = [
    "quality_inspection_report_gaps",
    "batch_lot_traceability_gaps",
    "calibration_certificate_gaps",
    "nonconformance_record_gaps",
    "capa_closure_evidence_gaps",
    "release_authorization_gaps",
    "accepted_for_quality_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("quality_state") != "blocked_no_real_source_record":
        failures.append("quality_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-021-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_021_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-021", "invalid round id")
    ...
    require(evidence.get("quality_requirements") == [
    ...
    require(checks.get("quality_state") == "blocked_no_real_source_record", "quality state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-021-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-021-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 021 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing quality case: {phrase}")

    print(
        "was_real_source_record_monitor_021=pass "
        "submitted_real_candidate_files=0 quality_inspection_report_gaps=0 batch_lot_traceability_gaps=0 "
        "calibration_certificate_gaps=0 nonconformance_record_gaps=0 capa_closure_evidence_gaps=0 "
        "release_authorization_gaps=0 accepted_for_quality_profile=0 accepted_for_next_gate=0 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-022"
    )
    return 0
