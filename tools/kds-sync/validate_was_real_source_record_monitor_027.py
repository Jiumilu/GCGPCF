#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 027."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-027-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-027-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-027.md"
PREVIOUS_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-026-20260622.json"
...
CHEMICAL_COMPLIANCE_KEYS = [
    "sds_availability_gaps",
    "restricted_substance_declaration_gaps",
    "hazardous_material_storage_record_gaps",
    "chemical_batch_traceability_gaps",
    "hazardous_waste_manifest_gaps",
    "emergency_incident_response_record_gaps",
    "accepted_for_chemical_compliance_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CHEMICAL_COMPLIANCE_KEYS:
    ...
    if value.get("chemical_compliance_state") != "blocked_no_real_source_record":
        failures.append("chemical_compliance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-027-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_027_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-027", "invalid round id")
    ...
    require(evidence.get("chemical_compliance_requirements") == [
        "sds_availability",
    ...
    ], "chemical compliance requirements mismatch")
    ...
    require(checks.get("chemical_compliance_state") == "blocked_no_real_source_record", "chemical compliance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-027-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-027-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 027 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing chemical compliance case: {phrase}")

    print(
        "was_real_source_record_monitor_027=pass "
        "submitted_real_candidate_files=0 sds_availability_gaps=0 restricted_substance_declaration_gaps=0 "
        "hazardous_material_storage_record_gaps=0 chemical_batch_traceability_gaps=0 "
        "hazardous_waste_manifest_gaps=0 emergency_incident_response_record_gaps=0 "
        "accepted_for_chemical_compliance_profile=0 accepted_for_next_gate=0 hold_required=1 "
        ...
        "production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-028"
    )
    return 0
