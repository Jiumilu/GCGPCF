#!/usr/bin/env python3
"""Validate D115 scoped session-auth localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-session-auth-repair-d115-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-session-auth-repair-d115-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D115-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_session_auth_repair_d115=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d115_evidence_or_loop")
    ...
    if evidence.get("changed_line_items") != 18:
        fail("changed_line_items_not_18")
    ...
    print("localization_debt_session_auth_repair_d115=pass")
