#!/usr/bin/env python3
"""Validate current execution evidence for WAS/Ontology P4 candidate precheck."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-CANDIDATE-PRECHECK-EXECUTION-001.md"
INTAKE_DIR = ROOT / "docs/harness/intake"
TEMPLATE = INTAKE_DIR / "was-real-source-record-candidate-template.yaml"
POSITIVE = ROOT / "fixtures/was/p4-candidate-precheck-execution-positive-hold.json"
NEGATIVE = ROOT / "fixtures/was/p4-candidate-precheck-execution-negative-false-promotion.json"
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-CANDIDATE-PRECHECK-EXECUTION-20260622", "invalid evidence id")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-CANDIDATE-PRECHECK-EXECUTION-001", "invalid round id")
    require(evidence.get("executed_round") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-CANDIDATE-PRECHECK-001", "executed round mismatch")
    require(evidence.get("source_round") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-025", "source round mismatch")
    ...
    require("GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-026" in evidence_md, "next round missing from evidence md")
    require("loop_was_context" in loop_round, "loop round missing loop_was_context")

    command_outputs = run_required_commands()
    require(len(command_outputs) == evidence.get("command_gate", {}).get("expected_pass_count") == 5, "command pass count mismatch")
    ...
    print(
        "was_real_source_record_candidate_precheck_execution=pass "
