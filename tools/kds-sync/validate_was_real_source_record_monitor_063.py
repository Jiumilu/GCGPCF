#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 063."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-063-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-063-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-063.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-062-20260622.json"
...
END_OF_LIFE_KEYS = [
    "retirement_notice_gaps",
    "deinstallation_record_gaps",
    "data_sanitization_certificate_gaps",
    "hazardous_component_removal_record_gaps",
    "takeback_authorization_gaps",
    "refurbishment_or_reuse_disposition_gaps",
    "final_recycling_or_destruction_certificate_gaps",
    "accepted_for_end_of_life_disposition_profile",
]
REQUIREMENTS = [
    "retirement_notice",
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("end_of_life_disposition_state") != "blocked_no_real_source_record":
        failures.append("end_of_life_disposition_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-063-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_063_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-063", "invalid round id")
    ...
    require(evidence.get("end_of_life_disposition_requirements") == REQUIREMENTS, "end-of-life disposition requirements mismatch")
    ...
    require(checks.get("end_of_life_disposition_state") == "blocked_no_real_source_record", "end-of-life disposition state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-063-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-063-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 063 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing end-of-life disposition case: {phrase}")

    print(
        "was_real_source_record_monitor_063=pass "
        "submitted_real_candidate_files=0 retirement_notice_gaps=0 deinstallation_record_gaps=0 "
        "data_sanitization_certificate_gaps=0 hazardous_component_removal_record_gaps=0 "
        "takeback_authorization_gaps=0 refurbishment_or_reuse_disposition_gaps=0 "
        "final_recycling_or_destruction_certificate_gaps=0 "
        "accepted_for_end_of_life_disposition_profile=0 accepted_for_next_gate=0 "
