#!/usr/bin/env python3
"""Validate D179 GCKF P0 repair owner notification preview current-state evidence."""
...
FIXTURE = ROOT / "fixtures" / "api" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-repair-request-acknowledgement-routing-repair-owner-notification-preview-current-state-d179-20260622.json"
EVIDENCE_JSON = ROOT / "docs" / "harness" / "evidence" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-repair-request-acknowledgement-routing-repair-owner-notification-preview-current-state-d179-20260622.json"
EVIDENCE_MD = ROOT / "docs" / "harness" / "evidence" / "gckf-p0-formal-evidence-execution-committee-acceptance-acknowledgement-notification-receipt-aggregation-repair-request-acknowledgement-routing-repair-owner-notification-preview-current-state-d179-20260622.md"
LOOP_MD = ROOT / "docs" / "harness" / "loops" / "loop-round-GPCF-GCKF-P0-D179-001.md"


def fail(message: str) -> None:
    print(
        "gckf_p0_formal_evidence_execution_committee_acceptance_acknowledgement_notification_receipt_aggregation_repair_request_acknowledgement_routing_repair_owner_notification_preview_current_state_d179="
...
def main() -> None:
    ...
    historical = load_json(ROOT / fixture["sourceHistoricalRepairOwnerNotificationPreview"])
    current_source = load_json(ROOT / fixture["sourceCurrentRoutingDeliveryPrecheck"])
    preview = fixture["repairOwnerNotificationPreview"]
    historical_preview = historical["repairOwnerNotificationPreview"]
    source_precheck = current_source["routingDeliveryPrecheck"]

    require(
        fixture.get("repairOwnerNotificationPreviewStatus") == expected["repairOwnerNotificationPreviewStatus"],
    ...
    require(
        fixture.get("notFinalRepairOwnerNotification") is expected["notFinalRepairOwnerNotification"],
    ...
    require(
        historical.get("repairOwnerNotificationPreviewStatus") == "candidate_preview",
    ...
    require(
        historical_preview.get("repairOwnerNotificationExecutionStatus") == "not_executed",
        "historical_execution_status_mismatch",
    )

    require(
        current_source.get("routingDeliveryPrecheckStatus") == expected["sourceRoutingDeliveryPrecheckStatus"],
    ...
    for source_key, expected_key in (
        ...
        ("deliveryExecutionStatus", "sourceRoutingDeliveryExecutionStatus"),
    ...
    require(
        preview.get("sourceRoutingDeliveryPrecheckId") == source_precheck.get("id"),
        "source_precheck_id_mismatch",
    )
    require(
        fixture.get("coveredRoutingDeliveryPrecheckStatus") == expected["coveredRoutingDeliveryPrecheckStatus"],
        "covered_status_mismatch",
    )
    require(
        fixture["coveredRoutingDeliveryPrecheckStatus"] == source_precheck.get("precheckStatus"),
        "covered_status_not_matched",
    )

    roles = set(preview.get("notificationPreviewRoles", []))
    sections = set(preview.get("notificationPreviewSections", []))
    fields = set(preview.get("candidateNotificationFields", []))
    prerequisites = set(preview.get("notificationReadinessPrerequisites", []))
    constraints = set(preview.get("notificationDecisionConstraints", []))
    checks = set(preview.get("notificationPreviewChecks", []))
    refs = set(preview.get("requiredNotificationRefs", []))
    ...
    require(len(roles) == expected["notificationPreviewRoleCount"], "notification_role_count_mismatch")
    require(len(sections) == expected["notificationPreviewSectionCount"], "notification_section_count_mismatch")
    require(len(fields) == expected["candidateNotificationFieldCount"], "notification_field_count_mismatch")
    require(
        len(prerequisites) == expected["notificationReadinessPrerequisiteCount"],
        "notification_prerequisite_count_mismatch",
    )
    require(
        len(constraints) == expected["notificationDecisionConstraintCount"],
        "notification_constraint_count_mismatch",
    )
    require(len(checks) == expected["notificationPreviewCheckCount"], "notification_check_count_mismatch")
    require(len(refs) == expected["requiredNotificationRefCount"], "notification_ref_count_mismatch")
    ...
    require(
        evidence.get("current_repair_owner_notification_preview_status")
        == expected["repairOwnerNotificationPreviewStatus"],
    ...
    require(
        package_scope.get("notification_preview_roles") == expected["notificationPreviewRoleCount"],
        "evidence_roles_mismatch",
    )
    require(
        package_scope.get("notification_preview_sections") == expected["notificationPreviewSectionCount"],
        "evidence_sections_mismatch",
    )
    require(
        package_scope.get("candidate_notification_fields") == expected["candidateNotificationFieldCount"],
        "evidence_fields_mismatch",
    )
    require(
        package_scope.get("notification_readiness_prerequisites")
        == expected["notificationReadinessPrerequisiteCount"],
        "evidence_prerequisites_mismatch",
    )
    require(
        package_scope.get("notification_decision_constraints")
        == expected["notificationDecisionConstraintCount"],
        "evidence_constraints_mismatch",
    )
    require(
        package_scope.get("notification_preview_checks") == expected["notificationPreviewCheckCount"],
        "evidence_checks_mismatch",
    )
    require(
        package_scope.get("required_notification_refs") == expected["requiredNotificationRefCount"],
    ...
    print(
        "gckf_p0_formal_evidence_execution_committee_acceptance_acknowledgement_notification_receipt_aggregation_repair_request_acknowledgement_routing_repair_owner_notification_preview_current_state_d179=pass"
    )
    print(f"repair_owner_notification_preview_status={fixture['repairOwnerNotificationPreviewStatus']}")
    ...
    print(f"notification_preview_roles={len(roles)}")
    print(f"notification_preview_checks={len(checks)}")
