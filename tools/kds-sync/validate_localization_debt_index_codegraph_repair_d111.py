#!/usr/bin/env python3
"""Validate D111 scoped index/codegraph localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-index-codegraph-repair-d111-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-index-codegraph-repair-d111-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D111-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_index_codegraph_repair_d111=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d111_evidence_or_loop")
    ...
    if len(target_findings) != 3:
        fail("unexpected_target_finding_count")
    for remaining in target_findings:
        if remaining.get("path") != "docs/harness/evidence/evidence-index.md":
            fail("unexpected_remaining_path")
        if remaining.get("kind") != "doc_english_line":
            fail("unexpected_remaining_kind")

    delta = evidence.get("delta", {})
    if delta.get("target_file_findings") != -1:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != 2:
        fail("unexpected_all_repo_delta")
    after = evidence.get("after", {})
    if after.get("target_file_findings") != 3:
        fail("after_target_not_three")
    if current.get("findings", 0) != after.get("all_repo_findings", 0):
        fail("after_all_repo_mismatch")

    print("localization_debt_index_codegraph_repair_d111=pass")
