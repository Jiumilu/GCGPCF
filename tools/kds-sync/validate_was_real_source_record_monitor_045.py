#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 045."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-045-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-045-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-045.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-044-20260622.json"
...
RISK_KEYS = [
    ...
    "logistics_disruption_gaps",
    "sanction_export_compliance_gaps",
    "cyber_data_incident_gaps",
    "emergency_response_plan_gaps",
    "insurance_claim_gaps",
    "business_continuity_review_gaps",
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-045-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_045_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-045", "invalid round id")
    ...
    require(evidence.get("risk_resilience_requirements") == REQUIREMENTS, "risk resilience requirements mismatch")
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-045-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-045-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 045 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
    ...
    print(
        "was_real_source_record_monitor_045=pass "
        "submitted_real_candidate_files=0 supply_disruption_gaps=0 "
        "logistics_disruption_gaps=0 sanction_export_compliance_gaps=0 "
        "cyber_data_incident_gaps=0 emergency_response_plan_gaps=0 "
        "insurance_claim_gaps=0 business_continuity_review_gaps=0 "
        "accepted_for_risk_resilience_profile=0 accepted_for_next_gate=0 hold_required=1 "
