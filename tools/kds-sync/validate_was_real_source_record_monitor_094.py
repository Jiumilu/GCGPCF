#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 094."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-094-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-094-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-094.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-093-20260622.json"
...
MONITOR_KEYS = [
    "third_party_auditor_independence_statement_gaps",
    "audit_scope_approval_record_gaps",
    "evidence_chain_of_custody_register_gaps",
    "audit_workpaper_index_gaps",
    "management_representation_letter_gaps",
    "finding_remediation_tracking_log_gaps",
    "assurance_report_release_approval_gaps",
    "accepted_for_third_party_assurance_profile",
]
REQUIREMENTS = [
...
LOOP_CASES = [
    "third_party_auditor_independence_statement_gap",
    "audit_scope_approval_record_gap",
    "evidence_chain_of_custody_register_gap",
    "audit_workpaper_index_gap",
    "management_representation_letter_gap",
    "finding_remediation_tracking_log_gap",
    "assurance_report_release_approval_gap",
]
NEGATIVE_FIXTURES = [
    "real-source-record-monitor-094-negative-third_party_auditor_independence_statement_gap.json",
    "real-source-record-monitor-094-negative-audit_scope_approval_record_gap.json",
    "real-source-record-monitor-094-negative-evidence_chain_of_custody_register_gap.json",
    "real-source-record-monitor-094-negative-audit_workpaper_index_gap.json",
    "real-source-record-monitor-094-negative-management_representation_letter_gap.json",
    "real-source-record-monitor-094-negative-finding_remediation_tracking_log_gap.json",
    "real-source-record-monitor-094-negative-assurance_report_release_approval_gap.json",
]
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("third_party_assurance_state") != "blocked_no_real_source_record":
        failures.append("third_party_assurance_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-094-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_094_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-094", "invalid round id")
    ...
    require(evidence.get("third_party_assurance_requirements") == REQUIREMENTS, "third-party assurance requirements mismatch")
    ...
    require(checks.get("third_party_assurance_state") == "blocked_no_real_source_record", "third-party assurance state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-094-positive.json")
    ...
    require("真实 P4 输入 monitor 094 已建立" in loop_round, "loop round debug missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing third-party assurance case: {phrase}")
    ...
    print(
        "was_real_source_record_monitor_094=pass submitted_real_candidate_files=0 "
        "third_party_auditor_independence_statement_gaps=0 audit_scope_approval_record_gaps=0 "
        "evidence_chain_of_custody_register_gaps=0 audit_workpaper_index_gaps=0 "
        "management_representation_letter_gaps=0 finding_remediation_tracking_log_gaps=0 "
        "assurance_report_release_approval_gaps=0 accepted_for_third_party_assurance_profile=0 "
