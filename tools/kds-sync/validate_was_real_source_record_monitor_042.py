#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 042."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-042-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-042-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-042.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-041-20260622.json"
...
CARBON_CIRCULARITY_KEYS = [
    "product_carbon_footprint_gaps",
    "energy_consumption_evidence_gaps",
    "emissions_factor_source_gaps",
    "environmental_compliance_certificate_gaps",
    "hazardous_substance_declaration_gaps",
    "recycling_end_of_life_plan_gaps",
    "green_claim_substantiation_gaps",
    "accepted_for_carbon_circularity_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CARBON_CIRCULARITY_KEYS:
    ...
    if value.get("carbon_circularity_state") != "blocked_no_real_source_record":
        failures.append("carbon_circularity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-042-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_042_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-042", "invalid round id")
    ...
    require(evidence.get("carbon_circularity_requirements") == [
    ...
    ], "carbon circularity requirements mismatch")
    ...
    require(checks.get("carbon_circularity_state") == "blocked_no_real_source_record", "carbon circularity state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-042-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-042-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 042 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing carbon circularity case: {phrase}")

    print(
        "was_real_source_record_monitor_042=pass "
        "submitted_real_candidate_files=0 product_carbon_footprint_gaps=0 "
        "energy_consumption_evidence_gaps=0 emissions_factor_source_gaps=0 "
        "environmental_compliance_certificate_gaps=0 hazardous_substance_declaration_gaps=0 "
        "recycling_end_of_life_plan_gaps=0 green_claim_substantiation_gaps=0 "
        "accepted_for_carbon_circularity_profile=0 accepted_for_next_gate=0 hold_required=1 "
