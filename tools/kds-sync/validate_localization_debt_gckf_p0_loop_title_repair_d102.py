#!/usr/bin/env python3
"""Validate D102 scoped GCKF P0 Loop title localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-gckf-p0-loop-title-repair-d102-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-gckf-p0-loop-title-repair-d102-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D102-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_gckf_p0_loop_title_repair_d102=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    ...
    if len(changed_files) != 18:
        fail("changed_file_count_not_18")
    if evidence.get("changed_line_items") != 72:
        fail("changed_line_items_not_72")
    ...
    for source_path in changed_files:
        ...
        if not title or not has_chinese(title):
            fail(f"title_not_chinese:{source_path}")
    ...
    if delta.get("target_file_findings") != -18:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -19:
    ...
    print("localization_debt_gckf_p0_loop_title_repair_d102=pass")
