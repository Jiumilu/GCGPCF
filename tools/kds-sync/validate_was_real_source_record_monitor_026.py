#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 026."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-026-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-026-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-026.md"
PREVIOUS_JSON = ROOT / "docs/harness/evidence/was-real-source-record-candidate-precheck-execution-20260622.json"
...
ENERGY_CARBON_KEYS = [
    "energy_meter_record_gaps",
    "scope1_fuel_record_gaps",
    "scope2_electricity_invoice_gaps",
    "scope3_transport_material_record_gaps",
    "renewable_certificate_gaps",
    "emission_factor_method_gaps",
    "accepted_for_energy_carbon_profile",
]
REQUIRED_COMMANDS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("energy_carbon_state") != "blocked_no_real_source_record":
        failures.append("energy_carbon_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-026-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_026_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-026", "invalid round id")
    ...
    require(evidence.get("energy_carbon_requirements") == [
        "energy_meter_record",
        "scope1_fuel_record",
    ...
    ], "energy carbon requirements mismatch")
    ...
    require(checks.get("energy_carbon_state") == "blocked_no_real_source_record", "energy carbon state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-026-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-026-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 026 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing energy carbon case: {phrase}")

    print(
        "was_real_source_record_monitor_026=pass "
        "submitted_real_candidate_files=0 energy_meter_record_gaps=0 scope1_fuel_record_gaps=0 "
        "scope2_electricity_invoice_gaps=0 scope3_transport_material_record_gaps=0 "
        "renewable_certificate_gaps=0 emission_factor_method_gaps=0 accepted_for_energy_carbon_profile=0 "
        ...
        "accepted=false integrated=false production_ready=false next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-027"
    )
    return 0
