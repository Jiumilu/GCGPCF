#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 070."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-070-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-070-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-070.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-069-20260622.json"
...
BIODIVERSITY_KEYS = [
    "biodiversity_baseline_survey_gaps",
    "land_use_compliance_permit_gaps",
    "protected_area_screening_record_gaps",
    "ecological_impact_assessment_gaps",
    "habitat_restoration_plan_gaps",
    "stakeholder_consultation_record_gaps",
    "nature_positive_kpi_monitoring_gaps",
    "accepted_for_biodiversity_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("biodiversity_state") != "blocked_no_real_source_record":
        failures.append("biodiversity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-070-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_070_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-070", "invalid round id")
    ...
    require(evidence.get("biodiversity_requirements") == REQUIREMENTS, "biodiversity requirements mismatch")
    ...
    require(checks.get("biodiversity_state") == "blocked_no_real_source_record", "biodiversity state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-070-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-070-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 070 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing biodiversity case: {phrase}")

    print(
        "was_real_source_record_monitor_070=pass "
        "submitted_real_candidate_files=0 biodiversity_baseline_survey_gaps=0 land_use_compliance_permit_gaps=0 "
        "protected_area_screening_record_gaps=0 ecological_impact_assessment_gaps=0 "
        "habitat_restoration_plan_gaps=0 stakeholder_consultation_record_gaps=0 "
        "nature_positive_kpi_monitoring_gaps=0 "
        "accepted_for_biodiversity_profile=0 accepted_for_next_gate=0 "
