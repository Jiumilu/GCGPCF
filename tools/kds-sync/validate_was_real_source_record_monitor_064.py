#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 064."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-064-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-064-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-064.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-063-20260622.json"
...
CIRCULARITY_KEYS = [
    ...
    "component_harvest_record_gaps",
    "remanufacturing_route_decision_gaps",
    "secondary_market_transfer_record_gaps",
    "disposal_deviation_report_gaps",
    "regulatory_recycling_report_gaps",
    "circularity_kpi_assurance_statement_gaps",
    "accepted_for_post_disposition_circularity_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("post_disposition_circularity_state") != "blocked_no_real_source_record":
        failures.append("post_disposition_circularity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-064-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_064_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-064", "invalid round id")
    ...
    require(evidence.get("post_disposition_circularity_requirements") == REQUIREMENTS, "post-disposition circularity requirements mismatch")
    ...
    require(checks.get("post_disposition_circularity_state") == "blocked_no_real_source_record", "post-disposition circularity state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-064-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-064-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 064 已建立" in loop_round, "loop round feedback missing")
    for phrase in LOOP_CASES:
        require(phrase in loop_round, f"loop round missing post-disposition circularity case: {phrase}")

    print(
        "was_real_source_record_monitor_064=pass "
        "submitted_real_candidate_files=0 material_recovery_accounting_gaps=0 component_harvest_record_gaps=0 "
        "remanufacturing_route_decision_gaps=0 secondary_market_transfer_record_gaps=0 "
        "disposal_deviation_report_gaps=0 regulatory_recycling_report_gaps=0 "
        "circularity_kpi_assurance_statement_gaps=0 "
        "accepted_for_post_disposition_circularity_profile=0 accepted_for_next_gate=0 "
