#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 035."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-035-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-035-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-035.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-034-20260622.json"
...
DESIGN_CIRCULARITY_KEYS = ['design_specification_record_gaps', 'bill_of_material_revision_gaps', 'engineering_change_order_gaps', 'material_substitution_approval_gaps', 'life_cycle_assessment_record_gaps', 'recyclability_or_reuse_declaration_gaps', 'accepted_for_design_circularity_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in DESIGN_CIRCULARITY_KEYS:
    ...
    if value.get("design_circularity_state") != "blocked_no_real_source_record":
        failures.append("design_circularity_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-035-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_035_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-035", "invalid round id")
    ...
    require(evidence.get("design_circularity_requirements") == ['design_specification_record', 'bill_of_material_revision', 'engineering_change_order', 'material_substitution_approval', 'life_cycle_assessment_record', 'recyclability_or_reuse_declaration'], "design circularity requirements mismatch")
    ...
    require(checks.get("design_circularity_state") == "blocked_no_real_source_record", "design circularity state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-035-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-035-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 035 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['design_specification_record_gap', 'bill_of_material_revision_gap', 'engineering_change_order_gap', 'material_substitution_approval_gap', 'life_cycle_assessment_record_gap', 'recyclability_or_reuse_declaration_gap']:
        require(phrase in loop_round, f"loop round missing design circularity case: {phrase}")

    print(
        "was_real_source_record_monitor_035=pass "
        "submitted_real_candidate_files=0 design_specification_record_gaps=0 "
        "bill_of_material_revision_gaps=0 engineering_change_order_gaps=0 "
        "material_substitution_approval_gaps=0 life_cycle_assessment_record_gaps=0 "
        "recyclability_or_reuse_declaration_gaps=0 accepted_for_design_circularity_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-036"
    )
    return 0
