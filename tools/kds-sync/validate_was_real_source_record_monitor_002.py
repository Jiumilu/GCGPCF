#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 002."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-002-20260621.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-002-20260621.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-002.md"
PREVIOUS_MONITOR_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-20260621.json"
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    if value.get("llm_role") == "source_of_record":
        failures.append("llm_must_not_be_source_of_record")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-002-20260621", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_002_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-002", "invalid round id")
    ...
    required_command_strings = [" ".join(command) for command in REQUIRED_COMMANDS]
    require(evidence.get("required_commands") == required_command_strings, "required command list mismatch")
    command_outputs = [run(command) for command in REQUIRED_COMMANDS]
    ...
    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-002-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-002-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 002 已建立" in loop_round, "loop round feedback missing")

    print(
        "was_real_source_record_monitor_002=pass "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-003"
    )
    return 0
