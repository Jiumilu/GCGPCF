#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 049."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-049-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-049-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-049.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-048-20260622.json"
...
CUSTOMER_CHANGE_CAPA_KEYS = [
    "customer_change_request_gaps",
    "deviation_waiver_request_gaps",
    "concession_release_approval_gaps",
    "nonconformance_report_gaps",
    "root_cause_analysis_gaps",
    "corrective_preventive_action_gaps",
    "effectiveness_verification_gaps",
    "accepted_for_customer_change_capa_profile",
]
REQUIREMENTS = [
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in CUSTOMER_CHANGE_CAPA_KEYS:
    ...
    if value.get("customer_change_capa_state") != "blocked_no_real_source_record":
        failures.append("customer_change_capa_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-049-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_049_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-049", "invalid round id")
    ...
    require(evidence.get("customer_change_capa_requirements") == REQUIREMENTS, "customer change CAPA requirements mismatch")
    ...
    require(checks.get("customer_change_capa_state") == "blocked_no_real_source_record", "customer change CAPA state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-049-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-049-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 049 已建立" in loop_round, "loop round feedback missing")
    for phrase in [
        ...
        require(phrase in loop_round, f"loop round missing customer change CAPA case: {phrase}")

    print(
        "was_real_source_record_monitor_049=pass "
        "submitted_real_candidate_files=0 customer_change_request_gaps=0 "
        "deviation_waiver_request_gaps=0 concession_release_approval_gaps=0 "
        "nonconformance_report_gaps=0 root_cause_analysis_gaps=0 "
        "corrective_preventive_action_gaps=0 effectiveness_verification_gaps=0 "
        "accepted_for_customer_change_capa_profile=0 accepted_for_next_gate=0 hold_required=1 "
