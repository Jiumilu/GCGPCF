#!/usr/bin/env python3
"""Validate WAS real source-record monitor round 036."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-036-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/was-real-source-record-monitor-036-20260622.md"
LOOP_ROUND = ROOT / "docs/harness/loops/loop-round-GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-036.md"
SOURCE_JSON = ROOT / "docs/harness/evidence/was-real-source-record-monitor-035-20260622.json"
...
DIGITAL_TRACEABILITY_KEYS = ['iot_sensor_calibration_record_gaps', 'machine_data_capture_record_gaps', 'digital_traceability_event_log_gaps', 'evidence_hash_manifest_gaps', 'model_or_algorithm_version_record_gaps', 'data_retention_policy_record_gaps', 'accepted_for_digital_traceability_profile']
...
def monitor_failures(value: dict[str, Any]) -> list[str]:
    ...
    for key in DIGITAL_TRACEABILITY_KEYS:
    ...
    if value.get("digital_traceability_state") != "blocked_no_real_source_record":
        failures.append("digital_traceability_state_must_remain_blocked")
...
def main() -> int:
    ...
    require(evidence.get("evidence_id") == "WAS-REAL-SOURCE-RECORD-MONITOR-036-20260622", "invalid evidence id")
    require(evidence.get("status") == "was_real_source_record_monitor_036_pass_with_hold", "invalid status")
    require(evidence.get("round_id") == "GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-036", "invalid round id")
    ...
    require(evidence.get("digital_traceability_requirements") == ['iot_sensor_calibration_record', 'machine_data_capture_record', 'digital_traceability_event_log', 'evidence_hash_manifest', 'model_or_algorithm_version_record', 'data_retention_policy_record'], "digital traceability requirements mismatch")
    ...
    require(checks.get("digital_traceability_state") == "blocked_no_real_source_record", "digital traceability state mismatch")

    positive = load_json(FIXTURE_DIR / "real-source-record-monitor-036-positive.json")
    require(not monitor_failures(positive), "positive fixture should pass")
    negatives = sorted(FIXTURE_DIR.glob("real-source-record-monitor-036-negative-*.json"))
    ...
    require("真实 P4 输入 monitor 036 已建立" in loop_round, "loop round feedback missing")
    for phrase in ['iot_sensor_calibration_record_gap', 'machine_data_capture_record_gap', 'digital_traceability_event_log_gap', 'evidence_hash_manifest_gap', 'model_or_algorithm_version_record_gap', 'data_retention_policy_record_gap']:
        require(phrase in loop_round, f"loop round missing digital traceability case: {phrase}")

    print(
        "was_real_source_record_monitor_036=pass "
        "submitted_real_candidate_files=0 iot_sensor_calibration_record_gaps=0 "
        "machine_data_capture_record_gaps=0 digital_traceability_event_log_gaps=0 "
        "evidence_hash_manifest_gaps=0 model_or_algorithm_version_record_gaps=0 "
        "data_retention_policy_record_gaps=0 accepted_for_digital_traceability_profile=0 "
        ...
        "next_round=GPCF-ONTOLOGY-WAS-REAL-SOURCE-RECORD-MONITOR-037"
    )
    return 0
