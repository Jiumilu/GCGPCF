#!/usr/bin/env python3
"""Validate D101 scoped Loop governance evidence localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-loop-governance-evidence-repair-d101-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-loop-governance-evidence-repair-d101-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D101-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_loop_governance_evidence_repair_d101=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    ...
    if len(changed_files) != 3:
        fail("changed_file_count_not_3")
    if evidence.get("changed_line_items") != 126:
        fail("changed_line_items_not_126")
    ...
    if delta.get("target_file_findings") != -9:
        fail("unexpected_target_delta")
    if delta.get("all_repo_findings") != -3:
    ...
    print("localization_debt_loop_governance_evidence_repair_d101=pass")
