#!/usr/bin/env python3
"""Validate D93 scoped GCKF heading localization repair."""
...
EVIDENCE_JSON = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d93-20260622.json"
EVIDENCE_MD = ROOT / "docs/harness/evidence/localization-debt-gckf-heading-repair-d93-20260622.md"
LOOP_MD = ROOT / "docs/harness/loops/loop-round-GPCF-GCKF-P0-D93-001.md"


def fail(message: str) -> None:
    print(f"localization_debt_gckf_heading_repair_d93=fail reason={message}")
    sys.exit(1)
...
def main() -> None:
    if not EVIDENCE_JSON.exists() or not EVIDENCE_MD.exists() or not LOOP_MD.exists():
        fail("missing_d93_evidence_or_loop")
    ...
    if len(changed_files) != 16:
        fail("changed_file_count_not_16")
    ...
    if delta.get("docs_gc_knowledge_fabric_h1_findings") != -16:
        fail("unexpected_h1_delta")
    if delta.get("docs_gc_knowledge_fabric_findings") != -16:
        fail("unexpected_gckf_delta")

    print("localization_debt_gckf_heading_repair_d93=pass")
