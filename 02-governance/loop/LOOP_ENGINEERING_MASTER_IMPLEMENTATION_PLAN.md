...
| 输出克制 | 必须遵守 `DO NOT send optional commentary`，只输出必要结论、阻塞项、确认请求、执行结果、验证证据和下一步必要动作 |
...
所有 LOOP 会话和跨会话 handoff 必须同时遵守输出克制规则：

```text
DO NOT send optional commentary
```

输出只允许承载必要结论、阻塞项、授权确认请求、执行结果、验证证据和下一步必要动作。可选过程性说明、无验证支撑的铺垫、非必要进度叙述、情绪化说明或用 commentary 替代确认边界的表达，均不得进入 LOOP 正式运行输出。
...
| 输出克制 | 每轮输出必须声明或等价遵守 `DO NOT send optional commentary`，不得发送 optional commentary |
...
## 6.1 GPCF 2.0 Feature 交付中心

GPCF 2.0 把项目群执行入口从项目治理中心升级为 Feature 交付中心：
...
GPCF 2.0 的最小交付单元是 `Feature`。新开发必须从 `features/active/F-xxx/` 进入，每个 Feature 只保留：
...
每个项目只维护 `projects/<project>/ROADMAP.md`、`STATUS.md`、`RISK.md`。旧 progress、review、task、status、log、notes、decision 等过程文件先进入降级归档清单，不直接删除历史证据。
