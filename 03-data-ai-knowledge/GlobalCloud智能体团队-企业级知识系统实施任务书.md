...
日期：2026-06-08  
适用范围：GlobalCloud 绿色供应链体系中的企业级知识主存域、LLM Wiki、GBrain、Brain、WAES 知识治理域、XiaoC/Hermes/XGD 知识消费域。  
...
6. 让 `WAES` 成为知识正文主账。
7. 让 AI 直接修改正式知识版本。
8. 同时把 `LLM Wiki` 和 `GBrain` 都定义成主知识引擎而不区分主次。
...
当前企业级知识系统实施的正确方向不是“在 `LLM Wiki` 和 `GBrain` 中二选一直接充当企业级知识库”，而是：
