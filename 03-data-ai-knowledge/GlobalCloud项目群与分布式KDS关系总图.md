...
这里的分布式 KDS 不是单一数据库，也不是单一知识库页面，而是以 KDS 11 个底座资源池为事实基础数据底座，以增强治理账本承载积分、收益、额度、悬赏、争议、潜在产值、SOP 和贡献等治理对象，以 GPCF 为治理与 Loop 入口，以 WAES 为规则和证据门禁，以 GFIS、GPC、PVAOS 等业务系统为事实来源和业务确认载体，以 Brain、PKC、XiaoC / 小即、XGD、XiaoG 等为知识编制、个人贡献、AI 服务和任务入口的项目群级知识事实操作系统。
...
flowchart LR
  ...
  GFIS["GFIS<br/>工厂 / 订单 / 生产 / 质量 / POD / 金融凭证事实主账"] --> KDS
...
| GFIS | 工厂、订单、生产、质量、发货、POD、金融凭证业务事实主账 | 需求、订单、生产、质检、发货、签收、开票、到账 | 业务事实、业务状态、责任拆分、运行证据 | 不把 AI 建议直接执行为业务写入 |
...
GFIS、GPC、PVAOS 等业务系统按自身流程确认业务事实。KDS 记录事实索引、证据索引和确认状态，不替代业务系统本身。
