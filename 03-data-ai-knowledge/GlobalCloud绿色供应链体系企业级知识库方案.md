...
当前 `LLM Wiki` 与 `GBrain` 都可以承担知识整理、索引、检索、RAG 和 Agent 消费能力，但两者都还不适合直接充当 GlobalCloud 绿色供应链体系的企业级知识库真源。

因此本方案的核心结论是：

1. 企业级知识库必须单独定义为 **知识主存层**。
2. `LLM Wiki` 与 `GBrain` 应归入 **知识引擎层** 或 **知识编制/检索层**。
3. `WAES` 负责知识准入、证据分级、引用审计、AI 授权和知识发布治理。
4. 测试结束后可以对 `LLM Wiki` 与 `GBrain` 做唯一性选择，但这个唯一性应优先发生在知识引擎层，而不是企业知识真源层。

## 2. 为什么当前两者都还不是企业级知识库

### 2.1 当前能力更像知识工作台

`LLM Wiki` 与 `GBrain` 当前更擅长：
...
| 层 | 主责 | 主组件 | 不做什么 |
|---|---|---|---|
| 业务事实层 | 产生业务事实 | PVAOS / GPC-Native / GFIS / Edge | 不承载知识真源 |
...
| 知识引擎层 | 切片、索引、RAG、图谱、引用回指 | LLM Wiki / GBrain / Brain | 不应反写主存层 |
...
1. 定义企业级知识对象模型。
2. 定义知识主存层与知识引擎层的边界。
3. 为 `LLM Wiki` 和 `GBrain` 建立统一 ingest 元数据。
4. 建立知识发布链和审计链。
5. 建立 `WAES` 的知识准入与授权门。
6. 完成双轨测试的评估矩阵。
...
当前最稳妥的方向不是在 `LLM Wiki` 和 `GBrain` 中直接挑一个充当企业级知识库，而是：

1. 先建立企业级知识主存层。
2. 再让 `LLM Wiki` 和 `GBrain` 在其之上承担知识编制、索引和检索职责。
3. 测试结束后再决定知识引擎层的唯一性。
