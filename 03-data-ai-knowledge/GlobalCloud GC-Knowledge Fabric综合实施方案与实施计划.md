...
用户建议的首批 8 份文档应作为 P0 文档包优先产出。

| 顺序 | 文件 | 目标 | 验收 |
|---|---|---|---|
| 1 | KDS 十一池挂接规则 | 固化内容类型到业务事实底座的挂接关系 | 每类内容有默认池、例外池和责任边界 |
| 2 | WAES 门禁规则 | 固化 Gate 类型、输入、输出、状态和 hard-stop | 每类对象有门禁路径 |
| 3 | RAG 准入规则 | 固化 T0-T5 与 RAG 准入关系 | T5 默认 blocked，敏感资料 metadata-only |
| 4 | 葛化 GFIS 知识库目录 | 固化葛化七类资料包和 GFIS 三件套目录 | 有编号、来源、密级、缺口 |
| 5 | 湖北磷材知识库目录 | 固化拓厂、原料/行业/订单、复制模板目录 | 不依赖 GFIS 深度即可运行 |
| 6 | 积分/收益/额度/悬赏台账模型 | 固化四类治理账本 | 候选与确认状态分离 |
| 7 | 敏感资料入库规则 | 固化 metadata-only、受控原件、哈希、摘要 | 金融、POD、质量争议、合同不开放原文 |
| 8 | LOOP 跟踪模板 | 固化每轮推进记录结构 | 可追踪输入、输出、门禁、风险、下一轮 |
...
03-data-ai-knowledge/GC-Knowledge-Fabric-RAG准入规则.md
...
| 周期 | 工作重点 | 产物 | 状态上限 |
|---|---|---|---|
| 第 1 周 | P0 8 份文档包 | 受控文档草案 + LOOP 记录 | `draft` / `controlled_analysis` |
...
1. 以本文档作为总底稿，拆出 P0 首批 8 份受控文档。
2. 为 8 份文档补齐 frontmatter、编号、KDS 路径和状态边界。
