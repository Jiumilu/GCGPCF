...
适用范围：GPC-Native、GFIS、WAES、PVAOS、Brain、XiaoC、Hermes/XGD、Edge 与连接器/事件总线；连接器必须落在 [总架构] §4 定义的 5 个根连接器编号下。

## 0. 口径修正（与 [总架构] 一致）

1. **GPC-Native** 是绿色供应链公共服务平台本体，事件中以 `gpc.` 域名承载平台公共服务事实。
...
3. **主责系统唯一**：GFIS 负责工厂事实；GPC-Native 负责平台公共服务事实；WAES 负责治理规则、证据、指标、状态审计和建议状态。
4. **WAES 不承办具体事务审批**：工单、质量、库存、发货、签收等事务审批在 GFIS 或 GPC-Native 内完成；WAES 只记录引用、治理确认和证据确证。
...
7. **平台事件优先围绕 GPC-Native**：绿色供应链公共服务平台事实事件默认由 `GPC-Native` 主责发布，`WAES` 不发布平台业务事实事件。
...
|---|---|---|---|---|---|---|---|
| `gfis.factory_order.accepted` | FactoryOrder | L2 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 工厂订单确认 |
...
| `gfis.quality_inspection.accepted` | QualityInspection | L3 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 质量合格 |
| `gfis.quality_inspection.rejected` | QualityInspection | L3 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 质量不合格 |
| `gfis.inventory_transaction.posted` | InventoryTransaction | L3 | GFIS | WAES | CON-GFIS-WAES-001 | 否 | 库存事务入账 |
| `gfis.lot.status_changed` | MaterialLot | L3 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 批次质量/可用状态变化 |
| `gfis.factory_shipment.released` | FactoryShipmentRelease | L3 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 工厂发货出库 |
...
## 5. GPC-Native 平台公共服务事件

| 事件 | 来源对象 | 风险等级 | 生产者 | 消费者 | 承载连接器 | AI 可发布 | 说明 |
|---|---|---|---|---|---|---|---|
| `gpc.platform_order.received` | PlatformOrder | L2 | GPC-Native | WAES | CON-GPC-WAES-001 | 否 | 平台订单接收 |
| `gpc.platform_order.dispatched_to_factory` | OrderMapping | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 订单分发到工厂 |
| `gpc.asn.created` | ASN | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 供应商 ASN |
| `gpc.appointment.confirmed` | Appointment | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 预约确认 |
| `gpc.vehicle.arrived` | Appointment / Vehicle | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 车辆到达 |
| `gpc.shipment.created` | Shipment | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 发运单创建 |
| `gpc.shipment.loaded` | Shipment | L3 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 装车完成 |
| `gpc.shipment.in_transit` | Shipment | L2 | GPC-Native | WAES | CON-GPC-WAES-001 | 否 | 在途 |
| `gpc.shipment.delayed` | Shipment | L2 | GPC-Native | WAES / Agent | CON-GPC-WAES-001 | 否 | 运输延迟 |
| `gpc.pod.signed` | ProofOfDelivery | L3 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 客户签收 |
| `gpc.pod.disputed` | ProofOfDelivery | L3 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 签收争议 |
| `gpc.external_exception.raised` | ExternalException | L2 | GPC-Native | WAES / Agent | CON-GPC-WAES-001 | 可生成草案，不自动确证 | 外部协同异常 |
| `gpc.external_exception.closed` | ExternalException | L3 | GPC-Native | WAES | CON-GPC-WAES-001 | 否 | 外部异常关闭 |
...
| `waes.business_approval.referenced` | BusinessApprovalReference | L2/L3 | GFIS / GPC-Native | WAES | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 引用业务系统内部事务审批结果，WAES 不承办该审批 |
| `waes.metric.calculated` | MetricSnapshot | L1 | WAES | ControlTower | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 指标计算 |
| `waes.exception.linked` | ExceptionCase | L2 | WAES | GFIS / GPC-Native / Agent | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 异常关联 |
...
| 事件 | 来源对象 | 风险等级 | 生产者 | 消费者 | 承载连接器 | AI 可发布 | 说明 |
|---|---|---|---|---|---|---|---|
| `pvaos.tenant.created` | Tenant | L2 | PVAOS | WAES / GPC-Native | CON-GPC-WAES-001 | 否 | 租户创建 |
| `pvaos.organization.onboarded` | Organization | L2 | PVAOS | GPC-Native / WAES | CON-GPC-WAES-001 | 否 | 组织接入 |
| `pvaos.partner.onboarded` | Partner | L2 | PVAOS | GPC-Native / WAES | CON-GPC-WAES-001 | 否 | 伙伴接入 |
| `pvaos.role_grant.changed` | RoleGrant | L3 | PVAOS | WAES / GPC-Native | CON-GPC-WAES-001 | 否 | 角色授权变化 |
...
| `ai.suggestion.governance_authorized` | AISuggestion | L3/L4 | WAES | GFIS / GPC-Native | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | AI 建议通过治理授权；具体业务执行仍由 GFIS/GPC-Native 内部流程确认，不表示业务批准 |
...
本节根据四流综合架构补充事件合同，优先服务治理规则生命周期、连接器治理、数据治理、AI 服务审计和多厂协同。以下事件均不得被解释为 GFIS/GPC-Native 具体事务审批。
...
| `waes.governance_rule.activated` | GovernanceRuleVersion | L4 | WAES | GFIS / GPC-Native / Agent | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 治理规则版本生效 |
| `waes.governance_rule.retired` | GovernanceRuleVersion | L4 | WAES | GFIS / GPC-Native / Agent | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 治理规则版本废止 |
...
| 事件 | 来源对象 | 风险等级 | 生产者 | 消费者 | 承载连接器 | AI 可发布 | 说明 |
|---|---|---|---|---|---|---|---|
| `gpc.factory_allocation.proposed` | FactoryAllocation | L2 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 多厂分单建议结果 |
| `gpc.factory_allocation.confirmed` | FactoryAllocation | L3 | GPC-Native | GFIS / WAES | CON-GPC-GFIS-001 / CON-GPC-WAES-001 | 否 | 分单确认；GFIS 仍需内部接单确认 |
| `gpc.cross_factory_exception.raised` | CrossFactoryException | L2 | GPC-Native | WAES / GFIS | CON-GPC-WAES-001 | 可生成草案，不自动确证 | 跨厂协同异常 |
| `gpc.cross_factory_exception.closed` | CrossFactoryException | L3 | GPC-Native | WAES / GFIS | CON-GPC-WAES-001 | 否 | 跨厂协同异常关闭 |
| `gfis.capacity_commitment.published` | CapacityCommitment | L2 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 工厂产能承诺发布 |
| `gfis.capacity_snapshot.published` | CapacitySnapshot | L1 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 工厂产能快照发布 |
| `gfis.inventory_visibility.snapshot_published` | InventoryVisibilitySnapshot | L2 | GFIS | GPC-Native / WAES | CON-GPC-GFIS-001 / CON-GFIS-WAES-001 | 否 | 多厂库存可见性快照 |
| `gfis.quality_trace_scope.created` | QualityTraceScope | L2 | GFIS / WAES | GPC-Native / Report | CON-GFIS-WAES-001 | 否 | 跨厂或跨链质量追溯范围建立 |
...
| `waes.connector_decision.published` | ConnectorDecision | L4 | WAES / Connector Registry | GFIS / GPC-Native / Harness | CON-GPC-WAES-001 / CON-GFIS-WAES-001 | 否 | 连接器治理结论发布 |
...
| 失败场景 | 补偿 |
|---|---|
| GFIS 发货事件已发布但 GPC-Native 未创建 Shipment | 通过 `CON-GPC-GFIS-001` 重放 `gfis.factory_shipment.released` |
| GPC-Native POD 已签收但 GFIS 未收到回传 | 通过 `CON-GPC-GFIS-001` 重放 `gpc.pod.signed` |
| Edge 信号未及时进入 GFIS | 通过 `CON-EDGE-GFIS-001` 重放 `edge.device_signal.received` |
| 主数据映射错误 | 生成 `ExceptionCase`，人工修正映射后重放 |
| AI 建议被误提交 | 撤回 `AISuggestion` 或治理草案；不得撤销业务事实事件，事务修正必须进入 GFIS/GPC-Native 内部流程 |
...
2. `gpc.*` 只能由 GPC-Native 发起；平台公共服务事实不允许由 WAES 或 GFIS 替代发布。
3. `gfis.*` 只能由 GFIS 发起；工厂执行事实不允许由 GPC-Native、WAES 或 Edge 替代发布。
