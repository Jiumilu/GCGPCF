...
适用范围：GPC-Native、GFIS、WAES、PVAOS、Edge、Brain、XiaoC、Hermes/XGD、Odoo back-office connector 候选。
...
4. 连接器不得改变主责系统事实归属；不得绕过 GFIS / GPC-Native / PVAOS 的业务确认流程；不得绕过 WAES 改写治理状态。
...
2. 连接器不能绕过 GFIS / GPC-Native / PVAOS 的业务确认流程。
...
6. 绿色供应链公共服务平台主线连接器优先围绕 `GPC-Native` 展开，治理内容在连接器层表现为注册、权限、证据、版本、状态和治理门禁。

## 2. 体系只承认的 5 个根连接器

| 编号 | 名称 | 两端 | 默认等级 | 主责 | 默认写权限 | 备注 |
|---|---|---|---|---|---|---|
| CON-GPC-GFIS-001 | GPC-Native 与 GFIS 的业务协同连接器 | GPC-Native ↔ GFIS | C1/C2 | GPC-Native / GFIS | 业务主账前置接口；不绕过业务确认 | 平台订单分发、ASN/预约、运输回传、发货回执、工厂事实回流 |
| CON-GPC-WAES-001 | GPC-Native 与 WAES 的平台证据 / 治理状态连接器 | GPC-Native ↔ WAES | C1/C4 | GPC-Native / WAES | 仅证据与治理引用，不写业务主账 | 平台事实证据、跨厂异常、状态升级请求、阶段验收引用 |
...
任何其他跨系统连接器（如 `PVAOS -> GPC-Native` 的运营上下文同步、`Brain -> WAES` 的知识准入等）必须挂接到这 5 个根连接器下，或在新版本基线中通过 [总架构] 补充新根连接器编号。
...
| sourceSystem | 是 | PVAOS / GPC-Native / GFIS / Edge / WAES / Brain / XiaoC / Hermes / XGD |
| targetSystem | 是 | 目标系统 |
| owner | 是 | 业务负责人和技术负责人 |
...
3. GFIS 内部库存调整。
4. GFIS 发货出库确认。
5. GPC-Native 客户签收和 POD 争议处理。
6. GPC-Native 客户交付承诺变更。

这些事项只能通过 `BusinessApprovalReference` 引用。

## 7. 5 个根连接器模板

### 7.1 CON-GPC-GFIS-001（GPC-Native ↔ GFIS 业务协同连接器）

```yaml
connectorId: CON-GPC-GFIS-001
connectorName: GPC-Native to GFIS Business Collaboration Connector
rootConnectorId: CON-GPC-GFIS-001
sourceSystem: GPC-Native
targetSystem: GFIS
owner:
...
### 7.2 CON-GPC-WAES-001（GPC-Native ↔ WAES 平台证据 / 治理状态连接器）

```yaml
connectorId: CON-GPC-WAES-001
connectorName: GPC-Native to WAES Platform Evidence and Governance State Connector
...
| 连接器 | 等级 | 主责 | 根连接器 | 一期目标 |
|---|---|---|---|---|
| PVAOS -> GPC-Native 上下文同步 | C1/C2 | PVAOS / GPC-Native | CON-GPC-WAES-001（治理引用）或挂接到 CON-GPC-GFIS-001 治理上下文 | 组织、项目、伙伴、门户事件 |
| GPC-Native -> WAES | C1/C4 | GPC-Native | CON-GPC-WAES-001 | 订单、运输、POD、外部异常、跨厂异常证据 |
| GPC-Native -> GFIS | C1/C2 | GPC-Native / GFIS | CON-GPC-GFIS-001 | 订单分发、预约、运输回传、发货回执、工厂事实回流 |
| GFIS -> WAES | C1/C4 | GFIS | CON-GFIS-WAES-001 | 工厂执行事实、工厂异常、审计、证据确证 |
| GFIS -> GPC-Native | C1 | GFIS | CON-GPC-GFIS-001 | 发货、质量、库存、产能事件 |
...
| Odoo -> GPC-Native | C0/C1 | GPC-Native | 挂接到 CON-GPC-WAES-001（治理引用） | 历史原型或 back-office 数据引用 |
