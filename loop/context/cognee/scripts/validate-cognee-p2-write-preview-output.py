#!/usr/bin/env python3
"""Validate COGNEE P2 write-preview evidence output."""
...
def main() -> int:
    ...
    if missing_top:
        raise SystemExit(f"missing top-level fields: {missing_top}")
    ...
    for i, rec in enumerate(records, 1):
        missing_fields = sorted(REQUIRED_RECORD_FIELDS - set(rec.keys()))
        if missing_fields:
            raise SystemExit(f"record#{i} missing fields: {missing_fields}")

    print("cognee_p2_write_preview_output=pass")
    print(
        ...
        f"preview_block_rate={payload.get('summary', {}).get('preview_block_rate', 0.0)}"
    )
    return 0
