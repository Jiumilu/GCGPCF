#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-241 digest delivery preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-241-digest-delivery-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-240-escalation-digest-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks241DigestDeliveryPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["digestPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: digestPreviewRefs must come from DKS-240 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_241_digest_delivery_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_241_digest_delivery_preview=pass "
