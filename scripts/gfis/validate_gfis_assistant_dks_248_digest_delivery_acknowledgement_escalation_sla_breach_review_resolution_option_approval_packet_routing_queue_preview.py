#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-248 approval packet routing queue preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-248-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-247-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks248DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueuePreview"
NO_WRITE_KEYS = ('writesGfis', 'writesGpc', 'writesErp', 'writesMes', 'writesRoutingQueue', 'writesQueueItem', 'writesApprovalAssignment', 'writesApprovalLock', 'writesApprovalPacket', 'writesApprovalRequest', 'writesApprovalDecision', 'writesCommitteeDecision', 'writesFreezeAction', 'writesWaesGateResult', 'writesKweWorkItem', 'writesHarnessEvidence', 'writesKdsLifecycle', 'writesKdsFact', 'writesKdsAcceptedFact', 'writesExternalApi')
...
def main() -> int:
    ...
    upstream = json.loads(UPSTREAM.read_text(encoding="utf-8"))["approvalPacketPreviews"]
    upstream_ids = {item["approvalPacketPreviewId"] for item in upstream}
    previews: list[dict[str, Any]] = fixture["routingQueuePreviews"]
    expected = fixture["expected"]
    failures: list[str] = []
    unions = {
        f"{TYPE_PREFIX}Type": policy["routing_queue_types"],
        f"{TYPE_PREFIX}Status": policy["routing_queue_statuses"],
        f"{TYPE_PREFIX}Decision": policy["routing_queue_decisions"],
        f"{TYPE_PREFIX}Scope": policy["routing_queue_scopes"],
        f"{TYPE_PREFIX}Slot": policy["routing_queue_slots"],
        f"{TYPE_PREFIX}Priority": policy["routing_queue_priorities"],
        f"{TYPE_PREFIX}Reason": policy["queue_reasons"],
    ...
    for preview in previews:
        ...
        if preview["approvalPacketPreviewRefs"] != preview["sourceApprovalPacketPreviewRefs"]:
            failures.append(f"{preview_id}: approvalPacketPreviewRefs must equal sourceApprovalPacketPreviewRefs")
        if not set(preview["approvalPacketPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: approvalPacketPreviewRefs must come from DKS-247 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_248_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_248_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_preview=pass "
        ...
        "creates_routing_queues=0 creates_queue_items=0 creates_approval_assignments=0 locks_approvers=0 "
        "creates_approval_packets=0 creates_approval_requests=0 creates_approval_decisions=0 creates_committee_decisions=0 "
        "creates_freeze_actions=0 creates_harness_evidence=0 creates_waes_gate_results=0 creates_kwe_work_items=0 "
