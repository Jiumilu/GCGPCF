#!/usr/bin/env python3
"""Validate GFIS Assistant acknowledgement escalation preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewType": policy["escalation_types"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewStatus": policy["escalation_statuses"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewDecision": policy["escalation_decisions"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewScope": policy["escalation_scopes"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewLevel": policy["escalation_levels"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewTrigger": policy["escalation_triggers"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewReason": policy["escalation_reasons"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    levels = set(policy["escalation_levels"])
    triggers = set(policy["escalation_triggers"])
    totals = {key: 0 for key in NO_WRITE_KEYS}
    counts = {
        "brainSurface": 0, "pkcSurface": 0, "gfisAssistantSurface": 0,
        "totalEscalationLevelCount": 0, "totalEscalationRecipientCount": 0,
        "totalRequiredEvidenceCount": 0, "totalEscalationReasonCount": 0,
    ...
    for preview in previews:
        ...
        if preview["escalationLevel"] not in levels:
            failures.append(f"{preview_id}: escalationLevel must be declared")
        if preview["escalationTrigger"] not in triggers:
        ...
        counts["totalEscalationRecipientCount"] += len(preview["candidateEscalationRecipientRefs"])
    ...
    if failures:
        print("gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_acknowledgement_escalation_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_acknowledgement_escalation_preview=pass "
        f"escalations={actual['escalationPreviewCount']} brain={actual['brainSurface']} pkc={actual['pkcSurface']} "
        f"gfis_assistant={actual['gfisAssistantSurface']} total_levels={actual['totalEscalationLevelCount']} "
        f"total_recipients={actual['totalEscalationRecipientCount']} total_required_evidence={actual['totalRequiredEvidenceCount']} "
        ...
        "writes_kwe_work_item=0 writes_notification=0 writes_acknowledgement=0 writes_receipt=0 writes_read_receipt=0 "
        "writes_delivery_status=0 writes_approval_assignment=0 writes_approval_lock=0 writes_approval_packet=0 "
        "writes_approval_request=0 writes_approval_decision=0 writes_committee_decision=0 writes_freeze_action=0 "
        "writes_waes_gate_result=0 writes_harness_evidence=0 writes_kds_lifecycle=0 writes_kds_fact=0 "
