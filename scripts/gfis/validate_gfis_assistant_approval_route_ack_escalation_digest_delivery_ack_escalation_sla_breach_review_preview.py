#!/usr/bin/env python3
"""Validate GFIS Assistant approval route acknowledgement escalation SLA breach review no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewType": policy["breach_review_types"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewStatus": policy["breach_review_statuses"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewDecision": policy["breach_review_decisions"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewScope": policy["breach_review_scopes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewSeverity": policy["breach_severities"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewReason": policy["breach_reasons"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    if failures:
        print("gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_preview=pass "
