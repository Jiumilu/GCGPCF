#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-254 digest delivery acknowledgement preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-254-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-acknowledgement-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-253-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks254DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAcknowledgementPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["deliveryPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: deliveryPreviewRefs must come from DKS-253 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_254_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_acknowledgement_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_254_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_acknowledgement_preview=pass "
