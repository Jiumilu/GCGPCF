#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-172 routing queue notification acknowledgement escalation digest delivery acknowledgement escalation SLA preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewType": policy["sla_types"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewStatus": policy["sla_statuses"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewDecision": policy["sla_decisions"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewScope": policy["sla_scopes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewRisk": policy["sla_risks"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewReason": policy["sla_reasons"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    counts = {
        ...
        "totalBlockedSlaEscalationCount": 0, "totalSlaWindowMinutes": 0,
        "totalElapsedMinutes": 0, "totalRemainingMinutes": 0,
        "totalOverdueMinutes": 0, "createsSlaTimers": 0,
        "createsEscalations": 0, "createsReminders": 0,
    ...
    for preview in previews:
        ...
        escalation_refs = set(preview["escalationPreviewRefs"])
        source_refs = set(preview["sourceEscalationPreviewRefs"])
        if not escalation_refs or not escalation_refs <= source_refs:
            failures.append(f"{preview_id}: escalationPreviewRefs must be included in source refs")
        if not preview["deliveryAcknowledgementPreviewRefs"] or not preview["deliveryPreviewRefs"] or not preview["digestPreviewRefs"]:
    ...
    if failures:
        print("gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_sla_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_sla_preview=pass "
        ...
        f"total_reasons={actual['totalReasonCount']} total_blocked_sla_escalations={actual['totalBlockedSlaEscalationCount']} "
        ...
        f"creates_sla_timers={actual['createsSlaTimers']} creates_escalations={actual['createsEscalations']} "
        f"creates_reminders={actual['createsReminders']} creates_escalation_tasks={actual['createsEscalationTasks']} "
        f"creates_delivery_acknowledgements={actual['createsDeliveryAcknowledgements']} creates_approval_requests={actual['createsApprovalRequests']} "
