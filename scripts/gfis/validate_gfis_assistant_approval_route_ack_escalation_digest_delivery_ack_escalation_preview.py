#!/usr/bin/env python3
"""Validate GFIS Assistant approval route delivery acknowledgement escalation preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "approval-route-ack-escalation-digest-delivery-ack-escalation-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewType": policy["escalation_types"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewStatus": policy["escalation_statuses"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewDecision": policy["escalation_decisions"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewScope": policy["escalation_scopes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewReason": policy["escalation_reasons"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    if failures:
        print("gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_preview=pass "
