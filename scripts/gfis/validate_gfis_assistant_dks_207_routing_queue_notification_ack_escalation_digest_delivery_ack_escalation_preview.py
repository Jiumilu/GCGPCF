#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-207 delivery acknowledgement escalation preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-dks-207-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-dks-207-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-207-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewType": policy["escalation_types"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewStatus": policy["escalation_statuses"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewDecision": policy["escalation_decisions"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewScope": policy["escalation_scopes"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewPriority": policy["escalation_priorities"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewReason": policy["escalation_reasons"],
        "GfisAssistantDks207RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    escalation_priorities = set(policy["escalation_priorities"])
    ...
    counts = {
        ...
        "totalRequiredEvidenceCount": 0, "totalBlockedEscalationCount": 0, "createsEscalations": 0,
    ...
    for preview in previews:
        ...
        if preview["escalationPriority"] not in escalation_priorities:
    ...
    if failures:
        print("gfis_assistant_dks_207_routing_queue_notification_acknowledgement_escalation_digest_delivery_ack_escalation_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_dks_207_routing_queue_notification_acknowledgement_escalation_digest_delivery_ack_escalation_preview=pass "
        ...
        f"total_reasons={actual['totalEscalationReasonCount']} total_required_evidence={actual['totalRequiredEvidenceCount']} "
        f"total_blocked_escalations={actual['totalBlockedEscalationCount']} "
