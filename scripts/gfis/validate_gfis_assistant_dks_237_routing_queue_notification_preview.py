#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-237 routing queue notification no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-dks-237-routing-queue-notification-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-dks-237-routing-queue-notification-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-237-routing-queue-notification-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantDks237RoutingQueueNotificationPreviewType": policy["notification_types"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewStatus": policy["notification_statuses"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewDecision": policy["notification_decisions"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewScope": policy["notification_scopes"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewChannel": policy["notification_channels"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewPriority": policy["notification_priorities"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewReason": policy["notification_reasons"],
        "GfisAssistantDks237RoutingQueueNotificationPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    if failures:
        print("gfis_assistant_dks_237_routing_queue_notification_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_dks_237_routing_queue_notification_preview=pass "
