#!/usr/bin/env python3
"""Validate GFIS Assistant acknowledgement escalation digest preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-digest-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-digest-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-acknowledgement-escalation-digest-preview-dry-run.json"

NO_WRITE_KEYS = (
    "writesGfis", "writesGpc", "writesErp", "writesMes",
    "writesDigest", "writesDigestDelivery", "writesEscalation",
    "writesTimeoutEvent", "writesKweWorkItem", "writesNotification",
    "writesAcknowledgement", "writesReceipt", "writesReadReceipt",
    "writesDeliveryStatus", "writesApprovalAssignment", "writesApprovalLock",
    "writesApprovalPacket", "writesApprovalRequest", "writesApprovalDecision",
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewType": policy["digest_types"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewStatus": policy["digest_statuses"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewDecision": policy["digest_decisions"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewScope": policy["digest_scopes"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewChannel": policy["digest_channels"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewPriority": policy["digest_priorities"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewReason": policy["digest_reasons"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAcknowledgementEscalationDigestPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    reasons = set(policy["digest_reasons"])
    channels = set(policy["digest_channels"])
    priorities = set(policy["digest_priorities"])
    totals = {key: 0 for key in NO_WRITE_KEYS}
    counts = {
        "brainSurface": 0, "pkcSurface": 0, "gfisAssistantSurface": 0,
        "totalDigestChannelCount": 0, "totalDigestRecipientCount": 0,
        "totalRequiredEvidenceCount": 0, "totalDigestReasonCount": 0,
        "totalBlockedDigestCount": 0, "createsDigests": 0,
        "createsDigestDeliveries": 0, "createsEscalations": 0,
        "createsTimeoutEvents": 0, "createsKweWorkItems": 0,
    ...
    false_flags = (
        "createsDigest", "createsDigestDelivery", "createsEscalation",
    ...
    for preview in previews:
        ...
        if (
            not preview["acknowledgementPreviewRefs"]
        ...
        if not set(preview["digestReasonRefs"]) <= reasons:
            failures.append(f"{preview_id}: digestReasonRefs must be declared")
        if preview["digestChannel"] not in channels:
            failures.append(f"{preview_id}: digestChannel must be declared")
        if preview["digestPriority"] not in priorities:
        ...
        counts["totalDigestRecipientCount"] += len(preview["candidateDigestRecipientRefs"])
    ...
    if failures:
        print("gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_acknowledgement_escalation_digest_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_acknowledgement_escalation_digest_preview=pass "
        f"digests={actual['digestPreviewCount']} brain={actual['brainSurface']} pkc={actual['pkcSurface']} "
        f"gfis_assistant={actual['gfisAssistantSurface']} total_channels={actual['totalDigestChannelCount']} "
        f"total_recipients={actual['totalDigestRecipientCount']} total_required_evidence={actual['totalRequiredEvidenceCount']} "
        f"total_digest_reasons={actual['totalDigestReasonCount']} total_blocked_digests={actual['totalBlockedDigestCount']} "
        f"creates_digests={actual['createsDigests']} creates_digest_deliveries={actual['createsDigestDeliveries']} "
        ...
        "writes_gfis=0 writes_gpc=0 writes_erp=0 writes_mes=0 writes_digest=0 writes_digest_delivery=0 "
        "writes_escalation=0 writes_timeout_event=0 writes_kwe_work_item=0 writes_notification=0 "
