#!/usr/bin/env python3
"""Validate GFIS Assistant approval packet routing queue notification acknowledgement escalation digest delivery acknowledgement escalation SLA breach review resolution option approval packet no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-breach-approval-packet-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-breach-approval-packet-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-breach-approval-packet-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewType": policy["approval_packet_types"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewStatus": policy["approval_packet_statuses"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewDecision": policy["approval_packet_decisions"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewScope": policy["approval_packet_scopes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewRoute": policy["approval_routes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewReason": policy["approval_reasons"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachApprovalPacketPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    counts = {
        ...
        "totalApproverCount": 0, "totalRequiredEvidenceCount": 0,
        "totalApprovalReasonCount": 0, "totalBlockedApprovalCount": 0,
    ...
    for preview in previews:
        ...
        if not set(preview["resolutionOptionPreviewRefs"]) <= set(preview["sourceResolutionOptionPreviewRefs"]):
            failures.append(f"{preview_id}: resolutionOptionPreviewRefs must be included in source refs")
    ...
    if failures:
        print("gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_sla_breach_approval_packet_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_sla_breach_approval_packet_preview=pass "
        ...
        f"total_blocked_approval={actual['totalBlockedApprovalCount']} creates_approval_packets={actual['createsApprovalPackets']} "
