#!/usr/bin/env python3
"""Validate GFIS Assistant approval route delivery acknowledgement escalation SLA preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-approval-route-ack-escalation-digest-delivery-ack-escalation-sla-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "approval-route-ack-escalation-digest-delivery-ack-escalation-sla-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewType": policy["sla_types"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewStatus": policy["sla_statuses"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewDecision": policy["sla_decisions"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewScope": policy["sla_scopes"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewRisk": policy["sla_risks"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewReason": policy["sla_reasons"],
        "GfisAssistantApprovalRouteAckEscalationDigestDeliveryAckEscalationSlaPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    if failures:
        print("gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_approval_route_ack_escalation_digest_delivery_ack_escalation_sla_preview=pass "
