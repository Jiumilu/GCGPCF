#!/usr/bin/env python3
"""Validate GFIS Assistant resolution option approval packet preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "repair-notification-snooze-queue-approval-acknowledgement-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-preview-dry-run.json"
...
def main() -> int:
    ...
    unions = {
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewType": policy["approval_packet_types"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewStatus": policy["approval_packet_statuses"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewDecision": policy["approval_packet_decisions"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewScope": policy["approval_packet_scopes"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewRoute": policy["approval_routes"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewReason": policy["approval_reasons"],
        "GfisAssistantRepairNotificationSnoozeQueueApprovalAcknowledgementDigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketPreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    for preview in previews:
        ...
        if not set(preview["resolutionOptionPreviewRefs"]) <= set(preview["sourceResolutionOptionPreviewRefs"]):
            failures.append(f"{preview_id}: resolutionOptionPreviewRefs must be included in sourceResolutionOptionPreviewRefs")
        if not preview["breachReviewPreviewRefs"]:
            failures.append(f"{preview_id}: breachReviewPreviewRefs are required")
        ...
        if preview["blockedApprovalCount"] < 0 or preview["blockedApprovalCount"] > len(preview["candidateApproverRefs"]):
            failures.append(f"{preview_id}: blockedApprovalCount must be between 0 and candidateApproverRefs length")
    ...
    if failures:
        print("gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_repair_notification_snooze_queue_approval_acknowledgement_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_preview=pass "
