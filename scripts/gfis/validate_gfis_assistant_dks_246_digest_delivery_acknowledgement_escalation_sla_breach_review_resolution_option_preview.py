#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-246 breach review resolution option preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-246-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-245-digest-delivery-acknowledgement-escalation-sla-breach-review-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks246DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["breachReviewPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: breachReviewPreviewRefs must come from DKS-245 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_246_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_246_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_preview=pass "
