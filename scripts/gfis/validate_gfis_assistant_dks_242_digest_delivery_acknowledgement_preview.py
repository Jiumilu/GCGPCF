#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-242 digest delivery acknowledgement preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-242-digest-delivery-acknowledgement-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-241-digest-delivery-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks242DigestDeliveryAcknowledgementPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["deliveryPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: deliveryPreviewRefs must come from DKS-241 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_242_digest_delivery_acknowledgement_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_242_digest_delivery_acknowledgement_preview=pass "
