#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-253 digest delivery preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-253-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-252-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks253DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["digestPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: digestPreviewRefs must come from DKS-252 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_253_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_253_digest_delivery_acknowledgement_escalation_sla_breach_review_resolution_option_approval_packet_routing_queue_notification_ack_escalation_digest_delivery_preview=pass "
