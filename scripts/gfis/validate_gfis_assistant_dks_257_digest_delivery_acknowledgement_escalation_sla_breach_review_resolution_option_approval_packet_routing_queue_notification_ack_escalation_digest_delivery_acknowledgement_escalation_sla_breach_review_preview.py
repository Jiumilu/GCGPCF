#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-257 SLA breach review preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-257-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-acknowledgement-escalation-sla-breach-review-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-256-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-acknowledgement-escalation-sla-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks257DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAcknowledgementEscalationSlaBreachReviewPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["slaPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: slaPreviewRefs must come from DKS-256 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_257_digest_delivery_acknowledgement_escalation_sla_breach_review_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_257_digest_delivery_acknowledgement_escalation_sla_breach_review_preview=pass "
