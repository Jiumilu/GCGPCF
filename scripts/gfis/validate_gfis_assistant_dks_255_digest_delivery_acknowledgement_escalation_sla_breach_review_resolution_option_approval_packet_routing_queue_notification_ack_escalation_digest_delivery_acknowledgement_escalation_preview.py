#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-255 digest delivery acknowledgement escalation preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-255-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-acknowledgement-escalation-preview"
...
UPSTREAM = ROOT / "fixtures" / "gfis" / "gfis-assistant-dks-254-digest-delivery-acknowledgement-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-acknowledgement-preview-dry-run.json"
TYPE_PREFIX = "GfisAssistantDks255DigestDeliveryAcknowledgementEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAcknowledgementEscalationPreview"
...
def main() -> int:
    ...
    for preview in previews:
        ...
        if not set(preview["deliveryAcknowledgementPreviewRefs"]) <= upstream_ids:
            failures.append(f"{preview_id}: deliveryAcknowledgementPreviewRefs must come from DKS-254 upstream")
    ...
    if failures:
        print("gfis_assistant_dks_255_digest_delivery_acknowledgement_escalation_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print(
        "gfis_assistant_dks_255_digest_delivery_acknowledgement_escalation_preview=pass "
