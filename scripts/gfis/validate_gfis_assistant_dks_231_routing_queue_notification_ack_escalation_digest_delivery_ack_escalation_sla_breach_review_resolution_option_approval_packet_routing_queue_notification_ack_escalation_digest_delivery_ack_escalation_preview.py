#!/usr/bin/env python3
"""Validate GFIS Assistant DKS-231 delivery ack escalation preview no-write boundary."""
...
SLUG = "gfis-assistant-dks-231-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-sla-breach-review-resolution-option-approval-packet-routing-queue-notification-ack-escalation-digest-delivery-ack-escalation-preview"
...
TYPE_PREFIX = "GfisAssistantDks231RoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationSlaBreachReviewResolutionOptionApprovalPacketRoutingQueueNotificationAckEscalationDigestDeliveryAckEscalationPreview"
...
def main() -> int:
    ...
    failures: list[str] = []
    unions = {f"{TYPE_PREFIX}Type": policy["escalation_types"], f"{TYPE_PREFIX}Status": policy["escalation_statuses"], f"{TYPE_PREFIX}Decision": policy["escalation_decisions"], f"{TYPE_PREFIX}Scope": policy["escalation_scopes"], f"{TYPE_PREFIX}Priority": policy["escalation_priorities"], f"{TYPE_PREFIX}Reason": policy["escalation_reasons"], f"{TYPE_PREFIX}BlockedAction": policy["blocked_actions"]}
    ...
    for preview in previews:
        ...
        if not set(preview["deliveryAcknowledgementPreviewRefs"]) <= set(preview["sourceDeliveryAcknowledgementPreviewRefs"]):
        ...
        if not preview["deliveryPreviewRefs"] or not preview["digestPreviewRefs"] or not preview["acknowledgementPreviewRefs"] or not preview["notificationPreviewRefs"] or not preview["routingQueuePreviewRefs"] or not preview["approvalPacketPreviewRefs"]:
    ...
    if failures:
        print("gfis_assistant_dks_231_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_preview=fail")
        for failure in failures:
            print(failure)
        return 1
    print("gfis_assistant_dks_231_routing_queue_notification_ack_escalation_digest_delivery_ack_escalation_preview=pass " f"escalations={actual['escalationPreviewCount']} brain={actual['brainSurface']} pkc={actual['pkcSurface']} gfis_assistant={actual['gfisAssistantSurface']} total_escalation_owners={actual['totalEscalationOwnerCount']} total_required_evidence={actual['totalRequiredEvidenceCount']} total_escalation_reasons={actual['totalEscalationReasonCount']} total_blocked_escalations={actual['totalBlockedEscalationCount']} creates_escalations={actual['createsEscalations']} creates_escalation_tasks={actual['createsEscalationTasks']} creates_delivery_acknowledgements={actual['createsDeliveryAcknowledgements']} creates_digest_deliveries={actual['createsDigestDeliveries']} creates_deliveries={actual['createsDeliveries']} creates_digests={actual['createsDigests']} creates_kwe_work_items={actual['createsKweWorkItems']} creates_notifications={actual['createsNotifications']} creates_acknowledgements={actual['createsAcknowledgements']} creates_receipts={actual['createsReceipts']} creates_read_receipts={actual['createsReadReceipts']} updates_delivery_status={actual['updatesDeliveryStatus']} sends_external_notifications={actual['sendsExternalNotifications']} creates_approval_assignments={actual['createsApprovalAssignments']} locks_approvers={actual['locksApprovers']} creates_approval_packets={actual['createsApprovalPackets']} creates_approval_requests={actual['createsApprovalRequests']} creates_approval_decisions={actual['createsApprovalDecisions']} creates_committee_decisions={actual['createsCommitteeDecisions']} creates_freeze_actions={actual['createsFreezeActions']} creates_harness_evidence={actual['createsHarnessEvidence']} creates_waes_gate_results={actual['createsWaesGateResults']} writes_gfis=0 writes_gpc=0 writes_erp=0 writes_mes=0 writes_escalation=0 writes_escalation_task=0 writes_delivery_acknowledgement=0 writes_digest_delivery=0 writes_delivery=0 writes_digest=0 writes_kwe_work_item=0 writes_notification=0 writes_acknowledgement=0 writes_receipt=0 writes_read_receipt=0 writes_delivery_status=0 writes_approval_assignment=0 writes_approval_lock=0 writes_approval_packet=0 writes_approval_request=0 writes_approval_decision=0 writes_committee_decision=0 writes_freeze_action=0 writes_waes_gate_result=0 writes_harness_evidence=0 writes_kds_lifecycle=0 writes_kds_fact=0 writes_kds_accepted_fact=0 writes_external_api=0")
    return 0
