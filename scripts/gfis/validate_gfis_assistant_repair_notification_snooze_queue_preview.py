#!/usr/bin/env python3
"""Validate GFIS Assistant repair notification snooze queue preview no-write boundary."""
...
POLICY = ROOT / "okf" / "gfis-assistant-repair-notification-snooze-queue-preview-policy.yaml"
TYPE_FILE = ROOT / "packages" / "shared" / "src" / "knowledge" / "gfis-assistant-repair-notification-snooze-queue-preview.ts"
FIXTURE = ROOT / "fixtures" / "gfis" / "repair-notification-snooze-queue-preview-dry-run.json"

NO_WRITE_KEYS = (
    "writesGfis", "writesGpc", "writesErp", "writesMes", "writesQueueItem",
    "writesSnoozeRecord", "writesScheduledReminder", "writesDismissalRecord",
...
def main() -> int:
    ...
    previews: list[dict[str, Any]] = fixture["queuePreviews"]
    expected = fixture["expected"]
    failures: list[str] = []

    unions = {
        "GfisAssistantRepairNotificationSnoozeQueuePreviewType": policy["queue_types"],
        "GfisAssistantRepairNotificationSnoozeQueuePreviewStatus": policy["queue_statuses"],
        "GfisAssistantRepairNotificationSnoozeQueuePreviewDecision": policy["queue_decisions"],
        "GfisAssistantRepairNotificationSnoozeQueuePreviewBlockedAction": policy["blocked_actions"],
    }
    ...
    counts = {
        ...
        "brainQueue": 0, "pkcQueue": 0, "gfisAssistantQueue": 0,
        "queuePreviewOnly": 0, "queueContainsBlockedItems": 0, "queueContainsRetainedItems": 0,
        "createsQueueItems": 0, "createsSnoozeRecords": 0, "createsScheduledReminders": 0,
        "createsDismissalRecords": 0, "createsNotifications": 0, "modifiesNotifications": 0,
        "createsHarnessEvidence": 0, "createsWaesGateResults": 0, "createsKweWorkItems": 0,
        "persistsEvidence": 0, "approvesBusinessWrite": 0, "promotesLifecycle": 0,
        "completesCommitteeDecision": 0,
    }
    surface_keys = {"brain": "brainSurface", "pkc": "pkcSurface", "gfis_assistant": "gfisAssistantSurface"}
    type_keys = {
        "brain_snooze_queue_preview": "brainQueue",
        "pkc_snooze_queue_preview": "pkcQueue",
        "gfis_assistant_snooze_queue_preview": "gfisAssistantQueue",
    }
    status_keys = {
        "queue_preview_only": "queuePreviewOnly",
        "queue_contains_blocked_items": "queueContainsBlockedItems",
        "queue_contains_retained_items": "queueContainsRetainedItems",
    }
    false_flags = (
        "createsQueueItem", "createsSnoozeRecord", "createsScheduledReminder",
        "createsDismissalRecord", "createsNotification", "modifiesNotification",
    ...
    for preview in previews:
        preview_id = preview["queuePreviewId"]
        ...
        if sorted(preview["sourceSnoozePreviewRefs"]) != sorted(preview["orderedSnoozePreviewRefs"]):
            failures.append(f"{preview_id}: ordered refs must be a permutation of source refs")
        ...
        if not preview["queueSummaryRef"] or not preview["reasonRefs"] or not preview["queueNoteRefs"]:
            failures.append(f"{preview_id}: queueSummaryRef, reasonRefs, and queueNoteRefs are required")
        counts[surface_keys[preview["surface"]]] += 1
        counts[type_keys[preview["queueType"]]] += 1
        counts[status_keys[preview["queueStatus"]]] += 1
    ...
    actual = {"queuePreviewCount": len(previews), **counts, **totals}
    ...
    if failures:
        print("gfis_assistant_repair_notification_snooze_queue_preview=fail")
        for failure in failures:
            print(failure)
        return 1

    print(
        "gfis_assistant_repair_notification_snooze_queue_preview=pass "
        f"queues={actual['queuePreviewCount']} brain={actual['brainSurface']} pkc={actual['pkcSurface']} "
        f"gfis_assistant={actual['gfisAssistantSurface']} brain_queue={actual['brainQueue']} "
        f"pkc_queue={actual['pkcQueue']} gfis_assistant_queue={actual['gfisAssistantQueue']} "
        ...
        "writes_gfis=0 writes_gpc=0 writes_erp=0 writes_mes=0 writes_queue_item=0 "
