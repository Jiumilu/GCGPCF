#!/usr/bin/env python3
"""Validate P0 notification acknowledgement receipt preview dry-run."""
...
FIXTURE = (
    ...
    / "fixtures"
    / "api"
    / "gckf-p0-formal-evidence-execution-notification-acknowledgement-receipt-preview-dry-run-v0.1.json"
)
...
def main() -> int:
    ...
    receipt = data["notificationAcknowledgementReceiptPreview"]

    if data.get("notificationAcknowledgementReceiptPreviewStatus") != expected[
        "notificationAcknowledgementReceiptPreviewStatus"
    ]:
        failures.append("notificationAcknowledgementReceiptPreviewStatus must remain candidate_preview")
    if data.get("executionMode") != expected["executionMode"]:
        failures.append("executionMode must remain dry_run_no_write")
    if data.get("notFinalNotificationAcknowledgementReceipt") is not expected[
        "notFinalNotificationAcknowledgementReceipt"
    ]:
        failures.append("receipt preview must state notFinalNotificationAcknowledgementReceipt=true")
    if receipt.get("previewType") != expected["previewType"]:
        failures.append("notification acknowledgement receipt previewType mismatch")
    if receipt.get("previewStatus") != expected["previewStatus"]:
        failures.append("notification acknowledgement receipt previewStatus must remain candidate_preview")
    if receipt.get("executionMode") != expected["executionMode"]:
        failures.append("notification acknowledgement receipt executionMode mismatch")
    if receipt.get("dryRunOnly") is not expected["dryRunOnly"]:
        failures.append("notification acknowledgement receipt must remain dryRunOnly=true")
    ...
    require_all(
        set(receipt.get("receiptSections", [])),
        {
            ...
            "candidate_receipt_payload_summary",
        ...
        "notification acknowledgement receipt section",
        failures,
    )
    require_all(
        ...
        "notification acknowledgement receipt constraint",
        failures,
    )
    require_all(
    ...
    if failures:
        print("gckf_p0_formal_evidence_execution_notification_acknowledgement_receipt_preview_dry_run=fail")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("gckf_p0_formal_evidence_execution_notification_acknowledgement_receipt_preview_dry_run=pass")
