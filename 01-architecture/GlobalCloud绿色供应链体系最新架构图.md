...
日期：2026-06-07  
状态：最新统一架构图 v1  
口径：以 `GPC-Native` 为绿色供应链平台主线，以 `WAES` 为治理与监控平面，以 `GFIS + Edge` 为生产与执行平面，横向统一为 `AI 与数据层`；其内部包含资源仓库、结构化数据库、企业级知识主存、知识引擎、AI 服务和技术运行底座。
...
flowchart TB
  subgraph GOV["治理与监控层"]
    ...
    Hermes["Hermes / XGD<br/>Agent运行 / 多端交互 / 后台任务"]
  end

  subgraph OPS["运营与协同层"]
    PVAOS["PVAOS<br/>租户 / 组织 / 伙伴 / 项目 / 门户入口"]
    GPC["GPC-Native<br/>平台订单 / ASN / 预约 / 运输 / POD / 外部异常 / 多厂协同"]
  ...
  subgraph DATA["AI 与数据层"]
    ...
    KE["知识引擎层<br/>LLM Wiki / GBrain / Brain索引"]
    AI["AI服务域<br/>XiaoC / Hermes / XGD"]
    APIGW["API Gateway"]
...
1. `GPC-Native` 是绿色供应链平台主线，不再把 Odoo 原型当成主线。
...
5. `LLM Wiki` 与 `GBrain` 当前位于知识引擎候选域，不直接等于企业级知识真源。
...
`GPC-Native 负责平台协同，GFIS 负责工厂执行，WAES 负责治理约束，PVAOS 负责生态入口，Edge 负责现场接入。`
