...
| 业务流 | 平台订单、样品申请、客户签样、转量产、ASN、预约、运输、POD、外部异常、多厂分单 | 不做 GFIS 工厂执行主账，不做配方研发和样品检测事实 |
...
| `SampleRequest` | 样品申请、客户签样和转量产门禁 |
| `OrderMapping` | 已签样或已转量产的 GPC 平台订单与 GFIS 工厂订单映射 |
| `Supplier` | 供应商 |
| `Customer` | 客户 |
| `ASN` | 供应商到货通知 |
| `Appointment` | 到货或发货预约 |
...
1. 一张平台订单必须先完成样品申请、客户签样或豁免、转量产门禁，才能映射到 GFIS 工厂订单。
