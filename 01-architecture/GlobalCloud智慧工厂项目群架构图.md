...
flowchart TB
  subgraph GOV["治理与监控层"]
    ...
    XiaoC["GlobalCloud XiaoC<br/>Prompt 工程 / MCP 服务<br/>模型适配 / Agent 模板 / 评估"]
    Hermes["GlobalCloud XiaoG Agent<br/>Hermes WebUI<br/>持续运行 Agent / 后台任务 / 会话工具"]
    XGD["GlobalCloud XGD<br/>桌面 / 语音 / 社交入口<br/>Brain UI / 现场交互 / 多渠道提醒"]
  end

  subgraph OPS["运营与协同层"]
    GPC["GlobalCloud GPC-Native<br/>轻量绿色供应链协同中台<br/>TMS / SRM协同 / CRM协同 / 外部协同<br/>预约 / 车辆 / 在途 / 签收 / 回单"]
  ...
  XiaoC -- "Agent Prompt / MCP / 模型配置" --> Hermes
  Hermes -- "日报 / 异常摘要 / 建议草案" --> WAES
  XGD -- "语音查询 / 异常上报 / 现场提醒" --> Hermes
...
2. **GPC-Native 是跨企业协同平台**：供应商、客户、运输、签收、公共服务和绿色供应链协同归 GPC-Native；Odoo GPC 仅保留为历史原型或可选 back-office connector。
3. **WAES 是治理与监控中枢和证据平面**：聚合指标、风险、治理规则、证据，不审批具体事务，不直接改写生产主账。
4. **Brain/XiaoC/Hermes/XGD 是 AI 支撑层**：提供知识、Prompt、Agent 执行和多端交互。
...
flowchart LR
  ...
  AI["Brain / XiaoC / Hermes / XGD"] -. "只能到 L1-L3 默认能力" .-> L3
  WAES["WAES 治理与证据台账"] -. "承接 L4 治理授权" .-> L4
  GFIS["GFIS / GPC-Native 主责系统"] -. "承接具体事务确认" .-> L4
```
