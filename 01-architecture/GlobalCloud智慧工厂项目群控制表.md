...
当前补充口径：阅读本表时，平台主线统一按 `GPC-Native` 理解，宪法内容统一按 `WAES` 治理约束理解。
...
| GPC-Native（目标主线）/ Odoo GPC（历史原型） | 目标主线待建；现有原型 `/Users/lujunxiang/Projects/GlobalCloud GPC` | 现有 Odoo 原型为 `19.0`，ahead 18，双 remote：`origin=odoo/odoo`、`fork=Jiumilu/odoo` | 轻量绿色供应链协同中台、TMS、外部协同、供应商/客户/监管平台；Odoo 降级为 back-office connector 候选 | 冻结 Odoo core 二开；建立 GPC-Native ADR、对象目录、事件合同；定义 GFIS-GPC-Native 订单、发货、签收、回单映射 | 不再继续维护私有 Odoo 发行版；目标主线需新建或重构 |
...
| XGD | `/Users/lujunxiang/Projects/GlobalCloud XGD` | `codex/xgd-harness-100`，双 remote：`origin=Jiumilu/XGD`、`xiaog=Jiumilu/XiaoG` | 桌面/语音/社交 AI 入口 | 设计班组长/调度/设备/质量语音查询与异常上报 | 当前状态有 runtime/release blockers；不得发布或签名 |
...
5. 用 WAES 做治理控制塔，不写 GFIS/GPC-Native 事实。
6. 用 Brain/XiaoC/Hermes/XGD 做 AI 辅助，不越过 L3/L4/L5 授权边界。
7. 具体事务确认留在 GFIS/GPC-Native；WAES 只做治理规则、证据确证、状态升级和 AI 授权。
