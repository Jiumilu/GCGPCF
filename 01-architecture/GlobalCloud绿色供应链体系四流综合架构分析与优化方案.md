...
继承基线：README、层次化结构、对象目录、事件合同、一期验收矩阵、全局初始化 SOP、项目群架构方案、项目群架构图、专项图集、GPC-Native ADR、GFIS V3.1。

## 1. 总体诊断结论

当前 GlobalCloud 绿色供应链体系的主方向是正确的：三层主架构已经建立，四阶段生命周期已经形成，WAES、GPC-Native、GFIS、PVAOS、Edge、Brain、XiaoC、Hermes/XGD 的基本边界已经明确。
...
6. 具体事务确认在 GFIS 或 GPC-Native 内部完成，WAES 只引用 `BusinessApprovalReference` 和 `EvidenceRecord`。
7. GPC-Native 是运营与协同层主线。
8. GFIS 是生产与执行层主线。
9. Edge 是现场与 GFIS 之间的边缘接入层，不是业务主账。
10. AI 建议不是业务事实，不能直接写入业务主账。
11. 绿色供应链平台一期主线系统为 `GPC-Native`，宪法内容主要通过 `WAES` 的治理规则、证据纪律、状态门禁和 AI 授权进入总架构。
...
6. **平台主线纪律**：不把宪法扩写成大平台，而是把宪法下沉为 `WAES` 治理门，并让 `GPC-Native` 保持平台主线边界。
...
flowchart TB
  ...
  subgraph Business["业务流 Business Flow"]
    ...
    C["GPC-Native<br/>订单 / ASN / 预约 / 运输 / POD / 外部异常"]
  ...
  subgraph AI["AI 服务流 AI Service Flow"]
    ...
    X["XiaoC<br/>Prompt / Model / MCP"]
    A["Hermes / XGD<br/>Agent / 多端交互"]
...
| 可能误解 | 风险 | 优化口径 |
|---|---|---|
| WAES 发起项目等于 WAES 拥有全部业务配置 | 会把 WAES 写成超级业务系统 | PVAOS 管组织/门户，GPC-Native/GFIS 管业务配置，WAES 管项目治理编排和发布验证 |
...
| 库存调整 | 否 | 是 | GFIS |
| 发货出库 | 否 | 是 | GFIS |
| 客户签收、POD 争议处理 | 否 | 是 | GPC-Native |
| 维修验收 | 否 | 是 | GFIS |
| 客户交付承诺变更 | 否 | 是 | GPC-Native |
...
| 外部协同 | GPC-Native | 平台订单、ASN、预约、车辆、运输、POD、外部异常 |
| 工厂执行 | GFIS | 工厂订单、工单、齐套、质量、库存、批次、LES、EAM、发货出库 |
| 现场接入 | Edge | 现场信号采集、协议转换、缓存、回执，不是业务主账 |
| AI 交互 | Brain/XiaoC/Hermes/XGD | 知识、Prompt、Agent、交互，受 WAES 治理授权约束 |
...
PVAOS 组织和伙伴接入
-> WAES 供应链项目初始化和发布验证
-> GPC-Native 平台订单和外部协同
-> GFIS 工厂订单确认和生产执行
-> GFIS 质量、库存、批次、LES、EAM、发货出库
-> GPC-Native 运输、POD、外部异常
...
| 产能承诺 | GPC-Native 不知道 GFIS 是否可接单 | `CapacityCommitment`、`CapacitySnapshot` |
...
| 平台订单、ASN、预约、车辆、运输、POD、外部异常 | GPC-Native | 读取事件、证据和业务确认引用 |
...
| 知识条目、Prompt 模板、Agent 任务 | Brain / XiaoC / Hermes / XGD | 受 WAES 授权约束 |
...
| GPC-Native | GPC Connector -> API Gateway -> Outbox/Inbox -> Event Bus -> WAES | 订单、运输、POD、外部异常 |
...
| XiaoC/Hermes/XGD | Agent Connector -> WAES | Prompt、AgentTask、AISuggestion、AgentRun |
...
| Evidence Ledger | 证据捕获、回指、确证、驳回、归档 | 不替代 GFIS/GPC-Native 主账 |
...
| XiaoC | Prompt、ModelConfig、MCP、Agent 模板和评估 |
| Hermes/XGD | Agent 运行、桌面/语音/多端交互、后台任务 |
...
-> WAES 风险和上下文编排
-> Brain 提供 SOP、案例、知识
-> XiaoC 提供 PromptTemplate、ModelConfig、MCP 工具
-> Hermes/XGD 执行 AgentTask
-> 生成 AISuggestion
-> WAES 检查授权、证据和风险等级
-> GFIS/GPC-Native 内部业务确认
-> 业务系统生成执行回执
-> WAES 记录建议结果、证据和复盘
```
...
4. AI 只能通过 WAES 授权的工具调用读取或发起草案，不得绕过 GFIS/GPC-Native 的业务确认流程。
5. AI 建议被采纳后，业务事实仍由 GFIS/GPC-Native 生成，并由 WAES 记录 `BusinessApprovalReference`。
...
2. GPC-Native 替代 Odoo core 二开作为主线，降低长期维护风险。
...
| 对象 | 主责 | 用途 |
|---|---|---|
| FactoryAllocationRule | GPC-Native / WAES | 多厂分单规则 |
| FactoryAllocation | GPC-Native | 平台订单到工厂的分配结果 |
| CapacityCommitment | GFIS | 工厂对订单或时段的产能承诺 |
| CapacitySnapshot | GFIS | 工厂产能快照 |
| InventoryVisibilitySnapshot | GFIS / GPC-Native | 多厂库存可见性快照 |
| QualityTraceScope | GFIS / WAES | 跨厂或跨链质量追溯范围 |
| CrossFactoryException | GPC-Native / WAES | 多厂协同异常 |
...
| 主责系统 | GFIS、GPC-Native、PVAOS、WAES、Edge |
| 主责角色 | 业务负责人、执行人、治理负责人、证据负责人 |
| 输入对象 | 触发前必须存在的业务对象 |
| 输出对象 | 执行后必须生成的业务对象 |
| 触发事件 | 事件合同中的事件 |
| 执行步骤 | 步骤、责任人、时限、系统动作 |
| 业务确认点 | 由 GFIS/GPC-Native/PVAOS 内部完成 |
...
flowchart LR
  ...
  subgraph BIZ["业务流"]
    ...
    BGPC["GPC-Native<br/>PlatformOrder / Shipment / POD"]
  ...
  subgraph AI["AI 服务流"]
    ...
    AXiaoC["XiaoC<br/>PromptTemplate / ModelConfig"]
    AAgent["Hermes / XGD<br/>AgentRun / AgentToolGrant"]
...
sequenceDiagram
  ...
  participant GPC as GPC-Native 业务流
  ...
  participant AI as Brain/XiaoC/Hermes/XGD AI 服务流
  ...
  alt 建议需要业务确认
    WAES->>GPC: 引用建议，业务确认在 GPC-Native 内完成
...
| 问题 | 类型 | 影响 |
|---|---|---|
| GPC-Native 后端技术栈最终选 FastAPI 还是 NestJS | 待确认 | 影响接口、事件和团队维护方式 |
...
| 文档 | 目的 | 优先级 |
|---|---|---|
| `GPC-Native一期产品与技术蓝图.md` | 将 GPC-Native 从 ADR 推进到一期产品、对象、接口和服务设计 | P0 |
