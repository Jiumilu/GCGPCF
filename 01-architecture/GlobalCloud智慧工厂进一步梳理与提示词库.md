---
doc_id: GPCF-DOC-1DCB795545
title: GlobalCloud 绿色供应链体系进一步梳理与提示词库
project: GPCF
...
日期：2026-06-07
用途：在已有 V3.1 方案、项目群架构图、专项架构图和 GPC ADR 基础上，进一步梳理实施逻辑，并提供可直接用于 Codex/Agent 的提示词。原“智慧工厂”降级为生产与执行层/工厂执行子域。
当前口径：主架构采用治理与监控层、运营与协同层、生产与执行层；WAES 只做规则、监控、治理、证据、状态和 AI 授权，不参与工单、质量、库存、发货、签收等具体事务审批。

当前补充口径：平台主架构以 `GPC` 为主，宪法内容通过 `WAES` 治理约束进入，而不是反向替代平台主架构。
...
2. **项目群架构**：`基于GlobalCloud项目群的智慧工厂架构设计方案.md` 明确 GFIS、GPC、WAES、PVAOS、Brain、XiaoC（蚁后）、Hermes/XGD（大象）、XiaoG Agent、Harness 的分工。
3. **专项架构图**：`GlobalCloud智慧工厂专项架构图集.md` 补齐了统一数据与事件、GFIS-GPC-WAES 集成、OT/IT 安全、异常闭环与 AI 治理授权。
4. **关键 ADR**：`ADR-GPC从Odoo二开调整为原生公共服务平台.md` 明确 GPC 主线从 Odoo 二开调整为 GPC。
...
| 事件合同 | 没有事件合同就无法让 GFIS、GPC、WAES 集成 | `02-event-contracts.md` |
...
| GPC 一期模型 | GPC 需要从 ADR 进入对象/API/事件设计 | `05-gpc-phase1-model.md` |
...
5. ADR-GPC从Odoo二开调整为原生公共服务平台.md
6. GlobalCloud智慧工厂项目群控制表.md

核心架构口径：
- GFIS 是单厂本地执行事实源，负责生产、质量、库存、设备、厂内物流、发货出库事实。
- GPC 是轻量绿色供应链协同平台，负责供应商、客户、订单协同、TMS、签收回单、公共服务。
...
- XiaoC 是蚁后，负责 Prompt、MCP、模型路由、Agent 模板、任务拆解和结果汇总。
- Hermes/XGD 是大象承载层，负责长程 Agent、重分析、多端交互和运行回执；XiaoG Agent 是轻量执行入口。
...
3. 每个缺失项为什么会影响实施。
4. 每个缺失项建议补哪份文档。
5. 是否存在边界冲突，例如 GFIS 与 GPC、WAES 与业务主账/具体事务审批、Brain 与运行事实、AI 与治理授权。
...
### Prompt 02：GPC 一期产品与技术蓝图

```text
请基于当前智慧工厂设计，进一步设计 GPC 一期产品与技术蓝图。
...
- GPC 不做工厂生产执行、不做库存主账、不做质量放行主账、不直接改 GFIS 事实。
- GPC 负责供应商、客户、平台订单、样品申请、客户签样、转量产、预约、车辆、承运商、在途、签收、回单、外部异常。
...
要求：
- 输出成可保存为 `architecture/05-gpc-phase1-model.md` 的 Markdown。
- 所有对象必须说明主责系统。
- 所有写入动作必须说明是在 GFIS/GPC 内部业务确认，还是需要 WAES/Harness 治理授权。
```

### Prompt 03：GPC 对象模型和数据库表

```text
请为 GPC 设计一期对象模型和数据库表。

业务范围：
- 平台订单
- 样品申请 / 客户签样 / 转量产
- 已签样平台订单到 GFIS 工厂订单映射
- 供应商
- 客户
- ASN 到货通知
- 到货/发货预约
...
请输出：
1. 当前 Odoo GPC 中哪些文档、脚本、测试、对象概念可作为 GPC 输入。
...
### Prompt 06：GFIS 到 GPC 发货与签收闭环

```text
请设计 GFIS 与 GPC 的发货、运输、签收、回单闭环。
...
```text
请设计 GFIS、GPC、WAES、PVAOS、Brain、XiaoC、Hermes/XGD（大象） 之间的 Connector Registry 和 API Gateway。
...
3. 可自动执行的 L1/L2 动作。
4. 只能生成建议的 L3 动作。
5. 必须治理授权的 L4 动作，以及必须回到 GFIS/GPC 的事务确认动作。
6. 永远禁止的 L5 动作。
7. 输出格式。
8. 证据引用格式。
9. 失败时如何报告。
...
7. 证据来源。
8. 阻塞条件。
9. 确认点，必须区分 GFIS/GPC 业务确认和 WAES/Harness 治理确认。
10. 完成判定。
...
### Prompt 18：启动 GPC 新项目

```text
请为 GPC 启动一个新项目设计和脚手架方案。
