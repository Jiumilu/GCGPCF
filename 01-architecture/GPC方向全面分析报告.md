...
治理与监控层 (WAES)
  ...
  ├── 不审批具体事务
  └── 主系统: WAES / Harness / Brain / XiaoC / Hermes / XGD

运营与协同层 (GPC-Native / PVAOS)
  ├── 跨企业协同、订单池、TMS、POD、外部异常
  ├── 不做工厂生产执行、库存主账、质量放行
  └── 主系统: GPC-Native（轻量协同中台）+ PVAOS（生态入口）
...
| S3 正式运营 | 真实 SOP 执行、事件、证据、复盘 | GPC-Native + GFIS |
...
第三阶段: GPC-Native（轻量协同中台） ← 当前官方方向
  → 仓库: 尚待建立
  → 方向: FastAPI/NestJS + React + Postgres 的原生协同中台
```

### 2.2 关键 ADR 结论

> **"GPC 从 Odoo 二开调整为原生协同中台"**（2026-06-07）

| 决定 | 内容 |
|------|------|
| 冻结 Odoo GPC | 不再新增 core 二开 |
| Odoo 降级 | 历史原型 + 流程验证样本 + 可选 back-office connector |
| 主线切换 | 新建 `GPC-Native` 轻量协同中台 |
...
| 资产 | 评估 |
|------|------|
| README.md（刚才改的） | 内容已错——GPC 不是供应链平台，GPC-Native 才是 |
| gpc_sop_engine/（刚才建的） | 方向正确但归属错——应归 GPC-Native，不归废弃的 GFIS 仓库 |
| index.html / app.js | 历史演示，无继续维护价值 |
| gcfis_custom/ | ERPNext 适配器——应归 GFIS 仓库，不是 GPC 仓库 |
| docs/ 44篇 | GFIS 历史文档，与 GPC-Native 无关 |
| scripts/ 36个 | GFIS 验证脚本，与 GPC-Native 无关 |
...
**关键判断**：SOP 引擎不是 GPC-Native 的一部分。SOP 的**管理、版本、发布、治理**归 WAES，SOP 的**执行**发生在 GPC-Native（协同层）和 GFIS（执行层）。
...
/Projects/GlobalCloud GPC-Native    ← 新建
  → FastAPI/NestJS + React/Vite
  → 轻量协同中台（订单/TMS/POD/异常）
  → 独立仓库，不继承 GFIS 代码

/Projects/GlobalCloud GPC           ← 废弃或归档
  → 移出 gpc_sop_engine/ 到 GPC-Native
...
| 我刚才做的事 | 评估 | 调整建议 |
|------------|------|---------|
| 改 README 为 GPC | ⚠️ 方向偏了 | 应改为 GPC-Native 介绍，不是 GPC |
| 改 index.html 品牌 | ✅ 可行但不紧急 | 历史演示可保留但不作为主线 |
| 建 gpc_sop_engine/ | ✅ 内容正确，但位置错 | 迁移到 GPC-Native 或 WAES 仓库 |
| 12段全局 SOP 库实例 | ✅ 质量好 | 保留，归 WAES 侧 SOP 治理 |
| 葛化 SOP 映射 | ✅ 有真实参考价值 | 保留，可作为 GFIS 适配器参考 |
| 虚拟工厂原型 | ✅ 方向正确 | 细化后归 GPC-Native + WAES |

### 原来 GPC 方案中需要修正的内容

| 原来方案中的内容 | 问题 |
|-----------------|------|
| "GPC 不是 GFIS 的升级版" | 没说明 GPC 是 GPC-Native |
| 三层架构（供应链基础服务/虚拟工厂/适配器） | 对标 GPC-Native 而不是 GFIS |
...
**GPC-Native 是轻量协同中台，不是重平台。** 它不是 GFIS 的替代，也是不是 Odoo 的替代。它是一个独立的、专注于跨企业协同的中间层系统。SOP 引擎和虚拟工厂是其治理层（WAES）和执行层（GPC-Native + GFIS）协同的结果。

### 7.2 下一步做什么

根据模块分级判定表，P0 最小闭环中需要 **全面重构** 的两个模块就是 GPC-Native 的起点：

1. **平台订单 (PlatformOrder)** — 当前分级：全面重构
2. **签收 (ProofOfDelivery)** — 当前分级：全面重构

**建议第一步**：在新建的 GPC-Native 仓库中，从这两个对象开始建立最小服务原型。

### 7.3 六个专项中的"链同专项"

链同专项（PVAOS + GPC 协同域）已经完成了只读预检，下一阶段是首轮实施前验证。这正是 GPC-Native 应该纳入的专项轨道。
...
| 问题 | 答案 |
|------|------|
| GPC 的正确方向是什么？ | GPC-Native — 轻量协同中台 |
| 当前 `GlobalCloud GPC` 仓库是什么？ | 废弃的历史原型（原 Odoo 路线） |
| SOP 引擎归哪里？ | WAES 治理解 + GPC-Native/GFIS 执行引用 |
| 虚拟工厂归哪里？ | S1-S2 的 ProjectTopology |
| 下一步做什么？ | 新建 GPC-Native 仓库，从 PlatformOrder + POD 开始 |
| 和 GFIS 是什么关系？ | GPC-Native 协同层 ↔ GFIS 执行层，事件驱动 |
| 和 WAES 是什么关系？ | WAES 治理约束 → GPC-Native 业务执行 |
