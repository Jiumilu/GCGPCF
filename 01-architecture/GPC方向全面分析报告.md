...
| SampleRequest | 样品申请、客户签样、转量产门禁 | P0 |
| OrderMapping | 已签样或已转量产的平台订单 → GFIS 工厂订单映射 | P0 |
...
根据模块分级判定表，P0 最小闭环中需要 **全面重构** 或独立门禁收口的模块就是 GPC 的起点：

1. **平台订单 (PlatformOrder)** — 当前分级：全面重构
2. **样品确认 (SampleRequest / SampleApproval / ProductionRelease)** — 当前分级：二次开发，必须作为平台订单与工厂订单之间的独立门禁
3. **签收 (ProofOfDelivery)** — 当前分级：全面重构

**建议第一步**：在新建的 GPC 仓库中，从平台订单、样品确认和签收三个对象开始建立最小服务原型。
