...
owner: KDS
kds_space: 开发
kds_path: 开发/05-KDS/docs/harness/evidence/globalcloud-project-group-wave1-execution-command-pack-20260626.md
...
| `WAVE1-GPCF-POST-SCHEME-REVIEW-001` | GPCF/all projects | 将 17 仓 scheme recognition review 转成逐仓授权执行入口 | `python3 tools/kds-sync/validate_project_group_post_scheme_recognition_review_authorization_request_20260626.py`、`python3 tools/kds-sync/validate_project_group_dirty_disposition_queue_post_scheme_recognition_20260626.py`、`python3 tools/kds-sync/validate_project_group_current_state_baseline_refresh_20260626.py`、`python3 tools/kds-sync/loop_document_gate.py` | `docs/harness/evidence/globalcloud-project-group-post-scheme-recognition-review-authorization-request-20260626.md`、逐仓 review 授权回执 | post-scheme review authorization gate、dirty queue gate、current baseline refresh gate、Git clean gate、Loop document gate | 不自动 review、stage、commit、push、delete、cleanup；任一仓缺确认则保持 `review_allowed=false` | 是，任何 review/stage/commit/push/delete 前必须逐仓确认 | 不声明项目群 Git 全量 clean、可提交、可推送、accepted、integrated 或客户验收 |

## 4. 执行顺序

| 顺序 | 动作 | 控制 |
|---|---|---|
| 1 | 复跑当前状态基线刷新验证器 | 确认 17 项目状态和 Git live 边界仍受控 |
| 2 | 用户按 pack_id 明确授权 | 未授权 pack 的所有动作保持 `false` |
| 3 | 登记授权回执 | 写入授权回执总账，不扩大授权范围 |
| 4 | 执行对应 pack 命令 | 只运行被授权 pack 的命令，不跨项目联动执行 |
| 5 | 写入执行证据 | 证据必须包含命令、结果、失败项、边界和回滚说明 |
| 6 | 复跑门禁 | 总控板、目标覆盖审计、Loop 文档门禁、Git clean 门禁 |
...
| `GPCF -> all projects` | 17 仓 dirty 未逐仓确认前，不得声明项目群 Git clean 或可提交 |
