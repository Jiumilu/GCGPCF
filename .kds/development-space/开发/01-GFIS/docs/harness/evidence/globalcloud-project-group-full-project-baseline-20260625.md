...
owner: GFIS
kds_space: 开发
kds_path: 开发/01-GFIS/docs/harness/evidence/globalcloud-project-group-full-project-baseline-20260625.md
...
| 8 | GlobalCoud GPCF | `controlled / git_clean_blocked / human_confirmation_request_prepared` | `implementation_plan_exists` | `dirty / governance_package_ready / live_sensitive_path_blocked` | 维护总控板、核心台账、全量基线和人工确认请求；等待逐包授权 | `validate_project_group_full_project_baseline.py`、`loop_document_gate.py` | 回滚本轮治理文档和 validator；失败最高 `partial/rework` | 不声明项目群 Git 全量 clean、可提交、可推送或已验收 |
