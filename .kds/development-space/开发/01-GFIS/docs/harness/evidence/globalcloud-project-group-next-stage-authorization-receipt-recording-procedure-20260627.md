...
本文只定义用户明确确认后，如何把当前 active 的 `GPCF/GFIS/SOP` next-stage 决策项从“示例回执”安全地录入到对应总账。本文不生成真实回执，不更新任何 ledger 行，不执行 review、cleanup、stage、commit、push、deploy、release 或真实 KDS API 同步。
...
|---|---|---|---|
| A-C：3 仓 Pre-Wave1 review 边界 | `AUTH-GPCF-SCHEME-REVIEW-20260626`、`AUTH-GFIS-SCHEME-REVIEW-20260626`、`AUTH-SOP-LOOP-GATE-DELEGATE-REVIEW-20260627` | `globalcloud-project-group-next-stage-authorization-receipt-example-pack-20260627.md` 中 A-C 对应示例 | `globalcloud-project-group-post-scheme-recognition-authorization-receipt-ledger-20260626.md` |
...
| 3 | 写入总账 | 当前 active 3 项均写入 post-scheme receipt ledger |
...
| 6 | 状态传导 | 只允许把当前 active 3 项传导到对应 review boundary；不得直接传导到 Wave 1 |
...
| `authorized_action` | 否 | 固定 `human_review_and_conclusion_registration_only` |
...
|---|---|---|
| `AUTH-GPCF-SCHEME-REVIEW-20260626` / `AUTH-GFIS-SCHEME-REVIEW-20260626` / `AUTH-SOP-LOOP-GATE-DELEGATE-REVIEW-20260627` | post-scheme recognition authorization receipt ledger | 不允许直接写成 `AUTH-WAVE1-*` 已授权 |
...
| 方向 | 控制说明 |
|---|---|
| run | 定义当前 `GPCF/GFIS/SOP` 3 个 next-stage 决策项从示例到真实回执的安全录入流程 |
