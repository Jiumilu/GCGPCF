...
| 4 | `KDS -> Brain` | GlobalCloud KDS | GlobalCloud Brain | `ready_for_review_to_human_review / git_sensitive_review_boundary / pre_wave1_pending` | `pre_wave1_review_bridge` | `docs/harness/KDS/evidence/kds-rag-export-repair-20260625.md`、`docs/harness/evidence/globalcloud-project-group-pre-wave1-review-authorization-request-20260627.md` | `BRAIN-HUMAN-REVIEW-DECISION-001` | KDS RAG export gate、pre-wave1 review authorization gate、Brain review handoff gate、human review gate | KDS 当前属于 `GlobalCoud GPCF / GlobalCloud GFIS / GlobalCloud KDS` 三仓 review 边界之一，且 2026-06-28 live recheck 继续确认 `.env.production.example` sensitive_path；在 KDS hold review 和 Pre-Wave1 未确认前，不得进入 Wave 1 Brain 审查回执 | 人工未确认且 sensitive_path 未归类前，Brain 保持 `ready_for_review / authorization_boundary` | 是，KDS sensitive_path review、Brain accepted/integrated 前 | 不声明 Brain accepted、integrated、production_ready |
...
| 优先级 | 依赖边 | 下一步 |
|---|---|---|
| P0 | `GPCF -> all projects` | 先完成 `GPCF-PRE-WAVE1-REVIEW-AUTHORIZATION-REQUEST-20260627-001`，再处理 `GPCF-NEXT-STAGE-AUTHORIZATION-HUMAN-FILL-REQUEST-20260627-001` / `GPCF-NEXT-STAGE-AUTHORIZATION-CHAIN-CONSISTENCY-AUDIT-20260627-001`，同步收口 KDS sensitive_path review 和当前 `GPCF/GFIS/KDS` 3 仓 review 边界，之后再进入逐包 review/stage/commit/push 判断 |
