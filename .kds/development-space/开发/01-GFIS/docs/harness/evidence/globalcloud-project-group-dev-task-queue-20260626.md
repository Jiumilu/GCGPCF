...
- 本队列保留 2026-06-26 初始开发态任务队列；当前执行判断必须以后续 2026-06-28 live override 为准。当前项目群 Git gate 为 `partial`，dirty 仓为 `GlobalCoud GPCF`、`GlobalCloud Brain`、`GlobalCloud SOP`，KDS 已从当前 dirty/sensitive 阻塞源移除；执行任何项目任务前仍必须复跑 17 仓 Git gate，以当次输出为准。
...
| GlobalCoud GPCF | `authorization_to_pre_execution_total_bridge` | `GPCF -> all projects` | `GPCF-PRE-WAVE1-REVIEW-AUTHORIZATION-REQUEST-20260627-001` | GPCF 当前开发态先过 `GPCF/Brain/SOP` 三仓 review 桥接入口；KDS 不再属于当前 active dirty/sensitive 边界 |
...
| GlobalCloud GFIS | `source_record_boundary` | `GFIS/GPC/PVAOS -> SCaaS` | `AUTH-GFIS-SCHEME-REVIEW-20260626` | GFIS 当前开发态保持 SOR 缺口边界，不进入真实业务链；当前 live Git 已 clean，不再属于 active dirty review 边界 |
...
| P0 | GlobalCoud GPCF | live dirty=current / diff=pass / git_gate_partial | 继续拆分治理/evidence/KDS mirror 大工作区，并先通过 `GPCF/Brain/SOP` 三仓 review 授权入口 | `python3 tools/kds-sync/loop_document_gate.py --check-only` |
...
| P0 | GlobalCloud GFIS | clean / diff=pass / repair_required | 保持真实 source-of-record 缺口边界，当前不作为 active dirty review 仓 | `git status --short --untracked-files=all && git diff --check -- .` |
...
5. P0-E：已建立四层授权矩阵；当前 `REVIEW-AUTH` 的第一入口已切到 `GPCF-PRE-WAVE1-REVIEW-AUTHORIZATION-REQUEST-20260627-001`。`REVIEW-AUTH-GPCF-WORKTREE-20260627` 仍保留为后续 GPCF review 包入口，但位于 `GPCF/Brain/SOP` 三仓 review 边界之后。
...
7. P0-G：已建立 `docs/harness/evidence/globalcloud-project-group-pre-wave1-review-authorization-request-20260627.md`；2026-06-28 live override 后，当前 active review 边界收口为 `GPCF/Brain/SOP` 三仓，KDS/AAAS/XWAIL/GFIS 已从当前 dirty/sensitive 阻塞源移除；任何 Wave 1 pack、GPCF worktree review、Git 动作仍不得绕过当前三仓边界。
8. P0-H：已建立 `docs/harness/evidence/globalcloud-project-group-next-stage-authorization-package-20260627.md`；2026-06-28 live override 后，当前 next-stage 授权包只允许围绕 `GPCF/Brain/SOP` 三仓 review 边界做人工确认，不允许真实回执落账。
...
| GlobalCloud Brain | `dirty=46 / diff_check=pass / human_review_boundary` | `BRAIN-HUMAN-REVIEW-DECISION-001` | `git status --short --untracked-files=all`、`npm run build -- --mode development`、`python3 tools/kds-sync/validate_brain_review_handoff.py` |
| GlobalCloud SOP | `dirty=1 / diff_check=pass / owner_review_required` | `AUTH-SOP-LOOP-GATE-DELEGATE-REVIEW-20260627` | `git status --short --untracked-files=all`、`python3 tools/kds-sync/loop_document_gate.py --check-only`、`python3 tools/kds-sync/validate_sop_scenario_owner_review.py` |
| GlobalCloud GFIS | `clean / diff_check=pass / repair_required` | `GFIS-RUNTIME-SOP-E2E-MIN-001` | `git status --short --untracked-files=all`、`python3 tools/kds-sync/validate_gfis_real_fact_entry_gate.py`、`python3 tools/kds-sync/validate_loop_v11_gfis_authorization_boundary.py` |
