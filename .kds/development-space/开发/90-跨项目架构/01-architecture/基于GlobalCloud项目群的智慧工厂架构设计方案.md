...
1. **生产与执行层：GlobalCloud GFIS + Edge**
   作为单厂本地运行第一执行系统，承载工厂订单、工单、生产、质量、库存、仓储、设备、厂内物流、发货出库和本地运行事实。GFIS 是智慧工厂 V3.1 中 MES/MOM、QMS、WMS 初始能力、EAM 初始能力和工厂本地控制台的核心落点；客户签收/POD 主账归 GPC。

2. **运营与协同层：PVAOS + GlobalCloud GPC**
   GPC 作为绿色供应链公共服务平台本体，承载平台订单、需求池、供应商协作、产能协同、工厂分配、ASN、预约、运输、POD、异常协同、绿色绩效、合规证据包、服务开放和生态接入。PVAOS 提供租户、组织、伙伴、用户、权限、门户和项目空间。GPC 不基于 Odoo core 二开，不替代工厂本地执行事实，应读取或接收 GFIS 的确认事件。现有 Odoo GPC 降级为历史原型、流程样本或可选 back-office connector。
...
1. **事实归属清楚**：生产、质量、库存、设备、发货出库事实归 GFIS；平台订单、运输、签收、POD 和外部协同事实归 GPC；知识事实归 Brain；治理规则、指标、状态和证据归 WAES。
...
| FactoryOrder | `FO-*` | GFIS | 工厂订单主账；与平台订单建立映射 |
...
| 验收项 | 主责系统 | 必须证据 |
|---|---|---|
| 一张平台订单完整走完生产、发货、运输、签收 | GPC / GFIS / WAES | 平台订单、工厂订单、工单、质检、发货出库、运输、POD、证据记录 |
