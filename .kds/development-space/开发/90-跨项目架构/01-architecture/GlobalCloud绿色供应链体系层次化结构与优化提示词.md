---
doc_id: GPCF-DOC-424D73E236
title: GlobalCloud 绿色供应链体系层次化结构与优化提示词
project: GPCF
...
| GPC 公共服务平台 | GPC 绿色供应链协同平台 | 不再基于 Odoo 二开，聚焦外部协同、TMS、POD、公共服务 |
| GFIS 工厂信息化系统 | 工厂执行与本地运行核心 | 继续作为单厂事实源，承载 MES/QMS/WMS/LES/EAM |
| WAES 控制塔 | 治理与监控中枢、证据平面 | 聚合 GFIS/GPC/PVAOS/Brain/Agent 证据，审批规则和治理事项，不审批具体事务，不写业务主账 |

当前补充口径：平台主架构以 `GPC` 为绿色供应链平台主线，宪法内容通过 `WAES` 的规则、证据、状态和授权体系进入。
...
flowchart TB
  subgraph GOV["治理与监控层"]
    ...
    Agent["Hermes / XGD（大象） / XiaoG Agent<br/>Agent任务 / 多端交互 / 告警入口"]
  end

  subgraph OPS["运营与协同层"]
    PVAOS["PVAOS<br/>组织 / 项目 / 伙伴 / 门户接入"]
    GPC["GPC<br/>订单协同 / ASN / 预约 / 运输 / POD / 外部异常"]
...
| 层级 | 主责 | 代表系统 | 不做什么 |
|---|---|---|---|
| 治理与监控层 | 规则、监控、治理、证据、状态、AI 授权、复盘 | WAES / Harness / Brain / XiaoC / Hermes / XGD（大象） | 不审批具体事务，不写业务主账 |
| 运营与协同层 | 生态接入、订单协同、外部物流、客户/供应商/承运商协同 | PVAOS / GPC | 不做工厂生产执行，不做库存和质量主账 |
...
- Edge 不决定工单、质量、库存、发货、签收和治理状态；业务事实必须进入 GFIS 或 GPC 后形成正式记录。
...
| 业务流 | PVAOS / GPC / GFIS / Edge | 组织接入、订单协同、工厂执行、现场接入 | 业务事实必须由主责系统产生 |
| 数据流 | Connector Registry / API Gateway / Event Bus / Evidence Ledger | API、事件、Outbox/Inbox、证据、指标、Trace、DLQ、重放 | 不共享业务数据库，不改变事实归属 |
| AI 服务流 | Brain / XiaoC / Hermes / XGD（大象） / WAES | 知识、Prompt、Agent、建议、摘要、预警、复盘 | 受 WAES 授权，不能直接写业务主账 |
...
flowchart TB
  ...
  subgraph L2["L2 供应链协同层"]
    GPC["GPC<br/>绿色供应链公共服务平台本体<br/>订单协同 / ASN / 预约 / TMS / POD / 公共服务"]
    Odoo["Odoo GPC 原型<br/>历史样本 / 可选 back-office connector"]
  end
  ...
  subgraph L5["L5 控制塔与智能层"]
    ...
    XiaoC["XiaoC（蚁后）<br/>Prompt / MCP / 模型路由 / Agent模板 / 任务拆解"]
    Agent["XiaoG Agent / Hermes / XGD（大象）<br/>持续运行 Agent / 桌面 / 语音 / 社交入口"]
  end
...
- XiaoC（蚁后）提供 Prompt、MCP、模型路由、Agent 模板、任务拆解和结果汇总。
- Hermes/XGD（大象承载层）提供持续运行 Agent、长程上下文、重分析和多端交互；XiaoG 作为轻量执行入口。
...
| 子域 | 主责系统 | 说明 |
|---|---|---|
| 生态接入域 | PVAOS / GPC Portal | 组织、项目、伙伴、客户、供应商、承运商入口 |
| 供应链协同域 | GPC | 订单协同、ASN、预约、TMS、POD、外部异常 |
| 工厂执行域 | GFIS | 生产、质量、库存、厂内物流、设备、发货出库 |
| 数据事件域 | WAES + Connector + Event Bus | 对象、事件、指标、证据、追踪 |
| 知识与 SOP 域 | Brain | SOP、案例、复盘、RAG 准入 |
| Prompt 与模型域 | XiaoC（蚁后） | Agent prompt、MCP、模型配置、任务拆解、路由、评估 |
| Agent 承载域 | Hermes / XGD（大象） | 长程 Agent、重分析、后台任务、桌面、语音、社交、异常提醒 |
...
- WAES 不负责工单、质量、库存、发货、签收等具体事务审批，只引用 GFIS/GPC 的业务确认结果。
- Brain 负责知识事实。
- AI 建议不是业务事实。
```

### Prompt C：GPC 绿色供应链公共服务平台深化

```text
请深化 GPC 轻量绿色供应链协同平台方案。
...
- GPC：平台订单、ASN、预约、车辆、运输、签收、POD、外部异常。
- WAES：治理规则、证据、指标、状态审计、AI 建议状态和业务审批引用。
- PVAOS：组织、项目、伙伴接入。
- Brain/XiaoC（蚁后）/Hermes/XGD（大象）：知识引用、prompt 版本、Agent 建议和执行记录。
...
6. 证据类型。
7. 来源系统。
8. 确认点，必须区分 GFIS/GPC 事务确认和 WAES/Harness 治理确认。
9. 阻塞条件。
10. 完成判定。
...
2. `GlobalCloud绿色供应链体系事件合同.md`：已建立事件信封、GFIS/GPC/WAES/PVAOS/Brain/Agent 一期事件、幂等和补偿规则。
...
2. `GPC一期产品与技术蓝图.md`：细化订单协同、ASN、预约、TMS、POD、外部异常和公共服务的一期模型。
