...
以下停止条件适用于普通 Loop。若当前处于 L3/L3.5/L4/L5 连续运行模式，阶段性汇报、完成单轮、完成两轮、完成一个小任务、完成一个局部验证、生成报告或生成下一轮建议不得作为停止条件。连续运行模式只能因轮次预算耗尽、时间预算耗尽、对应硬停止、用户明确暂停/停止、授权边界闭合或任务队列为空而停止。
