---
doc_id: GPCF-LOOP-UI-STUDIO-WORKBENCH-007
title: Loop Round GPCF-UI-STUDIO-WORKBENCH-007
project: GPCF
...
owner: GPCF
kds_space: 开发
kds_path: 开发/12-GPCF/docs/harness/loops/loop-round-GPCF-UI-STUDIO-WORKBENCH-007.md
source_path: docs/harness/loops/loop-round-GPCF-UI-STUDIO-WORKBENCH-007.md
...
# Loop Round GPCF-UI-STUDIO-WORKBENCH-007
...
1. 仅在 `chat` 主界面导航组件中补齐 `Jobs` 可见入口。
2. 扩展组件级测试，验证 `Jobs` 入口显示条件与跳转行为。
3. 在真实运行态中复验 `chat -> jobs`。
4. 在真实运行态中复验 `jobs -> chat`。
5. 记录默认凭据提醒弹窗对聊天主界面入口点击的运行态干扰。
...
- `docs/harness/evidence/studio-jobs-chat-entry-verified-20260622.json`
- `docs/harness/evidence/assets/studio-chat-jobs-entry-20260622.png`
- `docs/harness/evidence/assets/studio-jobs-from-chat-entry-20260622.png`
...
1. `Jobs` 入口是否只出现在 `chat` 主界面。
2. 点击入口是否进入 `Jobs`。
3. 点击 `Jobs` 页 `返回` 是否能回到 `chat`。
4. 默认凭据提醒弹窗是否会干扰主界面入口点击。

## 反馈

本轮目标已经完成，而且继续保持了最小改动：

1. `chat` 主界面新增了 `Jobs` 可见入口。
2. `chat -> jobs -> chat` 双向导航已完成真实复验。
3. 没有扩改 `Jobs` 页面主体逻辑。
4. 运行态额外发现：默认凭据提醒弹窗会挡住聊天主界面入口点击，但这不是 `Jobs` 导航缺陷。
...
| Scope | `chat` 主界面补齐 `Jobs` 可见入口，并验证双向导航 |
