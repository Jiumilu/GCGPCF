...
title: GPCF L4 GFIS Repair 029 运行时原料修复候选
project: GPCF
...
# GPCF L4 GFIS Repair 029 运行时原料修复候选
...
- 上一轮已为 `production_execution` 创建 candidate-only 修复项。
...
- GFIS runner 现在为 `raw_material_plan` 创建第二个 `GFISActionableRepairCandidate`。
- GFIS 运行时验证器现在要求运行时 evidence 同时包含 `production_execution` 和 `raw_material_plan` 修复候选调用。
- GFIS API contract 验证器现在覆盖原料修复候选行为。
- GPCF 完整性验证器现在读取 GFIS 运行时 evidence JSON，并检查两个候选缺口。
...
- 不进行生产写入。
- 不进行真实外部 API 写入。
- 不创建库存、采购收货、来料 QA 提交、WorkOrder 完成、WAES/KDS 最终写入、POD、财务确认、accepted 或 integrated 状态。
- Demo 仍仅作为显示回归验证。

## 结果

项目现在具备自诊断后的另一个真实 Loop Engineering 自修复步骤：`raw_material_plan` 可以转换为受控 GFIS 运行时修复候选并完成清理。完整 SOP E2E 仍保持 `repair_required`，项目群评分继续冻结在 79/100。

## 下一步

继续处理 `get_runtime_sop_gap_resolution_plan` 中另一个 GFIS 负责的可执行缺口，例如 `raw_material_batch` 或 `incoming_quality_inspection`，或为外部依赖缺口收集已验证真实业务输入。
