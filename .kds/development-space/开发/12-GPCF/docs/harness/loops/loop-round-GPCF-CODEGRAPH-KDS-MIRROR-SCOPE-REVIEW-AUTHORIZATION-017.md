...
- 动作：复核 KDS live CodeGraph 与 Git 状态，发现当前 dirty 与 pending 均已清零。
- 输出：
...
- stop_type：`current_state_clean`
- 停止证据：KDS 当前 Git dirty=0 且 CodeGraph pending=0；无需 scope review 授权。
...
- 可重试动作：重新只读采集 KDS status；如 dirty 重新出现，再生成 KDS scope review 授权包。
...
- 当前阻塞：Brain/GFIS 仍未授权；KDS 当前仅需 monitor。
