---
doc_id: GPCF-DOC-8C7F2F5002
title: Cognee P4 真实写入前置预检 Evidence
project: GPCF
...
owner: GPCF
kds_space: 开发
kds_path: 开发/12-GPCF/docs/harness/evidence/cognee-p4-real-writeback-precheck-20260623.md
source_path: docs/harness/evidence/cognee-p4-real-writeback-precheck-20260623.md
...
# Cognee P4 真实写入前置预检 Evidence

## 1. 运行方式

```bash
authority_file=fixtures/cognee/cognee-p4-real-writeback-precheck-template.json
python3 loop/context/cognee/scripts/run-cognee-p4-real-writeback-precheck.py \
  --input "$authority_file" \
  --output-json docs/harness/evidence/cognee-p4-real-writeback-precheck-20260623.json

python3 loop/context/cognee/scripts/validate-cognee-p4-real-writeback-precheck.py \
  --input docs/harness/evidence/cognee-p4-real-writeback-precheck-20260623.json
```

> 说明：本文件用于 P4 真实写回前置矩阵，仍属于预检/受控预演，不构成生产写入闭环。

## 1.1 P4 预检结果（更新于 2026-06-23）
...
- owner_authorization_presence_rate: `0.8`
- waes_pass_rate: `1.0`
- runtime_dependency_ok_rate: `0.8`
- rollback_readiness_rate: `0.8`
- authorization_token_source_coverage: `0.8`
...
> 该轮判定说明：目前 P4 预检可闭合真实写入前置门禁基线，存在 `runtime_dependency_missing` 与 `authorization_absent` 两类阻断示例，验证了阻断路径。
...
- records: 与 `fixtures/cognee/cognee-p4-real-writeback-precheck-template.json` 的对齐结果
- summary:
  - `record_count`
