---
doc_id: GPCF-DOC-1362AFC964
title: GlobalCloud 绿色供应链体系全局初始化 SOP 方案
project: KDS
...
owner: KDS
kds_space: 开发
kds_path: 开发/05-KDS/03-data-ai-knowledge/GlobalCloud绿色供应链体系全局初始化SOP方案.md
source_path: 03-data-ai-knowledge/GlobalCloud绿色供应链体系全局初始化SOP方案.md
...
8. **平台主架构优先**：绿色供应链平台架构以 `GPC` 为业务主线，宪法内容通过 `WAES` 的规则、证据、状态和授权门禁进入，不直接替代平台业务编排。

关键边界：

- WAES 审批或确认的是治理事项：项目模板、规则、指标、AI 授权、Agent 工具权限、证据确证、状态升级、SOP 发布、连接器上线和阶段验收。
- WAES 不审批工单、质量放行、库存调整、发货、签收、维修验收和客户交付承诺。
- 具体业务确认留在 GFIS 或 GPC，WAES 只记录 `BusinessApprovalReference` 和 `EvidenceRecord`。
...
| 运营与协同层 | PVAOS/GPC 建组织、伙伴、供应商、客户、承运商、平台对象 | GPC 配订单、ASN、预约、TMS、POD、外部异常 SOP | GPC 形成真实订单、运输、签收、异常事实 | GPC 向 WAES 输出事件和业务确认引用 |
...
| S3 正式运营 | 控制塔、证据平面、复盘台账 | 看板、日报、异常复盘、证据归档、AI 建议治理、状态审计 | 不替代 GFIS/GPC 写业务主账 |
...
| SupplyChainPlatformInstance | `SPI-*` | GPC / WAES | 一个供应链平台实例 |
| FactoryInstance | `FIN-*` | GFIS / WAES | 一个工厂实例在项目中的注册 |
| ExternalPartyBinding | `EPB-*` | PVAOS / GPC | 外部客户、供应商、承运商、监管、园区或第三方协同对象绑定 |
...
| ObjectMappingProfile | `OMP-*` | WAES / GPC / GFIS | 业务对象映射配置 |
...
| 供应链平台 | GPC 实例、租户、角色、外部门户 | SupplyChainPlatformInstance | 平台连接器可用 |
| 工厂配置 | GFIS 实例、工厂、车间、仓库、设备、Edge | FactoryInstance | 工厂连接器可用 |
| 外部组织 | 客户、供应商、承运商、监管、园区 | ExternalPartyBinding | PVAOS/GPC 对象一致 |
...
| SOPExecution | `SOPE-*` | GFIS / GPC / WAES | SOP 执行实例 |
| SOPCheckpoint | `SOPC-*` | WAES / GFIS / GPC | 业务确认点或治理确认点 |
...
|---|---|---|---|
| 订单到交付 | GPC / GFIS | 证据链、指标、风险、日报 | 订单、工单、质检、发货、运输、POD 均有来源记录 |
| 批次追溯 | GFIS / GPC | 追溯视图和证据回指 | 供应商、ASN、批次、工单、成品、客户可串联 |
...
| 运输签收异常 | GPC / WAES | 异常看板、证据归档、业务审批引用 | GPC 完成争议处理，WAES 只引用结果 |
...
| 规则生效 | 管治理规则、风险规则、证据规则 | 不改 GFIS/GPC 主账 |
...
| I3 | 配置 GFIS/GPC/PVAOS/Edge 连接器 | S1 | A1-A9 的集成前置 |
...
| GPC | 供应链平台与外部协同 | 平台订单、ASN、预约、运输、POD、外部异常 | 订单、运输、签收、异常、业务确认引用 | 不做生产工单、库存主账、质量放行 |
...
| L3 建议草案 | 生成建议，需业务系统确认 | Hermes / XiaoC / Brain / GFIS / GPC | 补料建议、CAPA 草案、运输异常建议 |
...
| 证据 | 来源 | 用途 |
|---|---|---|
| BusinessRecord | GFIS / GPC / PVAOS | 证明业务事实 |
| DomainEvent | 主责系统 / Event Bus | 证明事实已发布 |
| BusinessApprovalReference | GFIS / GPC | 证明具体事务已由业务系统确认 |
...
flowchart TB
  ...
  subgraph OPS["运营与协同层"]
    ...
    GPC["GPC<br/>平台订单 / ASN / 预约 / TMS / POD / 外部异常"]
...
3. GPC 和 GFIS 的部署目标是否由 WAES 直接编排，还是只生成配置包交给运维执行。
