---
doc_id: GPCF-DOC-1A3581D521
title: GlobalCloud 绿色供应链体系对象目录
project: KDS
...
owner: KDS
kds_space: 开发
kds_path: 开发/05-KDS/03-data-ai-knowledge/GlobalCloud绿色供应链体系对象目录.md
source_path: 03-data-ai-knowledge/GlobalCloud绿色供应链体系对象目录.md
...
1. **GPC** 是绿色供应链公共服务平台本体，平台主线与公共服务平台本体，不是"公共服务平台"，不是"新增中间层"。
...
8. GPC 的内部流程引擎统一命名为"平台服务流程编排引擎 / 绿色供应链协同流程运行时 / 供应链公共服务流程引擎"，不再称为"虚拟工厂运行时"。
...
4. **不共享业务数据库**：GFIS、GPC、WAES、PVAOS、Brain 等系统通过 API、事件和证据引用协同，不直接共享主账表。
...
7. **WAES 不审批具体事务**：WAES 只审批治理规则、指标口径、AI 授权、证据确证和状态升级；工单、质量、库存、发货、签收等事务审批留在 GFIS 或 GPC。
8. **平台主架构优先**：当前绿链平台主线系统是 `GPC`；平台侧对象优先围绕 `GPC` 建模，不再回到 Odoo 式大平台建模。
9. **GPC 引擎命名**：GPC 内部流程引擎不得再以"虚拟工厂运行时"命名。
...
| 运营与协同层 | 平台协同、平台订单、需求池、供应商协作、产能协同、工厂分配、ASN、预约、运输、POD、异常协同、绿色绩效、合规证据包、服务开放、生态接入 | GPC | PlatformOrder、Supplier、Customer、Carrier、Vehicle、ASN、Appointment、Shipment、ProofOfDelivery、ExternalException、CrossFactoryException、FactoryAllocation、CapacityCommitment |
...
| L2 | 供应链公共服务层 | GPC | 平台订单、需求池、供应商协作、产能协同、工厂分配、ASN、预约、车辆、运输、签收、POD、外部异常、绿色绩效、合规证据包、服务开放 |
...
| 验收矩阵 | AcceptanceMatrix | `ACC-*` | WAES / Harness | GFIS / GPC | 否 | 否 | 否 | draft / ready / accepted / blocked | 阶段验收清单 |
| 验收场景 | AcceptanceScenario | `ACS-*` | WAES / Harness | GFIS / GPC | 否 | 否 | 否 | not_started / running / pass / fail / blocked | 单个验收用例 |
...
| 中文名 | 英文名 | ID 前缀 | 主责系统 | 协同系统 | 主数据 | 业务事实 | 允许 AI 写入 | 关键状态 | 说明 |
|---|---|---|---|---|---|---|---|---|---|
| 租户 | Tenant | `TEN-*` | PVAOS | WAES / GPC | 是 | 否 | 否 | active / suspended / archived | 体系内组织租户 |
| 组织 | Organization | `ORG-*` | PVAOS | GPC / WAES | 是 | 否 | 否 | active / inactive | 客户、供应商、承运商、园区、监管等组织实体 |
| 项目 | ProgramProject | `PRJ-*` | PVAOS | WAES / Brain | 是 | 否 | 否 | planning / active / paused / closed | 绿色供应链体系内项目 |
| 伙伴 | Partner | `PAR-*` | PVAOS | GPC | 是 | 否 | 否 | prospect / onboarded / suspended | 合作伙伴关系 |
| 门户账号 | PortalAccount | `PA-*` | PVAOS / GPC | WAES | 是 | 否 | 否 | invited / active / locked / revoked | 外部用户账号 |
| 角色授权 | RoleGrant | `RG-*` | PVAOS / WAES | GPC / GFIS | 否 | 否 | 否 | active / expired / revoked | 跨系统角色授权 |

### 4.3 L2 供应链公共服务对象（GPC 主账）

| 中文名 | 英文名 | ID 前缀 | 主责系统 | 协同系统 | 主数据 | 业务事实 | 允许 AI 写入 | 关键状态 | 说明 |
|---|---|---|---|---|---|---|---|---|---|
| 平台订单 | PlatformOrder | `PO-*` | GPC | PVAOS / GFIS / WAES | 否 | 是 | 否 | draft / confirmed / dispatched / fulfilled / cancelled | 公共服务平台订单 |
| 工厂订单映射 | OrderMapping | `OM-*` | GPC | GFIS | 否 | 是 | 否 | pending / linked / mismatch / closed | GPC 订单与 GFIS 订单关系 |
| 供应商 | Supplier | `SUP-*` | GPC | GFIS / PVAOS | 是 | 否 | 否 | onboarding / active / suspended / blacklisted | 外部供应商主数据 |
| 客户 | Customer | `CUS-*` | GPC | GFIS / PVAOS | 是 | 否 | 否 | prospect / active / suspended | 外部客户主数据 |
| 承运商 | Carrier | `CAR-*` | GPC | PVAOS | 是 | 否 | 否 | active / suspended / expired | 承运商主数据 |
| 车辆 | Vehicle | `VEH-*` | GPC | GFIS | 是 | 否 | 否 | active / unavailable / expired | 车辆档案 |
| 供应商到货通知 | ASN | `ASN-*` | GPC | GFIS | 否 | 是 | 否 | created / confirmed / arrived / received / cancelled | 供应商发货预告 |
| 预约 | Appointment | `APT-*` | GPC | GFIS | 否 | 是 | 否 | requested / confirmed / checked_in / completed / no_show / cancelled | 到货或发货预约 |
| 月台时段 | DockSlot | `DS-*` | GPC | GFIS | 是 | 否 | 否 | open / reserved / occupied / closed | 月台资源 |
| 发运单 | Shipment | `SHIP-*` | GPC | GFIS / WAES | 否 | 是 | 否 | planned / released / loaded / in_transit / delivered / exception / closed | 厂外发运与运输主对象 |
| 在途节点 | TransitCheckpoint | `TCP-*` | GPC | WAES | 否 | 是 | 可生成草案，不自动确认 | planned / reached / delayed / skipped | 运输在途节点 |
| 签收回单 | ProofOfDelivery | `POD-*` | GPC | GFIS / WAES | 否 | 是 | 否 | pending / signed / disputed / rejected / archived | 客户签收与回单 |
| 外部协同异常 | ExternalException | `EEX-*` | GPC | WAES / GFIS | 否 | 是 | 可生成草案，不自动关闭 | raised / assigned / resolving / closed / escalated | 客户、供应商、承运商协同异常 |
...
| 仓库 | Warehouse | `WH-*` | GFIS | GPC / WAES | 是 | 否 | 否 | active / blocked | 仓库 |
| 工厂订单 | FactoryOrder | `FO-*` | GFIS | GPC / WAES | 否 | 是 | 否 | accepted / planned / in_production / completed / shipped / cancelled | 工厂接收并确认的订单 |
| 物料 | Material | `MAT-*` | GFIS | GPC | 是 | 否 | 否 | active / blocked / obsolete | 物料主数据 |
| 批次 | MaterialLot | `LOT-*` | GFIS | GPC / WAES | 否 | 是 | 否 | pending_inspection / qualified / quarantined / rejected / consumed / shipped | 物料或成品批次 |
...
| 发货出库 | FactoryShipmentRelease | `FSR-*` | GFIS | GPC / WAES | 否 | 是 | 否 | pending / released / handed_over / reversed | 工厂发货出库事实 |
...
| 主数据映射 | MasterDataMapping | `MDM-*` | WAES / GPC / GFIS | 各系统 | 是 | 否 | 否 | proposed / active / conflict / retired | 跨系统主数据映射 |
| 指标定义 | MetricDefinition | `MET-*` | WAES | GFIS / GPC | 是 | 否 | 否 | draft / active / retired | 指标口径 |
...
| 业务审批引用 | BusinessApprovalReference | `BAR-*` | GFIS / GPC | WAES | 否 | 是 | 否 | referenced / missing / stale / invalid | 引用业务系统内部审批结果，WAES 不承办具体事务审批 |
...
| Agent 任务 | AgentTask | `AGT-*` | Hermes / WAES | XGD / XiaoC | 否 | 否 | 是 | queued / running / waiting_governance / completed / failed | Agent 执行任务；具体业务执行仍进入 GFIS/GPC |
...
| 中文名 | 英文名 | ID 前缀 | 主责系统 | 关键状态 | 说明 |
|---|---|---|---|---|---|
| 工厂分配规则 | FactoryAllocationRule | `FAR-*` | GPC / WAES | draft / active / retired | 一链多厂或多链多厂的订单分发规则 |
| 工厂分配结果 | FactoryAllocation | `FA-*` | GPC | proposed / confirmed / rejected / closed | 平台订单分配到工厂的结果；工厂执行仍由 GFIS 确认 |
...
| 库存可见性快照 | InventoryVisibilitySnapshot | `IVS-*` | GFIS / GPC | calculated / stale / invalid | 多厂库存可见性，不替代 GFIS 库存主账 |
...
| 跨厂异常 | CrossFactoryException | `CFE-*` | GPC / WAES | raised / assigned / resolving / closed | 多厂协同异常和责任链 |
