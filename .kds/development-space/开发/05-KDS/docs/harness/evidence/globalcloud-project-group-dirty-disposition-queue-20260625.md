...
| `DISP-AAAS-DELEGATE-REVIEW-20260628` | `GlobalCloud AAAS` | delegated loop gate wrapper 边界与最小 dirty/untracked | `noise_or_review_boundary_confirmation_required` | 仅确认 delegated wrapper replay 边界仍需人工 review；未确认前不进入 stage/commit/push | `git status --short --untracked-files=all`、`python3 tools/kds-sync/loop_document_gate.py --check-only`、`python3 tools/kds-sync/validate_project_group_external_loop_gate_delegate_baseline_20260627.py` | `globalcloud-project-group-external-loop-gate-delegate-baseline-20260627.md`、AAAS delegated replay 证据 | external delegate baseline gate、Git clean gate、authorization route | 未确认保持 delegated wrapper replay 边界；失败保持 `partial/rework` | 是，需要确认 review 边界 | 不声明 AAAS wrapper 已通过、不声明可提交或可推送 |
| `DISP-XWAIL-DELEGATE-REVIEW-20260628` | `GlobalCloud XWAIL` | delegated loop gate wrapper 边界与最小 dirty/untracked | `noise_or_review_boundary_confirmation_required` | 仅确认 delegated wrapper replay 边界仍需人工 review；未确认前不进入 stage/commit/push | `git status --short --untracked-files=all`、`python3 tools/kds-sync/loop_document_gate.py --check-only`、`python3 tools/kds-sync/validate_project_group_external_loop_gate_delegate_baseline_20260627.py` | `globalcloud-project-group-external-loop-gate-delegate-baseline-20260627.md`、XWAIL delegated replay 证据 | external delegate baseline gate、Git clean gate、authorization route | 未确认保持 delegated wrapper replay 边界；失败保持 `partial/rework` | 是，需要确认 review 边界 | 不声明 XWAIL wrapper 已通过、不声明可提交或可推送 |
...
| `DISP-GFIS-REAL-SOR-20260628` | `GlobalCloud GFIS` | repair boundary 与真实 source-of-record 待补证 | `owner_decision_required` | 维持 GFIS 真实 source-of-record 补证入口；未确认前不进入 stage/commit/push | `python3 tools/kds-sync/validate_gfis_real_fact_entry_gate.py`、`git status --short --untracked-files=all` | `gfis-real-runtime-baseline-20260624.md`、GFIS real SOR 相关证据 | GFIS source-record gate、authorization route | 未确认保持 `repair_required`；失败保持 `partial/rework` | 是，需要业务 owner/正式回执 | 不声明真实 source-of-record 已取得、不声明客户验收 |

## 3. 状态传导

| 输入 | 输出 | 限制 |
|---|---|---|
| `project_group_git_clean = blocked` | `dirty_disposition_queue_ready` | 只说明当前 7 个 live dirty 仓已有逐仓处置路径，不说明 Git clean |
| `project_group_review_packages = controlled` | `review_candidate_count = 1` | 当前只剩 GPCF 治理包继续作为 review 候选，不授权 stage/commit/push |
| `owner_review_required` | `owner_decision_count = 2` | KDS 与 SOP 必须 owner 决策后才能改变事实状态 |
| `noise_or_review_boundary_confirmation_required` | `noise_decision_count = 4` | WAS `.DS_Store` 与 AAAS/XWAIL delegated wrapper replay 边界、GFIS repair 边界均需人工确认 |
