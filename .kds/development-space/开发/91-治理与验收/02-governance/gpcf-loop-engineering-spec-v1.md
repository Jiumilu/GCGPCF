---
doc_id: GPCF-DOC-20CE547AAF
title: GPCF Loop Engineering v1.0 — 项目群工程闭环运行协议
project: WAES
...
owner: WAES
kds_space: 开发
kds_path: 开发/91-治理与验收/02-governance/gpcf-loop-engineering-spec-v1.md
source_path: 02-governance/gpcf-loop-engineering-spec-v1.md
...
4. 阻塞项必须在循环记录中显式列出
5. 文档治理门禁必须通过或显式登记文档债务

### 3.5 文档治理门禁

每轮 Loop 必须执行文档治理检查。该门禁由 `09-status/globalcloud-document-governance-loop-gate.md` 定义，执行命令为：
...
| KDS 同步 | KDS 开发空间镜像和同步流水一致 | `partial` / `blocked` |
| KDS TOKEN | 只有用户本人专用 TOKEN 可访问 `开发` 空间 | `blocked` |
| 防污染 | 旧命名、旧 AI 定位、真实性污染为 0 | `rework_required` |
| 文档债务 | 无未关闭文档债务 | 有债务时最高 `partial` |

关键约束：

1. 没有文档记录或明确“不需要文档变更”的说明，不得进入 `evidence_ready`。
2. KDS 本地镜像不得写成真实 KDS API 已同步。
3. TOKEN 缺失或权限不符合时，Loop 状态必须为 `blocked`，直到用户本人专用 TOKEN 配置完成。
4. 文档健康报告 `09-status/globalcloud-document-health-report.md` 是大循环状态升级的必备 evidence。
