...
| 下一轮 | 生成下一轮候选任务 |
| L3 连续推进 | 若 L3 session active 且未触发停止条件，必须继续下一轮；阶段性汇报不是停止条件 |

## L3 连续推进执行规则

L3 托管冲刺启动后，每轮收口必须先判断停止条件，再决定是否继续。默认判定为继续。
...
| 字段 | 允许值/要求 |
|---|---|
| 停止类型 | hard_stop / user_stop / budget_exhausted / time_exhausted / task_queue_empty |
| 停止证据 | 对应命令、门禁、用户指令或任务队列证据 |
| 是否符合 L3 停止规则 | yes / no |
| 已完成轮次 | `n/15` |
| 已用时间 | `x/120 分钟` |
| 下一步 | 继续、降级、等待确认或退出 L3 |
...
- L3 session active 时，收口后仍未触发停止条件的，必须继续下一轮，不得把阶段性汇报当作停止。
