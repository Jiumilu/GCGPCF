...
非授权类阻塞默认自动排查并修复，包括依赖服务、端口/路由漂移、环境加载、代码缺陷、测试失败、CodeGraph 索引、Git 同步与本地运行态问题。

除高危险操作或用户明确要求人工授权外，为完成当前任务所需的授权默认视为已提供，执行过程中不再逐项请求确认。

高危险操作包括：凭据创建、披露、持久化或扩大使用范围；不可逆、批量或生产环境真实数据写入与删除；KDS/MMC 生产或共享配置写入；权限提升或访问控制放宽；生产部署或对外发布；`accepted`、`integrated`、`production_ready`、`customer_accepted` 等状态提升。涉及此类操作时必须暂停，明确说明目标、影响范围、风险与回滚边界，取得人工授权后方可继续。

现有凭据在既有授权范围内使用、普通本地代码与文档写入、测试或开发环境配置、开发服务启动、验证和可回滚 Git 操作，默认自动授权。用户对任一事项明确要求人工授权时，以人工授权要求为准。

不得把可由工程排查解决的问题直接交回用户；必须持续执行，直至验证结果明确。
